r"""Support for simplified access to data on nodes of type isaacsim.robot.surface_gripper.SurfaceGripper
...
class OgnSurfaceGripperDatabase(og.Database):
    ...
    INTERFACE = og.Database._get_interface([
        ('inputs:SurfaceGripper', 'target', 0, None, 'The Surface Gripper prim to control', {}, True, None, False, ''),
        ('inputs:Toggle', 'execution', 0, None, 'Toggle the gripper', {}, True, None, False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.SurfaceGripper = og.AttributeRole.TARGET
        role_data.inputs.Toggle = og.AttributeRole.EXECUTION
        return role_data

    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"Toggle", "enabled", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.Toggle, self._attributes.enabled]
            self._batchedReadValues = [None, True]

        @property
        def SurfaceGripper(self):
            data_view = og.AttributeValueHelper(self._attributes.SurfaceGripper)
            return data_view.get()

        @SurfaceGripper.setter
        def SurfaceGripper(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.SurfaceGripper)
            data_view = og.AttributeValueHelper(self._attributes.SurfaceGripper)
            data_view.set(value)
            self.SurfaceGripper_size = data_view.get_array_size()
    ...
    class abi:
        ...
        def initialize_type(node_type):
            ...
            if needs_initializing:
                ...
                node_type.set_metadata(ogn.MetadataKeys.CATEGORIES, "isaacSurfaceGripper")
                node_type.set_metadata(ogn.MetadataKeys.CATEGORY_DESCRIPTIONS, "isaacSurfaceGripper,Surface Gripper inside Isaac Sim")
                node_type.set_metadata(ogn.MetadataKeys.DESCRIPTION, "Surface Gripper")
                node_type.set_metadata(ogn.MetadataKeys.LANGUAGE, "Python")
                icon_path = carb.tokens.get_tokens_interface().resolve("${isaacsim.robot.surface_gripper}")
                icon_path = icon_path + '/' + "ogn/icons/isaacsim.robot.surface_gripper.SurfaceGripper.svg"
    ...
    def register(node_type_class):
        ...
        og.register_node_type(OgnSurfaceGripperDatabase.abi, 2)
