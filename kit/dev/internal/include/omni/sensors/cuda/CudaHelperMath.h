...
// converters
////////////////////////////////////////////////////////////////////////////////
inline NV_HOSTDEVICE float3 toFloat3(const float* a)
{
    return make_float3(a[0], a[1], a[2]);
}

inline NV_HOSTDEVICE float4 toFloat4(const float* a)
{
...
inline NV_HOSTDEVICE float lengthsq(float2 v)
{
    return dot(v, v);
}
inline NV_HOSTDEVICE float lengthsq(float3 v)
{
    return dot(v, v);
}
inline NV_HOSTDEVICE float lengthsq(float4 v)
{
    return dot(v, v);
}
...
// length tests
////////////////////////////////////////////////////////////////////////////////
inline NV_HOSTDEVICE bool lengthGreater(const float2& v, const float threshold)
{
    return ((fabsf(v.x) > threshold) || (fabsf(v.y) > threshold));
}
inline NV_HOSTDEVICE bool lengthGreater(const float2& v, const float2& w, const float threshold)
{
    return dot(v, w) > threshold;
}
inline NV_HOSTDEVICE bool lengthGreaterEqual(const float2& v, const float threshold)
{
    return ((fabsf(v.x) >= threshold) || (fabsf(v.y) >= threshold));
}
inline NV_HOSTDEVICE bool lengthGreaterEqual(const float2& v, const float2& w, const float threshold)
{
    return dot(v, w) >= threshold;
}
inline NV_HOSTDEVICE bool lengthLess(const float2& v, const float threshold)
{
    return ((fabsf(v.x) < threshold) && (fabsf(v.y) < threshold));
}
inline NV_HOSTDEVICE bool lengthLess(const float2& v, const float2& w, const float threshold)
{
    return dot(v, w) < threshold;
}
inline NV_HOSTDEVICE bool lengthLessEqual(const float2& v, const float threshold)
{
    return ((fabsf(v.x) <= threshold) && (fabsf(v.y) <= threshold));
}
inline NV_HOSTDEVICE bool lengthLessEqual(const float2& v, const float2& w, const float threshold)
{
    return dot(v, w) <= threshold;
}
inline NV_HOSTDEVICE bool lengthGreater(const float3& v, const float threshold)
{
    return ((fabsf(v.x) > threshold) || (fabsf(v.y) > threshold) || (fabsf(v.z) > threshold));
}
inline NV_HOSTDEVICE bool lengthGreater(const float3& v, const float3& w, const float threshold)
{
    return dot(v, w) > threshold;
}
inline NV_HOSTDEVICE bool lengthGreaterEqual(const float3& v, const float threshold)
{
    return ((fabsf(v.x) >= threshold) || (fabsf(v.y) >= threshold) || (fabsf(v.z) >= threshold));
}
inline NV_HOSTDEVICE bool lengthGreaterEqual(const float3& v, const float3& w, const float threshold)
{
    return dot(v, w) >= threshold;
}
inline NV_HOSTDEVICE bool lengthLess(const float3& v, const float threshold)
{
    return ((fabsf(v.x) < threshold) && (fabsf(v.y) < threshold) && (fabsf(v.z) < threshold));
}
inline NV_HOSTDEVICE bool lengthLess(const float3& v, const float3& w, const float threshold)
{
    return dot(v, w) < threshold;
}
inline NV_HOSTDEVICE bool lengthLessEqual(const float3& v, const float threshold)
{
    return ((fabsf(v.x) <= threshold) && (fabsf(v.y) <= threshold) && (fabsf(v.z) <= threshold));
}
inline NV_HOSTDEVICE bool lengthLessEqual(const float3& v, const float3& w, const float threshold)
{
    return dot(v, w) <= threshold;
}
inline NV_HOSTDEVICE bool lengthGreater(const float4& v, const float threshold)
{
    return ((fabsf(v.x) > threshold) || (fabsf(v.y) > threshold) || (fabsf(v.z) > threshold) || (fabsf(v.w) > threshold));
}
inline NV_HOSTDEVICE bool lengthGreater(const float4& v, const float4& w, const float threshold)
{
    return dot(v, w) > threshold;
}
inline NV_HOSTDEVICE bool lengthGreaterEqual(const float4& v, const float threshold)
{
    return ((fabsf(v.x) >= threshold) || (fabsf(v.y) >= threshold) || (fabsf(v.z) >= threshold) ||
            (fabsf(v.w) >= threshold));
}
inline NV_HOSTDEVICE bool lengthGreaterEqual(const float4& v, const float4& w, const float threshold)
{
    return dot(v, w) >= threshold;
}
inline NV_HOSTDEVICE bool lengthLess(const float4& v, const float threshold)
{
    return ((fabsf(v.x) < threshold) && (fabsf(v.y) < threshold) && (fabsf(v.z) < threshold) && (fabsf(v.w) < threshold));
}
inline NV_HOSTDEVICE bool lengthLess(const float4& v, const float4& w, const float threshold)
{
    return dot(v, w) < threshold;
}
inline NV_HOSTDEVICE bool lengthLessEqual(const float4& v, const float threshold)
{
    return ((fabsf(v.x) <= threshold) && (fabsf(v.y) <= threshold) && (fabsf(v.z) <= threshold) &&
            (fabsf(v.w) <= threshold));
}
inline NV_HOSTDEVICE bool lengthLessEqual(const float4& v, const float4& w, const float threshold)
{
    return dot(v, w) <= threshold;
}
...
// fabsf of vectors
////////////////////////////////////////////////////////////////////////////////
inline NV_HOSTDEVICE float2 fabsf(float2& v)
{
    return make_float2(fabsf(v.x), fabsf(v.y));
}
inline NV_HOSTDEVICE float3 fabsf(float3& v)
{
    return make_float3(fabsf(v.x), fabsf(v.y), fabsf(v.z));
}
inline NV_HOSTDEVICE float4 fabsf(float4& v)
{
    return make_float4(fabsf(v.x), fabsf(v.y), fabsf(v.z), fabsf(v.w));
}

inline NV_HOSTDEVICE void inPlaceFabsf(float2& v)
{
    v.x = fabsf(v.x);
    v.y = fabsf(v.y);
}
inline NV_HOSTDEVICE void inPlaceFabsf(float3& v)
{
    v.x = fabsf(v.x);
    v.y = fabsf(v.y);
    v.z = fabsf(v.z);
}
inline NV_HOSTDEVICE void inPlaceFabsf(float4& v)
{
    v.x = fabsf(v.x);
    v.y = fabsf(v.y);
    v.z = fabsf(v.z);
...
//-----------------------------------------------------------------------------
inline NV_HOSTDEVICE float CToCC360To180Interval(float angle)
{
    const float ccAngle{ 360.f - angle };
    return ccAngle > 180.f ? ccAngle - 360.f : ccAngle;
}

//-----------------------------------------------------------------------------
inline NV_HOSTDEVICE float CCToC180To360Interval(float angle)
{
    float cAngle{ 360.f - angle };
    cAngle = cAngle < 0.f ? cAngle + 360.f : cAngle;
    return cAngle = cAngle > 360.f ? cAngle - 360.f : cAngle;
}
...
// inplace conversion from spherical to cartesian for ISO855 coords
NV_HOSTDEVICE inline void inPlaceToCartesianISO855(float& x, float& y, float& z)
{
    const float dist{ z };
    const float cosAzimuth{ cosf(Deg2Rad(x)) };
    const float sinAzimuth{ sinf(Deg2Rad(x)) };
    const float cosElevation{ cosf(Deg2Rad(y)) };
    const float sinElevation{ sinf(Deg2Rad(y)) };
    x = dist * cosAzimuth * cosElevation;
    y = dist * sinAzimuth * cosElevation;
    z = dist * sinElevation;
}

// inplace conversion from cartesian to spherical for ISO855 coords
NV_HOSTDEVICE inline void inPlaceToSphericalISO855(float& x, float& y, float& z)
{
    float3 res{ ToSpherical({ x, y, z }) };
    x = Rad2Deg(res.x);
    y = Rad2Deg(HALF_PI - res.y);
    z = res.z;
}
...
//-----------------------------------------------------------------------------
inline NV_HOSTDEVICE void assignQuatToHTM(const float* quat, float* htm)
{
    htm[0] = 2.0f * (quat[0] * quat[0] + quat[3] * quat[3]) - 1.0f;
    htm[1] = 2.0f * (quat[0] * quat[1] - quat[3] * quat[2]);
    htm[2] = 2.0f * (quat[1] * quat[2] + quat[3] * quat[1]);
    // htm[3] = 0.0f; // Don't overwrite translation
    htm[4] = 2.0f * (quat[0] * quat[1] + quat[3] * quat[2]);
    htm[5] = 2.0f * (quat[1] * quat[1] + quat[3] * quat[3]) - 1.0f;
    htm[6] = 2.0f * (quat[1] * quat[2] - quat[3] * quat[0]);
    // htm[7] = 0.0f;
    htm[8] = 2.0f * (quat[0] * quat[2] - quat[3] * quat[1]);
    htm[9] = 2.0f * (quat[1] * quat[2] + quat[3] * quat[0]);
    htm[10] = 2.0f * (quat[2] * quat[2] + quat[3] * quat[3]) - 1.0f;
    // htm[11] = 0.0f;
    //  Make sure the standard is followed
    htm[12] = 0.0f;
    htm[13] = 0.0f;
    htm[14] = 0.0f;
    htm[15] = 1.0f;
