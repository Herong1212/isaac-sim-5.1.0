...
/*************************************************************************
 * Data types for WPM compatible materials
 * **********************************************************************/

/**
 * @brief and enum that specifies a ray type
 *
 */
enum struct RayType : uint8_t
{
    R = 0, /**< Secondary, Reflection */
    T = 1, /**< Secondary, Refraction (Transmission) */
    I = 2, /**< Incidence Ray. Since every ray can be an incidence ray, this is only used for indexing the components
              that make up the contributions. */
    P = 3, /**< Primary */
};

/**
 * @brief a struct defining the input to a wpm compatible material, a wpm compatible material has some mandatory members
 * in the material input in addition to a custom properties data blob
 *
 */
struct MaterialInput
{
    uint32_t matId; /**< RtxSensor material id */
    uint32_t objId; /**< RtxSensor Object id */
    float distPrevHit; /**< distance since previous hit */
    float3 inRayDir; /**< unit vector defining incident ray direction */
    float3 matNormal; /**< unit vector defining the surface normal at incidence hit point */
    float3 outRayDir; /**< unit vector defining the output direction for BSDF lookup */
    RayType outRayType; /**< requested ray type, incase outRayDir is 0, will only be R ot T */
    CustomRayProps* inRayProps; /**< pointer to custom blob of data */
};

/**
 * @brief a struct defining the output of a wpm compatible material, a wpm compatible material has some mandatory
 * members in the material output in addition to a custom properties data blob
 * @note Material outputs are passed to materials as inputs to be modified
 *
 */
struct MaterialOutput
{
    float3 outRayDir; /**< unit vector defining the direction of requested output ray */
    CustomRayProps* outRayProps;
};
...
struct Config
{
    ...
    /** @brief enables calculating round trip path velocity in contributions (not yet supported) */
    bool enRtVelocity;
};
...
struct Transmitter
{
    ...
    Frame frame; /**< a frame of reference defining the pose of the transmitter relative to the sensor frame */
...
struct Receiver
{
    Frame frame; /**< a frame of reference defining the pose of the receiver relative to the sensor frame */
...
struct TraceDefinition
{
    ...
    Transmitter* transmitters1D{ nullptr };
...
struct Contribution
{
    ...
    float3 vertices[3]; /**< vertex positions for the hit triangle in detector frame */
...
struct TraceResult
{
     ...
     * + receiver2(...)+....] */
    Contribution* contributions4D{ nullptr };
...
public:
     ...
     */
    virtual TraceDefinition* getTraceDefinition(const Config& cfg) = 0;
     ...
     * @param matName an array of names of the materials as specified in the material plugins
     ...
     */
    virtual bool setupMaterials(const uint32_t* matId, const char** matName, uint32_t numMats) = 0;
