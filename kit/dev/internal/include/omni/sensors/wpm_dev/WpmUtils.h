...
struct Ray
{
    ...
    uint32_t targetDetGroupIdxInRx; /**< idx of target receiver group in corresponding receiver */
};
...
{
    return def->traceTreeDepth + def->contributionRayDepth + 1;
}
...
inline uint32_t utilGetMaxNumFirings(const Config& cfg)
{
    ...
    return numRaysLastDepthSingleTx * (1 + cfg.maxNumIdxsPerTarRxGroup);
}
...
{
    float3 forward = boreRay.direction;
    const float3 defaultUp[3] = { { 0, 1, 0 }, { 0, 0, 1 }, { 1, 0, 0 } };
    float3 right, up;

    resolveBasisVectors(up, right, forward, defaultUp);
