...
class HttpClient(Client):
    ...
    async def call(self, interface: InterfaceName, method: MethodName, request, request_type: Type[RequestType], return_type: Type[ResponseType]) -> ResponseType:
        ...
        response = await self.session.post(url, data=self._send(request), ssl_context=self.ssl_context)
...
class HttpServer(Server):
    ...
    def __init__(self, host: str, port: int, *, marshaller: Marshaller = None, ssl_context: ssl.SSLContext = None):
        if marshaller is None:
            try:
                marshaller = Marshaller(JSONSerializer())
            except (ImportError, ModuleNotFoundError):
    ...
    async def _handler(self, request: _Request, dispatcher: Dispatcher):
        ...
        try:
            ...
            Request.set(HttpRequest(interface, method, params, http=request))
            ...
            if call.returns_many:
                ...
                await response.prepare(request)
                await self._write(result, response, log_id)
    ...
    async def _write(self, data, response, log_id):
