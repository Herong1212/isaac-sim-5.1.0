    ...
    def __init__(self):
        ...
        try:
            ...
            with self:
                ...
                ui.MenuItem(f"{omni.kit.ui.get_custom_glyph_code('${glyphs}/plus.svg')}   Add at Current Selection", triggered_fn=self.__add_at_current_selection)
                ui.MenuItem(f"{omni.kit.ui.get_custom_glyph_code('${glyphs}/plus.svg')}   Replace Current Selection", triggered_fn=self.__replace_current_selection)
        except ImportError:
        ...
        with self:
            try:
                ...
                if has_collect or has_stage:
                    ui.Separator()
                ui.MenuItem(f"{omni.kit.ui.get_custom_glyph_code('${glyphs}/share.svg')}  Copy URL Link", triggered_fn=self.__copy_url_link)
            except ImportError:
