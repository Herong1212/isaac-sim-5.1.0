...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        {
            '': {
                'inputs': [
                    ...
                    ['inputs:z', {'type': 'double', 'value': 0}, False],
                    ['inputs:w', {'type': 'double', 'value': 0}, False],
                ],
                'outputs': [
                    ['outputs:tuple', {'type': 'double[4]', 'value': [42.0, 1.0, 0.0, 0.0]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:x', {'type': 'half', 'value': 42.0}, False],
                    ['inputs:y', {'type': 'half', 'value': 1.0}, False],
                    ['inputs:z', {'type': 'half', 'value': 0}, False],
                    ['inputs:w', {'type': 'half', 'value': 0}, False],
                ],
                'outputs': [
                    ['outputs:tuple', {'type': 'half[4]', 'value': [42.0, 1.0, 0.0, 0.0]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:x', {'type': 'double[]', 'value': [42, 1, 0]}, False],
                    ['inputs:y', {'type': 'double[]', 'value': [0, 1, 2]}, False],
                    ['inputs:z', {'type': 'double[]', 'value': [0, 2, 4]}, False],
                    ['inputs:w', {'type': 'double[]', 'value': [5, 6, 7]}, False],
                ],
                'outputs': [
                    ['outputs:tuple', {'type': 'double[4][]', 'value': [[42, 0, 0, 5], [1, 1, 2, 6], [0, 2, 4, 7]]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:x', {'type': 'half[]', 'value': [42, 1, 0]}, False],
                    ['inputs:y', {'type': 'half[]', 'value': [0, 1, 2]}, False],
                    ['inputs:z', {'type': 'half[]', 'value': [0, 2, 4]}, False],
                    ['inputs:w', {'type': 'half[]', 'value': [5, 6, 7]}, False],
                ],
                'outputs': [
                    ['outputs:tuple', {'type': 'half[4][]', 'value': [[42, 0, 0, 5], [1, 1, 2, 6], [0, 2, 4, 7]]}, False],
                ],
            },
        },
        {
            '': {
                'inputs': [
                    ['inputs:x', {'type': 'int[]', 'value': [42, 1, 0]}, False],
                    ['inputs:y', {'type': 'int[]', 'value': [0, 1, 2]}, False],
                    ['inputs:z', {'type': 'int[]', 'value': [0, 2, 4]}, False],
                    ['inputs:w', {'type': 'int[]', 'value': [5, 6, 7]}, False],
                ],
                'outputs': [
                    ['outputs:tuple', {'type': 'int[4][]', 'value': [[42, 0, 0, 5], [1, 1, 2, 6], [0, 2, 4, 7]]}, False],
                ],
            },
        },
    ]
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_MakeVector4", "omni.graph.nodes.MakeVector4", test_run, _TestGraphAndNode())
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_MakeVector4","omni.graph.nodes.MakeVector4", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
    ...
    async def test_thread_safety(self):
        ...
        for (key, test_info) in instance_setup.copy().items():
            instance_setup[key] = await _test_setup_scene(self, og.Controller(allow_exists_prim=True), key, "TestNode_omni_graph_nodes_MakeVector4", "omni.graph.nodes.MakeVector4", test_data_setup[key], test_info)
        ...
        for (key, test_info) in instance_setup.items():
            _test_verify_scene(self, og.Controller(), test_data_setup[key], test_info, f"omni.graph.nodes.MakeVector4 User test case instance{key}")
