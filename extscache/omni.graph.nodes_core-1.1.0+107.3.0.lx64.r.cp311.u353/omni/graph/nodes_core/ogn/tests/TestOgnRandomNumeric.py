...
class TestOgn(ogts.OmniGraphTestCase):
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:useSeed', True, False],
                    ['inputs:seed', 1955209015103813879, False],
                    ['inputs:min', {'type': 'double[2][]', 'value': [[0, 1], [10, -10]]}, False],
                    ['inputs:max', {'type': 'double', 'value': 1}, False],
                    ['inputs:execIn', 1, False],
                    ['inputs:isNoise', True, False],
                ],
                'outputs': [
                    ['outputs:random', {'type': 'double[2][]', 'value': [[0, nan], [nan, 0.28955788]]}, False],
                    ['outputs:execOut', 1, False],
                ],
            },
        },
        ...
        },
        {
            '': {
                'inputs': [
                    ...
                    ['inputs:max', {'type': 'uchar[]', 'value': [255, 100]}, False],
                    ['inputs:execIn', 1, False],
                    ['inputs:isNoise', True, False],
                ],
                'outputs': [
                    ['outputs:random', {'type': 'uchar[]', 'value': [153, 100]}, False],
                    ['outputs:execOut', 1, False],
                ],
            },
        },
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_RandomNumeric", "omni.graph.nodes.RandomNumeric", test_run, _TestGraphAndNode())
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_RandomNumeric","omni.graph.nodes.RandomNumeric", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
    ...
    async def test_thread_safety(self):
        ...
        for (key, test_info) in instance_setup.copy().items():
            instance_setup[key] = await _test_setup_scene(self, og.Controller(allow_exists_prim=True), key, "TestNode_omni_graph_nodes_RandomNumeric", "omni.graph.nodes.RandomNumeric", test_data_setup[key], test_info)
        ...
        for (key, test_info) in instance_setup.items():
            _test_verify_scene(self, og.Controller(), test_data_setup[key], test_info, f"omni.graph.nodes.RandomNumeric User test case instance{key}")
