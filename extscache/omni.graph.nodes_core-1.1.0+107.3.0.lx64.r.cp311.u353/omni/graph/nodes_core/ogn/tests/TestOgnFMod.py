...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:a', {'type': 'float', 'value': 4.0}, False],
                    ['inputs:b', {'type': 'float', 'value': 0.0}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'float', 'value': 0.0}, False],
                ],
            },
        },
        {
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_FMod", "omni.graph.nodes.FMod", test_run, _TestGraphAndNode())
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_FMod","omni.graph.nodes.FMod", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
