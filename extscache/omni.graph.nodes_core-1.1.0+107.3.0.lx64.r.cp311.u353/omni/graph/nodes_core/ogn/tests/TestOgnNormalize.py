...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        {
            '': {
                'inputs': [
                    ['inputs:vector', {'type': 'double[2]', 'value': [1.0, 1.0]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'double[2]', 'value': [0.70710677, 0.70710677]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:vector', {'type': 'double[4]', 'value': [1.0, 2.0, 3.0, 4.0]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'double[4]', 'value': [0.18257418, 0.36514837, 0.5477226, 0.73029673]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:vector', {'type': 'half[2]', 'value': [1.0, 1.0]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'half[2]', 'value': [0.70703125, 0.70703125]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:vector', {'type': 'half[4]', 'value': [1.0, 2.0, 3.0, 4.0]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'half[4]', 'value': [0.18261719, 0.36523438, 0.54785156, 0.73046875]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                ...
                'outputs': [
                    ['outputs:result', {'type': 'half[3][]', 'value': [[0.26733398, 0.53466797, 0.8017578], [0.45581055, 0.5698242, 0.68408203]]}, False],
                ],
            },
        },
        {
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_Normalize", "omni.graph.nodes.Normalize", test_run, _TestGraphAndNode())
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_Normalize","omni.graph.nodes.Normalize", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
    ...
    async def test_thread_safety(self):
        ...
        for (key, test_info) in instance_setup.copy().items():
            instance_setup[key] = await _test_setup_scene(self, og.Controller(allow_exists_prim=True), key, "TestNode_omni_graph_nodes_Normalize", "omni.graph.nodes.Normalize", test_data_setup[key], test_info)
        ...
        for (key, test_info) in instance_setup.items():
            _test_verify_scene(self, og.Controller(), test_data_setup[key], test_info, f"omni.graph.nodes.Normalize User test case instance{key}")
