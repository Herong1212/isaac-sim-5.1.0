...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        ...
        },
        {
            '': {
                ...
                'outputs': [
                    ['outputs:result', {'type': 'vectorf[3]', 'value': [81, 32, 3]}, False],
                ],
            },
        },
        {
            '': {
                'inputs': [
                    ['inputs:matrix', {'type': 'matrixd[4]', 'value': [1, 0, 0, 0, 0, 1, 0, 0, 10, 10, 1, 0, 50, 0, 0, 1]}, False],
                    ['inputs:vector', {'type': 'vectorh[3]', 'value': [1, 2, 3]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'vectorh[3]', 'value': [81, 32, 3]}, False],
                ],
            },
        },
        {
            '': {
                'inputs': [
                    ['inputs:matrix', {'type': 'matrixd[3]', 'value': [1, 0, 0, 0, 0, 1, 0, -1, 0]}, False],
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:matrix', {'type': 'matrixd[3]', 'value': [1, 0, 0, 0, 0, 1, 0, -1, 0]}, False],
                    ['inputs:vector', {'type': 'vectorh[3]', 'value': [0, 0, 1]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'vectorh[3]', 'value': [0, -1, 0]}, False],
                ],
            },
        },
        {
            '': {
                ...
                'outputs': [
                    ['outputs:result', {'type': 'vectord[3][]', 'value': [[81, 32, 3], [81, 32, 3]]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                ...
                'outputs': [
                    ['outputs:result', {'type': 'vectorh[3][]', 'value': [[81, 32, 3], [81, 32, 3]]}, False],
                ],
            },
        },
        {
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:matrix', {'type': 'matrixd[4]', 'value': [1, 0, 0, 0, 0, 1, 0, 0, 10, 10, 1, 0, 50, 0, 0, 1]}, False],
                    ['inputs:vector', {'type': 'vectord[3][]', 'value': [[1, 2, 3], [1, 2, 3]]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'vectord[3][]', 'value': [[81, 32, 3], [81, 32, 3]]}, False],
                ],
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:matrix', {'type': 'matrixd[4]', 'value': [1, 0, 0, 0, 0, 1, 0, 0, 10, 10, 1, 0, 50, 0, 0, 1]}, False],
                    ['inputs:vector', {'type': 'vectorh[3][]', 'value': [[1, 2, 3], [1, 2, 3]]}, False],
                ],
                'outputs': [
                    ['outputs:result', {'type': 'vectorh[3][]', 'value': [[81, 32, 3], [81, 32, 3]]}, False],
                ],
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_TransformVector", "omni.graph.nodes.TransformVector", test_run, _TestGraphAndNode())
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_TransformVector","omni.graph.nodes.TransformVector", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
    ...
    async def test_thread_safety(self):
        ...
        for (key, test_info) in instance_setup.copy().items():
            instance_setup[key] = await _test_setup_scene(self, og.Controller(allow_exists_prim=True), key, "TestNode_omni_graph_nodes_TransformVector", "omni.graph.nodes.TransformVector", test_data_setup[key], test_info)
        ...
        for (key, test_info) in instance_setup.items():
            _test_verify_scene(self, og.Controller(), test_data_setup[key], test_info, f"omni.graph.nodes.TransformVector User test case instance{key}")
