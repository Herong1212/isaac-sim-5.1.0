...
class GoToSection(Command):
    ...
    def is_valid_command(
        ...
        try:
            ...
            carb.log_warn("failing due to import error in gotosection")
            return False
    ...
    def setup(self):
        ...
        if Utils.get_navmesh_area_index(self.section_name) != -1:
            ...
            carb.log_error(
                "character {character_name} attempts to access an invalid section {section}. ".format(
                    character_name=self.character_name, section=self.section_name
                )
            )
