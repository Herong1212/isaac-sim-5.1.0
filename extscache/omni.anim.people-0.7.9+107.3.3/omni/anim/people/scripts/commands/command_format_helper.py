...
def cast_to_type(value: Any, target_type: type) -> Any | None:
    ...
    try:
        ...
        if (target_type == list or get_origin(target_type) == list) and isinstance(value, list):
            # Assuming `target_type` is `list[float]` (or nested lists)
            inner_type_tuple = get_inner_type(target_type)  # This function should be defined elsewhere
...
def can_cast_to_type(value: Any, target_type: type) -> bool:
    return not cast_to_type(value=value, target_type=target_type) is None
...
class CommandParamter:
    ...
    def __init__(
        ...
        self.name: str = name  # place holder name of this parameter, would be shown when showing command format
        ...
        if special_value and isinstance(special_value, tuple) and len(special_value) == 3:
            self.special_value = special_value  # record the special value and type of this parameter
    ...
    def get_normal_parameter(self, parsed_command: Any) -> Any:
        ...
        if isinstance(parsed_command, list) and len(parsed_command) == 1 and get_origin(self.param_type) != list:
    ...
    def get_special_parameter(self, parsed_command: Any) -> Any:
        target_input = parsed_command
        if isinstance(parsed_command, list) and len(parsed_command) == 0 and get_origin(self.param_type) != list:
            target_input = parsed_command[0]
...
class CommandFormatHelper:
    ...
    command_description: str | None = None  # varaible that record command description
    ...
    def validate_command_format(cls, command: list[str] | str) -> dict[str, Any] | None:
        ...
        while input_index < command_len and parameter_index < parameter_len:
            ...
            if parameter.special_value:
                ...
                special_parameter = parameter.get_special_parameter(
        ...
        if parameter_index < parameter_len:
            ...
            # check whether all missing paraemter is optional
