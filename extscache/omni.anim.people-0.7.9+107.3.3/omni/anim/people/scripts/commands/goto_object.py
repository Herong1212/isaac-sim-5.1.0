...
class GoToObject(Command):
    ...
    def is_valid_command(
        ...
        try:
            ...
            if not (object_prim and object_prim.IsValid() and object_prim.IsActive()):
                carb.log_warn(
                    "{target_object_path} is not a valid prim in stage".format(target_object_path=target_object_path)
                )
                return False
            ...
            carb.log_warn("command is not valid, error occur")
            return False
    ...
    def set_command_description_usage(cls):
        ...
        cls.command_usage = "Navigate the character to a target object. Interaction commnads could be added after this command to mimic the interact between character and target object."
