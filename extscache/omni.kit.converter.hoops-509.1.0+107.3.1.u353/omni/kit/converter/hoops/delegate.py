...
from omni.kit.converter.hoops_core import HoopsConverterHelper, HoopsOptions
from omni.kit.notification_manager import NotificationStatus, post_notification
from pxr import Usd, UsdUtils

__all__ = ["HoopsConverterDelegate"]

APP_NAME = "omni.kit.converter.hoops"
HOOPS_LAUNCH_SCRIPT_PATH = "/omni/kit/converter/hoops/process/launch_hoops_app.py"
...
class HoopsConverterDelegate(ai.AbstractImporterDelegate):
    ...
    def __init__(self, name, filters, descriptions):
        super().__init__()
        self._hoops_options = HoopsOptions()
        self._hoops_options_builder = CadConverterOptionsBuilder(options_model=self._hoops_options)
        self._name = name
    ...
    async def convert_assets(self, paths, **kargs) -> Dict[str, Union[str, None]]:
        ...
        import_to_stage = kargs["import_to_stage"] if "import_to_stage" in kargs else True
        ...
        for file_path in paths:
            ...
            if not file_path:
                post_notification(f"Invalid path: {file_path}", duration=4, status=NotificationStatus.WARNING)
                continue
            ...
            self._hoops_options.sOptimizeConfig = scene_optimizer_str
    ...
    def create_temp_json(self, import_path: str) -> str:
        ...
        with open(default_config_json_path, "w") as outfile:
            json.dump(self._hoops_options.toArgs(), outfile, indent=4)
            outfile.close()
    ...
    async def launch_kit_app(self, file_path: str, output_path: str) -> tuple[int, str]:
        ...
        script_path = Path(ext_path + HOOPS_LAUNCH_SCRIPT_PATH)
        ...
        args.extend(["--enable", "omni.kit.converter.hoops_core", "--exec"])
        ...
        # Call the script that invokes kit with HOOPS Converter
        ...
        log_consumer = ProgressLogConsumer("[omni.converter.hoops_progress]")
