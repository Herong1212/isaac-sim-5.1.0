r"""Support for simplified access to data on nodes of type omni.graph.action.OnStageEvent
...
when certain USD stage-related actions are performed by the system:
Saved: USD file saved.
Selection Changed: USD Prim selection
has changed.
Hierarchy Changed: USD stage hierarchy has changed, e.g. a prim is added, deleted or moved.
OmniGraph Start
Play: OmniGraph started.
...
Animation Stop Play: Animation playback has
stopped.
"""
...
class OgnOnStageEventDatabase(og.Database):
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:onlyPlayback', 'bool', 0, 'Only Simulate On Play', 'When true, the node is only executed while the Stage is being played.', {ogn.MetadataKeys.LITERAL_ONLY: '1', ogn.MetadataKeys.DEFAULT: 'true'}, True, True, False, ''),
        ('outputs:execOut', 'execution', 0, None, 'After the stage event is received, signal to the graph that execution can continue downstream.', {}, True, None, False, ''),
    ])
    ...
    class abi:
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnOnStageEventDatabase.abi.initialize(context, n)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnOnStageEventDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnOnStageEventDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnOnStageEventDatabase._release_per_node_instance_data(node, graph_instance_id)
        ...
        def initialize_type(node_type):
            ...
            if needs_initializing:
                ...
                node_type.set_metadata(ogn.MetadataKeys.DESCRIPTION, "Activates the downstream graph execution when the specified 'Event Name' from the Stage occurs. Stage Events are emitted when certain USD stage-related actions are performed by the system:\nSaved: USD file saved.\nSelection Changed: USD Prim selection has changed.\nHierarchy Changed: USD stage hierarchy has changed, e.g. a prim is added, deleted or moved.\nOmniGraph Start Play: OmniGraph started.\nOmniGraph Stop Play: OmniGraph stopped.\nSimulation Start Play: Simulation started.\nSimulation Stop Play: Simulation stopped.\nAnimation Start Play: Animation playback has started.\nAnimation Stop Play: Animation playback has stopped.")
    ...
    def register(node_type_class):
        ...
        og.register_node_type(OgnOnStageEventDatabase.abi, 4)
