r"""Support for simplified access to data on nodes of type omni.graph.action.OnCustomEvent
...
class OgnOnCustomEventDatabase(og.Database):
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('outputs:bundle', 'bundle', 0, 'Bundle', "Bundle received with the event. The contents of the bundle are determined by the input bundle\ngiven to the corresponding 'SendCustomEvent' node.", {}, True, None, False, ''),
        ('outputs:execOut', 'execution', 0, 'Received', 'When the custom event was received signal to the graph that execution can continue downstream.', {}, True, None, False, ''),
    ...
    class abi:
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnOnCustomEventDatabase.abi.initialize(context, n)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnOnCustomEventDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnOnCustomEventDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnOnCustomEventDatabase._release_per_node_instance_data(node, graph_instance_id)
        ...
        def initialize_type(node_type):
            ...
            if needs_initializing:
                ...
                node_type.set_metadata(ogn.MetadataKeys.DESCRIPTION, "Event node which fires when the specified custom event is sent. This node is used in combination with 'omni.graph.action.SendCustomEvent'.")
    ...
    def register(node_type_class):
        ...
        og.register_node_type(OgnOnCustomEventDatabase.abi, 3)
