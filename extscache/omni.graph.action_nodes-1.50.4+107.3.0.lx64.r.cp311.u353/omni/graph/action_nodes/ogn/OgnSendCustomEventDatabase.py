r"""Support for simplified access to data on nodes of type omni.graph.action.SendCustomEvent
...
Sends a custom event, which will asynchronously trigger any 'On Custom Event' nodes which are listening for the same 'Event
Name'.
"""
...
class OgnSendCustomEventDatabase(og.Database):
    ...
    INTERFACE = og.Database._get_interface([
        ('inputs:bundle', 'bundle', 0, 'Bundle', "Bundle of data with information that is to be sent with the event. The payload for the event\nconsists of a dictionary whose keys are the names of the attributes in this bundle and whose\nvalues are the values of the named attribute in Python pickled format. Optionally the value\nof the 'Path' attribute is also added to the payload.", {}, False, None, False, ''),
        ('inputs:eventName', 'token', 0, 'Event Name', 'The name of the custom event to be sent out.', {}, True, "", False, ''),
        ('inputs:execIn', 'execution', 0, None, 'Signal to the graph that this node is ready to be executed.', {}, True, None, False, ''),
        ('inputs:path', 'token', 0, 'Path', "The path associated with the event. If specified, the path is added to the payload for the event\nunder the dictionary key '!path'.", {}, True, "", False, ''),
    ...
    class abi:
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnSendCustomEventDatabase.abi.initialize(context, n)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnSendCustomEventDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnSendCustomEventDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnSendCustomEventDatabase._release_per_node_instance_data(node, graph_instance_id)
        ...
        def initialize_type(node_type):
            ...
            if needs_initializing:
                ...
                node_type.set_metadata(ogn.MetadataKeys.DESCRIPTION, "Sends a custom event, which will asynchronously trigger any 'On Custom Event' nodes which are listening for the same 'Event Name'.")
    ...
    def register(node_type_class):
        ...
        og.register_node_type(OgnSendCustomEventDatabase.abi, 2)
