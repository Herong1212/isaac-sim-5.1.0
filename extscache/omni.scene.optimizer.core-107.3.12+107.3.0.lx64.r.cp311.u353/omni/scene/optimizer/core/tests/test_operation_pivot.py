...
# Remark:
# TODO: use primvars:omni:kit:isGizmo are likely authored carefully with dedicated purpose and should not be changed, e.g., Cameras.
# For now we decided to handle nested Gprims. Hence checking that the inverse has been added to the bottom of the local transform stack of each child.
...
class Test_Operation_Pivot(Test_Operation):
    ...
    async def test_Pivot_command(self):
        ...
        for file in get_test_files():
            ...
            args = {"meshPrimPaths": []}
            cmd = commands.SceneOptimizerOperation(operation="pivot", args=args)
            cmd.do()
