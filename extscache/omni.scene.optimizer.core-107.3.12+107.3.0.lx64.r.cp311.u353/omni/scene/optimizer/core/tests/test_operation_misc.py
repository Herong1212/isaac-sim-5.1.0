...
class Test(omni.kit.test.AsyncTestCase):
    async def setUp(self):
        ...
        # Default values for merge attributes.
        self._merge_args = {
            "meshPrimPaths": [],
        ...
        # Arguments for Merge Geometry that mimic the behaviour of the old "Merge At Leaf" operation.
        self._merge_at_leaf_args = {
            ...
            "rootPath": "",
            "mergePoint": 1,  # Merge at first xformable parent
        }
    ...
    def _get_scene(self, filename):
        extension_path = omni.kit.app.get_app().get_extension_manager().get_extension_path_by_module(__name__)
        test_data_path = pathlib.Path(extension_path).joinpath("data")
        test_file_path = test_data_path.joinpath(filename).absolute()
        return str(test_file_path)

    def _load_scene(self, filename):
        test_file_path = self._get_scene(filename)

        usd_context = omni.usd.get_context()
        success = usd_context.open_stage(test_file_path)
        if not success:  # pragma: no cover
            raise Exception("Failed to open usd file")
    ...
    async def _do_merge(self, stage, args: dict):
        ...
        merge = commands.SceneOptimizerOperation("merge", args=args)
        merge.do()
    ...
    async def _do_deduplicate_geometry(self, stage, args: dict):
        before_meshes = _get_meshes(stage)
        cmd = commands.SceneOptimizerOperation(operation="deduplicateGeometry", args=args)
        cmd.do()
        after_meshes = _get_meshes(stage)
        new_meshes = set(after_meshes) - set(before_meshes)
        return before_meshes, after_meshes, new_meshes
    ...
    async def test_Merge_no_considerMaterials(self):
        ...
        stage = self._load_scene("threeLambertShaders.usd")
    ...
    async def test_Merge_no_uvs(self):
        # Test case is 3 meshes with different materials.
        # This case ignores material diffs and merges all meshes into one.
        stage = self._load_scene("meshNoUVs.usd")
        before_meshes, after_meshes, new_meshes = await self._do_merge(stage, self._merge_args)
    ...
    async def test_Merge_faceVarying_uvs(self):
        # Test case is 3 meshes with different materials.
        # This case ignores material diffs and merges all meshes into one.
        stage = self._load_scene("license_plate.usd")
        before_meshes, after_meshes, new_meshes = await self._do_merge(stage, self._merge_args)
    ...
    async def test_Merge_per_face(self):
        ...
        stage = self._load_scene("twoPlanesOneHasPerFaceMaterial.usda")
        _merge_args = self._merge_args
    ...
    async def test_Merge_per_face_same_material(self):
        ...
        stage = self._load_scene("twoBluePlanes.usd")
        _merge_args = self._merge_args
        _merge_args["considerMaterials"] = False
        _, _, new_meshes = await self._do_merge(stage, _merge_args)
        new_prim = new_meshes.pop()
    ...
    async def test_Merge_per_face_missing_material(self):
        ...
        stage = self._load_scene("twoPlanesOneWithoutMaterial.usda")
        _merge_args = self._merge_args
        _merge_args["considerMaterials"] = False
        _, _, new_meshes = await self._do_merge(stage, _merge_args)
        new_prim = new_meshes.pop()
    ...
    async def test_Merge_preserve_st_indices_size(self):
        ...
        stage = self._load_scene("kit_not_camera.usd")
    ...
    async def test_Merge_check_camera_ignored(self):
        ...
        stage = self._load_scene("kit_camera.usd")
    ...
    async def test_MergeLevel_invalid_xform_name(self):
        ...
        stage = self._load_scene("fourMeshesAtLevelOne.usd")
        _merge_args = self._merge_args
    ...
    async def test_MergeLevel_group_at_leaf(self):
        ...
        stage = self._load_scene("twoMeshPairsLevelTwo.usd")
        _merge_args = self._merge_at_leaf_args
    ...
    async def test_MergeAtLeafReference(self):
        ...
        stage = self._load_scene("instancedMesh.usd")
        _merge_args = self._merge_args
    ...
    async def test_MergeAtLeaf(self):
        ...
        stage = self._load_scene("groupingScene.usd")
        _merge_args = self._merge_at_leaf_args
        _, _, new_meshes = await self._do_merge(stage, _merge_args)
    ...
    async def test_MergeReferenceNoMaterials(self):
        ...
        stage = self._load_scene("meshNoMaterial.usd")
        _merge_args = self._merge_args
    ...
    async def test_MergeReferenceMaterials(self):
        ...
        stage = self._load_scene("referencedMesh.usd")
        _merge_args = self._merge_args
        _merge_args["meshPrimPaths"] = ["/Root"]
        _merge_args["originalGeomOption"] = ORIGINAL_GEOM_IGNORE
        _, _, new_meshes = await self._do_merge(stage, _merge_args)
    ...
    async def test_Merge_MaterialNameCollisions(self):
        ...
        stage = self._load_scene("materialNameCollision.usd")
        _merge_args = self._merge_args
        _merge_args["meshPrimPaths"] = ["/Root"]
    ...
    async def test_DeduplicateGeometryCycle(self):
        ...
        stage = self._load_scene("cycleSource.usda")
    ...
    async def test_DeepDeeduplicateFive(self):
        ...
        stage = self._load_scene("localAndWorldSpaceMeshes.usd")
    ...
    async def test_JsonParserAllCommands(self):
        ...
        stage = self._load_scene("fourMeshesAtLevelOne.usd")
        filepath = self._get_scene("all_operations.json")
    ...
    async def test_Normals(self):
        ...
        stage = self._load_scene("normalsTest.usda")
        _merge_args = self._merge_args
        _merge_args["originalGeomOption"] = ORIGINAL_GEOM_DELETE
    ...
    async def test_Colors(self):
        ...
        stage = self._load_scene("cpv_two_planes_skewed.usda")
        _merge_args = self._merge_args
        _merge_args["originalGeomOption"] = ORIGINAL_GEOM_DELETE
    ...
    async def test_pathResolver(self):
        ...
        stage = self._load_scene("pathResolverTest.usda")
    ...
    async def test_computeExtents(self):
        ...
        # Load the test stage
        stage = self._load_scene("extentsTest.usda")
        ...
        filepath = self._get_scene("extentsTest.json")
        status = standalone.execute_commands_from_json(stage, filepath)
        self.assertTrue(status)
        ...
        args = {"meshPrimPaths": []}
        cmd = commands.SceneOptimizerOperation(operation="computeExtents", args=args)
        cmd.do()
    ...
    async def test_computeExtentTimeSamples(self):
        ...
        stage = self._load_scene("extentsTestTimeSamples.usda")
    ...
    async def test_executeJson(self):
        ...
        # Assert executing via command
        args = {"jsonFile": self._get_scene("extentsTest.json")}
        cmd = commands.SceneOptimizerJsonParser(args)
    ...
    async def test_delete_prims(self):
        ...
        result = commands.IFACE.delete_prims(opts, ["/World/CubeNoExtent"])
