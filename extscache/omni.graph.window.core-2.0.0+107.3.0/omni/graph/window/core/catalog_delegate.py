...
#
__all__ = ["OmniGraphCatalogTreeDelegate"]
...
class OmniGraphCatalogTreeDelegate(GraphEditorCoreTreeDelegate):
    ...
    def build_section_icon(self, model, item, column_id, level, expanded):
        ...
        if icon_model and category_model:
            ...
            self._build_icon(thumbnail, 1, self._icon_size, 9, category)
        ui.Spacer(width=10)
    ...
    def build_item_widget(self, model, item, column_id, level, expanded):
        """override build_item_widget in graph_editor_core_tree_delegate"""
        ...
        with ui.ZStack(tooltip_fn=lambda: _create_tooltip(tooltip)):
            super().build_item_widget(model, item, column_id, level, expanded)
