...
class CacheStateDelegate(ui.MenuDelegate):
    ...
    def __init__(self, cache_enabled, hub_enabled, **kwargs):
        ...
        self.ngc_api_key_is_token = config["ngc_api_key_is_token"]
    ...
    async def get_latest_version(self) -> str:
        ...
        try:
            self.ngc_token = await self.get_token()
            ...
            carb.log_warn(f"Failed to check if new version is available: {e}")
        return ""
    ...
    async def get_token(self) -> str:
        if self.ngc_api_key_is_token:
            return self.ngc_api_key
        return await get_token(self.ngc_api_key, self.ngc_org, self.ngc_team)
            ...
            async with self.create_session_with_headers(self.ngc_token) as client:
                async with client.get(uri) as response:
                    if response.ok:
                        ...
                        carb.log_error(f"Failed to download the file with code {response.code}: {response.reason}")
        except Exception as e:
            carb.log_error(f"Failed to download the file: {e}")
    ...
    async def check_latest_hub_version(self, resource: str) -> str:
        ...
        try:
            ...
            carb.log_error(f"Failed to download hub: ${e}")
        return ""
    ...
    async def get_correct_file(self, ngc_resource: str, hub_version: str) -> str:
        ...
        try:
            ...
            carb.log_error(f"Failed to get the proper file: ${e}")
        return ""

    def mark_hub_uninstalled(self, exclude: str):
        path = get_pck_install_folder(self.pck_install_path, get_omniverse_config())
        carb.log_info(f"Local install path found: {path} - {self.pck_install_path}")
        ...
        for entry in os.listdir(path):
            ...
            # Check if it's a directory and its name starts with "hub-"
            if os.path.isdir(entry_path) and entry.startswith("hub-"):
                version = entry.replace("hub-", "")
                install_marker = os.path.join(entry_path, ".installed")
                if os.path.exists(install_marker):
                    if version == exclude:
                        carb.log_info(f"Excluding Hub-{version} from uninstallation.")
                if os.path.exists(install_marker) and version != exclude:
                    carb.log_info(f"Marking Hub-{version} as uninstalled.")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    carb.log_error(f"Failed to move '{source_folder}' to '{destination_folder}': {e}")
            ...
            # mark prior version of hub as uninstalled
            self.mark_hub_uninstalled(exclude=hub_version)

            if await self.try_bounce_hub() and await self.test_hub_version(hub_version):
                asyncio.ensure_future(self._confirm_hub_upgrade(hub_version))
                self.toggle_ui_state(UIState.HUB_RUNNING)
            else:
                omni.kit.app.get_app().try_cancel_shutdown(
                    reason="Hub installation: Asking for User confirmation to restart."
                )
                asyncio.ensure_future(self._prompt_for_confirmation_to_quit(hub_version))
                self.toggle_ui_state(UIState.HUB_INSTALLED)

        except Exception as e:
            carb.log_error(f"Failed to download hub: ${e}")

    async def _prompt_for_confirmation_to_quit(self, version) -> None:
        ...
        try:
            ...
            confirm_dialog = Prompt(
                title=f"{ui.get_custom_glyph_code('${glyphs}/exclamation.svg')} Hub installed!",
                text=f"Hub version {version} has been installed. For the new installation to take effect, please restart your application.",
                ok_button_text="Ok",
    ...
    async def _confirm_hub_upgrade(self, version) -> None:
        """Inform the user that the installation was successful."""
        ...
        try:
            ...
            confirm_dialog = Prompt(
                title=f"{ui.get_custom_glyph_code('${glyphs}/exclamation.svg')} Hub upgraded!",
                text=f"Hub has been upgraded to version {version}.",
                ok_button_text="Ok",
                cancel_button_fn=lambda: confirm_dialog.hide(),
                modal=True,
            )
    ...
    async def _on_settings_open(self):
        ...
        try:
            async with self.create_session_with_headers(None) as client:
    ...
    async def get_hub_settings_url(self, default="http://127.0.0.1:14090"):
        [result, uri] = await omni.client.get_hub_http_uri_async()
        if result == omni.client.Result.OK and uri:
            return uri
        return default

    async def try_bounce_hub(self):
        """
        We can infer whether Hub is running by whether the settings url call
        returns a value. The bounce is only successful if Hub is running and
        the shutdown API call succeeds.
        """

        settings_url = await self.get_hub_settings_url(None)
        if not settings_url:
            return False

        parsed_url = urlparse(settings_url)
        shutdown_url = urlunparse(parsed_url._replace(path="/api/1.0/shutdown"))

        try:
            async with self.create_session_with_headers(None) as client:
                async with client.get(shutdown_url) as response:
                    if response.ok:
                        carb.log_info(f"Hub shutdown succeeded on: {shutdown_url}")
                        return True
        except Exception as e:
            carb.log_info(f"Hub shutdown failed on: {shutdown_url}, {e}") 
        return False

    async def test_hub_version(self, expect):
        [result, version] = await omni.client.get_hub_version_async()
        carb.log_info(f"Testing hub version {version} against {expect}")
        if result == omni.client.Result.OK and version == expect:
            return True
        return False
            ...
            if contents:
                ...
                if self._all_cache_apis:
                    ...
                    self._update_subscription = get_eventdispatcher().observe_event(
                        observer_name="omni.kit.widget.live update",
