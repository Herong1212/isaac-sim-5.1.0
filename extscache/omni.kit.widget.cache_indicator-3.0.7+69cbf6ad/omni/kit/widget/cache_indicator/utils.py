...
def log_http_error(e, missing):
    msg = f"{missing} was not found. May be a permission issue. Error: ${e}"
...
def get_pck_install_folder(cfg_default_install_path: str, omniverse_config: str) -> str:
    ...
    2. Tries to check if an environment variable with the name of OMNICLIENT_HUB_EXE exists and is not empty
    ...
    # check if the path to run Hub was passed in environment variable
    env_hub_path = os.environ.get("OMNICLIENT_HUB_EXE", "")
    # if OMNICLIENT_HUB_EXE is pointed directoy to a hub exe, it isn't safe to
    # assume that it's installed into the expected directory structure (with a
    # parent directory named like "hub-<version>"), so in this case we have to
    # ignore it and try to determine the install path another way
    if env_hub_path and not os.path.isfile(env_hub_path):
        carb.log_info(f"OmniClient Hub folder found in environment variable (OMNICLIENT_HUB_EXE): {env_hub_path}")
        return env_hub_path
...
def format_auth_token(api_key: str) -> str:
...
async def get_token(api_key: str, org: str, team: str) -> str:
    ...
    headers = {
        ...
        "Authorization": format_auth_token(api_key)
    }

    uri = f"https://authn.nvidia.com/token?service=ngc&scope=group/ngc:{org}&group/ngc:{org}/{team}"
        ...
        async with aiohttp.ClientSession(headers=headers, trust_env=True) as client:
            async with client.get(uri) as response:
                if response.ok:
                    ...
                    msg = f"Failed to obtain an NGC auth token with code: {code}"
                    if code in [401, 403, 404]:
                        carb.log_warn(msg)
                    else:
                        carb.log_error(msg)
    except Exception as e:
            carb.log_error(f"Failed to obtain an NGC auth token with message: {e}")


class ZipFileWithPermissions(ZipFile): # pragma: no cover
