...
def get_sdr_shader_node_for_prim(
    ...
    if implementation_source == UsdShade.Tokens.id:
        ...
        # If implementation source is 'id' try to get from the SDR registry first, this fails we fallback to the UsdMdl registry.
        ...
        if sdr_shader_node:
            ...
            # UsdPreviewSurface SDR nodes will be first loaded via the Sdr registry above, however they will point to the usda based definitions that
            # that are included in UsdShaders.
    ...
    # This is necessary to support older scene files where the subidentifiers may not map to the ones generated by the code in omni.kit.usd.mdl.
    # e.g.
    ...
    # The subIdentifier generated by omni.kit.usd.mdl will be 'construct_float' due to this function not being overloaded.
...
def get_mdl_subidentifiers_for_prim(prim: Usd.Prim) -> Union[List[str] | None]:
    ...
    # if the subidentifier is not in the list, meaning it's not valid, prepend it so that it's clear to the user what has been chosen.
...
def get_sdr_shader_property_default_value(sdr_shader_property: Sdr.ShaderProperty, metadata: dict) -> Union[Any | None]:
    ...
    def get_default_from_type(sdr_shader_property: Sdr.ShaderProperty) -> Union[Any | None]:
        ...
        If ndr_type_indicator[1] contains a value this means we have an unclean mapping from the from SdrProperties type to an Sdf.Type.
        If this is the case we lookup the Tf.Type and if it exists and it has a Python class associated with it, we construct
        the Python class to extract a default value
        """
    ...
    # handle case where an enumerated value is mapped to a Token or String value
    elif (type_name in [Sdf.ValueTypeNames.Token, Sdf.ValueTypeNames.String]) and isinstance(default_value, int):
        allowed_tokens = metadata.get("allowedTokens", [])

        if 0 <= default_value < len(allowed_tokens):
            default_value = allowed_tokens[default_value]
...
def remove_properties_and_connections(prim: Usd.Prim) -> Tuple[bool, List, Dict]:
    ...
    def remove_connections(
        ...
        for child in prims_to_check:
            ...
            #  1. The target attribute no longer exists, which would be the case if its not part of the current Sdr.Shadernode
            ...
            for usdshade_port in usdshade_ports:
                ...
                to_placeholder = None
                for p in GetPlaceholderPropertiesForPrim(child):
                ...
                # handle missing/empty render type metadata on material output ports, e.g. outputs:mdl:surface, outputs:surface
    ...
    usdshade_shader = UsdShade.Shader(prim)
    if usdshade_shader:
        placeholders = GetPlaceholderPropertiesForPrim(prim, {"overlay_property_metadata": False})
