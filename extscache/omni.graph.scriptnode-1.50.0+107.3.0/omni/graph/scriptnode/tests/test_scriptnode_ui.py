...
from omni.graph.scriptnode._impl.extension import (
    SCRIPTNODE_ENABLE_OPT_IN_SETTING,
...
"""
    ...
    async def test_setting_interaction(self):
        ...
        def _get_settings() -> tuple[bool, bool]:
            return (
                bool(settings.get(SCRIPTNODE_ENABLE_OPT_IN_SETTING)),
                bool(settings.get(SCRIPTNODE_OPT_IN_SETTING)),
            )

        def _set_settings(enable_opt_in: bool, opt_in: bool):
            settings.set(SCRIPTNODE_ENABLE_OPT_IN_SETTING, enable_opt_in)
            settings.set(SCRIPTNODE_OPT_IN_SETTING, opt_in)
        ...
        try:
            ...
            settings.destroy_item(SCRIPTNODE_ENABLE_OPT_IN_SETTING)
            self.assertTrue(is_check_enabled())
            settings.set(SCRIPTNODE_ENABLE_OPT_IN_SETTING, False)
            self.assertFalse(is_check_enabled())
            settings.set(SCRIPTNODE_ENABLE_OPT_IN_SETTING, True)
            self.assertTrue(is_check_enabled())

            verify_scriptnode_load([])
            no_button_ref = ui_test.find('ScriptNode Warning//Frame/**/Button[*].text=="No"')
            yes_button_ref = ui_test.find('ScriptNode Warning//Frame/**/Button[*].text=="Yes"')
            if no_button_ref:
            ...
            _ = og.Controller.edit(
                "/TestGraph",
                {
                    og.Controller.Keys.CREATE_NODES: ("Script", "omni.graph.scriptnode.ScriptNode"),
                },
            )
            check_for_scriptnodes()
            if yes_button_ref:
