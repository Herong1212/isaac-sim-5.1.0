...
import omni.usd
from omni.graph.scriptnode._impl.parse_examples import extract_snippets
from omni.graph.scriptnode.ogn.OgnScriptNodeDatabase import OgnScriptNodeDatabase
from pxr import Usd


# ==============================================================================================================
class TestScriptNodeSnippets(ogts.OmniGraphTestCase):
    ...
    async def _test_compute_timer(self, graph: og.Graph, script_node: og.Node):
        ...
        """
        await og.Controller.evaluate(graph)
        state_info = OgnScriptNodeDatabase.get_internal_state(script_node, script_node.get_graph_instance_id())
    ...
    async def test_extracted_snippets(self):
        ...
        for index, (title, content) in enumerate(snippets, 1):
            ...
            if (
                ...
                continue
            script_node = controller.create_node((f"ScriptNode{index}", graph), "omni.graph.scriptnode.ScriptNode")
            ...
            OgnScriptNodeDatabase.NODE_TYPE_CLASS.try_cleanup(script_node)
