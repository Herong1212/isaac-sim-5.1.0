```{csv-table}
**Extension**: {{ extension_version }},**Documentation Generated**: {sub-ref}`today`,{ref}`changelog_omni_graph_scriptnode`
```

(ext_omni_graph_scriptnode)=
...
See the {doc}`complete instructions for the script node<GeneratedNodeDocumentation/OgnScriptNode>`
...
GeneratedNodeDocumentation/OgnScriptNode
```
