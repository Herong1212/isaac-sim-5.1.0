...
class AnimationVisualTestBase(omni.kit.test.AsyncTestCase):
    ...
    '''
    async def do_visual_test(self, threshold=5e-4, inverse=False, img_name=None, img_suffix="", skip_assert=False):
        def compare_ge(a, b):
            return a >= b
        ...
        if not skip_assert:
            self.assertTrue("The image doesn't match the golden one" and res)
        return res

    async def capture_and_compare_test(self, threshold=5e-4, inverse=False, img_name=None, img_suffix="", skip_assert=False):
        def compare_ge(a, b):
            return a >= b
