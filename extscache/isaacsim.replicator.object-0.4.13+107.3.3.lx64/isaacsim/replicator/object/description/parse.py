...
from .symbol import Description, resolve_scene
from ..utility.misc import error, ensure_folder_recursive, ensured_retrieve, P, PROFILE, read_description
...
async def simulate(yaml_path, scene):
    with PROFILE("simulate inner"):  # demo_kaleidoscope
        with P("------ init ------"):
            ...
            description.initialize(9999999)  # for file name compose
        ...
        await rep.orchestrator.run_async()  # after writer is attached
        for index in range(scene.num_frames):
            with P(f"------ frame {index} ------ "):
                description.initialize(index)
                metadata = resolve_scene(description)
                await scene.step(metadata, index, description.seed)
        rep.orchestrator._orchestrator.status = rep.orchestrator.Status.STARTED  # noqa
        await scene.clean_up(metadata)
