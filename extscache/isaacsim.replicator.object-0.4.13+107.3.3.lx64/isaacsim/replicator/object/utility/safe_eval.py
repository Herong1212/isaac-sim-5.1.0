...
def safe_eval(expr: str, _allowed_names: Iterable = []):
    """
    Eval a python expression with restrictions
    """
    allowed_names = dict(_flatten(_allowed_names))

    if isinstance(_allowed_names, list) and "seed" in _allowed_names[0] and "seed" in allowed_names:
        allowed_names["seed"] = _allowed_names[0]["seed"]
    ...
    walk(node)
    return eval(compile(source=node, filename="<string>", mode="eval"), None, allowed_names)
