...
_precisions = {"orient": UsdGeom.XformOp.PrecisionFloat, "scale": UsdGeom.XformOp.PrecisionHalf}
...
def create_xform_op(prim, op_key, suffix, op_func):
    ...
    if not op or not op.HasValue():
        ...
        # override if precision exists; precision can't change after creating the xformOp,
        # default precisions are too volatile across different versions
        if op and op_key in [
            "translate",
            "rotateXYZ",
            "rotateXZY",
...
def log_ops(prim):
    logging.warn(UsdGeom.Xform(prim).GetXformOpOrderAttr().Get())  # noqa: PLW4902 deprecated method warn()


def clear_ops(prim):
    # clearing will not delete the attributes, but you can call AddXOp on a cleared op.
    # we block before clearing, and check whether it HasValue() before creating.
    # also if you play with the UI, the ops will HasValue() again, without being in the order.
    # we have to block all attributes starting with xformOp.
...
def ensure_matrix(m, size=4):
    if not isinstance(m, (list, tuple)) or len(m) != size:
        error(f"expected matrix with row count {size}, got {m}")
    for row in m:
        ensure_vector(row, size)


def append_blocked_xform_op(prim, name):
    op = prim.GetAttribute("xformOp:" + name)
    if not op or not op.HasValue():
        create_xform_op(prim, name, None, eval(f"UsdGeom.Xform(prim).Add{name[0].upper()+name[1:]}Op"))
...
def set_xform_ops(prim, ops):
    ...
    for op in ops:
        ...
        value = op[1]
        if name == "scale":
            scale_values.append((op[1], suffix))
            continue
        ...
        if value:
            if name in ["translate", "rotateXYZ", "rotateXZY", "rotateYXZ", "rotateYZX", "rotateZXY", "rotateZYX"]:
                ...
                ensure_scalar(value)
                operator.Set(value)
    append_blocked_xform_op(prim, "translate")
    append_blocked_xform_op(prim, "orient")
    append_blocked_xform_op(prim, "rotateZYX")
    for scale_value, scale_suffix in scale_values:
        operator = create_xform_op(prim, "scale", scale_suffix, UsdGeom.Xform(prim).AddScaleOp)
        if scale_value:
            ensure_vector(scale_value, 3)
            operator.Set(resolve_gfvec3(scale_value, operator.GetPrecision()))
    append_blocked_xform_op(prim, "scale")


def get_total_xform(prim):
    total_xform = Gf.Matrix4d(1)
    xform_order = UsdGeom.Xform(prim).GetXformOpOrderAttr().Get()
    if xform_order is None:
        return total_xform
    for name in xform_order:
        total_xform = UsdGeom.XformOp(prim.GetAttribute(name)).GetOpTransform(Usd.TimeCode.Default()) * total_xform
    return total_xform


# if op_name is xformOp:translate:local, return translate
def get_xform_op_type(op_name):
    if op_name.count(":") == 1:
        return op_name[op_name.find(":") + 1 :]
    else:
        return op_name[op_name.find(":") + 1 : op_name.rfind(":")]


# mainly for the camera, which will have problems if it's not translate-rotateXYZ/rotateYXZ-scale order,
# compatibility with different Kit versions


def set_xlate_xyz_from_prim_xform(target_prim, source_prim):
    xform = get_total_xform(source_prim)
    xlate, xyz = RotationUtil.get_xlate_and_xyz_from_xform(xform)
...
def set_xlate_yxz_from_prim_xform(target_prim, source_prim):
    xform = get_total_xform(source_prim)
    xlate, yxz = RotationUtil.get_xlate_and_yxz_from_xform(xform)
    get_xform_op(target_prim, "translate").Set(xlate)
...
def print_ops(prim):
    ops = UsdGeom.Xform(prim).GetXformOpOrderAttr().Get()
    print(prim.GetPath(), "xformOps")
    for op in ops:
        print(f"{op} -> {prim.GetAttribute(op).Get()}")
