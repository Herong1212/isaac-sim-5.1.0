...
class ScriptManager:
    ...
    def __init__(self):
        ...
        self._allow_scripts_to_execute = False
    ...
    def _on_script_event(self, e: carb.events.IEvent):
        ...
        if e.type == int(EVENT_TYPE_SCRIPT_FILE_UPDATED):
            ...
            if updated_script_path in self._script_to_prims:
                ...
                carb.log_info(f"Script dependency file updated: {updated_script_path}")
    ...
    async def _pending_sync_task_handler(self):
        ...
        # get the latest scripting api prim paths
        if self._stage:
            self._scripting_api_prim_paths = self._stage.GetPrimsWithAppliedAPIName("OmniScriptingAPI")
            scripting_api_set = set(self._scripting_api_prim_paths)

            # destroy scripts where prims are no longer in stage.
            scripting_apis_removed = []
            for prim_path in self._prim_to_scripts:
                prim = self._stage.GetPrimAtPath(prim_path)
                if prim.GetPath() not in scripting_api_set:
                    scripting_apis_removed.append(prim_path)

            for prim_path in scripting_apis_removed:
    ...
    def _create_script_instance(self, prim_path_str, script_path):
        if not self._allow_scripts_to_execute:
            return None
    ...
    def _apply_scripts(self, prim: Usd.Prim):
        ...
        if not scripts_attr:
            scripts_attr = prim.CreateAttribute(
                SCRIPTS_ATTR,
                usdrt.Sdf.ValueTypeNames.AssetArray,
                False
            )
        script_assets = scripts_attr.Get()
    ...
    async def _security_check_and_wait(self, future, proceed_fn):
        ...
        await future
        if future.result():
            self._allow_scripts_to_execute = True
            proceed_fn()
        else:
            self._allow_scripts_to_execute = False
