...
import traceback
import urllib.parse
from typing import Type, AsyncIterator, Optional, Dict, Tuple, Iterable, Callable, Awaitable, List
...
DEBUG_LOGGING = ws_logger.isEnabledFor(logging.DEBUG)
...
class WebSocketClient(Client):
    ...
    def __init__(self, uri: str, *, marshaller: Marshaller = None, max_size: int = MAX_SIZE, max_qsize: int = 0,
        ...
        :param max_qsize: Maximum queue size of pending messages for a single request.
        If that limit is exceeded, then the client stops receiving any other messages in the socket
        (including close frames) until the queue message is read.
    ...
    async def _listen(self):
        try:
            while True:
                ...
                messages = self._messages.get(request_id)
                if messages:
    ...
    async def call(self, interface: InterfaceName, method: MethodName, request: dict, request_type: Type[RequestType], return_type: Type[ResponseType]) -> ResponseType:
        ...
        pending_result = connection_closed = None
        try:
            pending_result = asyncio.ensure_future(results.get())
            connection_closed = asyncio.ensure_future(self._closed.wait())

            done, pending = await asyncio.wait(
                {pending_result, connection_closed},
                return_when=asyncio.FIRST_COMPLETED
            )
            if connection_closed in done:
                pending_result.cancel()
                raise TransportError("Connection has been closed")
            elif pending_result in done:
                ...
                raise TransportError("Unexpected command results")
        finally:
            if pending_result:
                pending_result.cancel()
        ...
        try:
            while True:
                ...
                if connection_closed in done:
                    ...
                    lock = self._locks.get(request_id, None)
    ...
    async def _send(self, interface: str, method: str, request, return_type: Type):
        ...
        self._messages[request_id] = asyncio.Queue(maxsize=self.max_qsize)
        self._results[request_id] = results = asyncio.Queue()
        self._tasks[request_id] = task = self._loop.create_task(self._receive(request_id, return_type))
    ...
    async def _receive(self, request_id: int, return_type: Type):
        ...
        results = self._results.get(request_id)
        if not results:
            return
        ...
        try:
            while True:
                ...
                if response_type == _Error.type:
                    ...
                    byte_queue = asyncio.Queue(maxsize=self.max_qsize)
            ...
            results.put_nowait(TransportError(str(exc)))
    ...
    def _cleanup(self):
        while self._messages:
            request_id, messages = self._messages.popitem()
            drain(messages)
    ...
    def _cancel_request(self, request_id: int):
            ...
            try:
                while True:
                    ...
                    if packet_type == _Stop.type:
                        ...
                        messages_by_id[request_id] = messages = asyncio.Queue(maxsize=self.max_qsize)
                        if request.blob:
