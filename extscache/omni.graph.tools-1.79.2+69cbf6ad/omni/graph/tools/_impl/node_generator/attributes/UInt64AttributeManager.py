...
class UInt64AttributeManager(NumericAttributeManager):
    """Support class for attributes of type unsigned 64-bit integer"""

    OGN_TYPE = "uint64"

    # Hardcoded type configuration using USD. Dictionary key is the OGN attribute type name (without arrays).
    CPP_CONFIGURATION = {
        "uint64": CppConfiguration("uint64_t", cast_required=False),
    }
    CUDA_CONFIGURATION = {
        "uint64": CudaConfiguration("uint64_t", include_files=["stdint.h"], cast_required=False),
    }
    ...
    def validate_value(self, value):
        ...
        if not is_type_or_list_of_types(value, int, self.tuple_count):
            raise ParseError(f"Value {value} on a uint64[{self.tuple_count}] attribute is not a matching type")
        if not values_in_range(value, 0, 18446744073709551615):
            raise ParseError(f"Value {value} on a uint64[{self.tuple_count}] attribute is out of range")
    ...
    def python_type_name(self):
        ...
        return self.python_add_containers("int", "numpy.uint64")
    ...
    def usd_type_name(self):
        ...
        return self.usd_add_containers("uint64")
