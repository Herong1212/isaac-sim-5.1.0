...
from omni.kit.test_suite.helpers import get_random_material_list

from .test_func_create_prims import *  # noqa: F403, F401
...
class TestMenuCreate(omni.kit.test.AsyncTestCase):
    ...
    async def test_create_menus(self):
        ...
        mdl_list = await get_random_material_list(3)
        mdl_list = [mtl_name for mtl_name, mdl_path, submenu in mdl_list] + [
            "USD_Preview_Surface",
            "USD_Preview_Surface_Texture",
            "Add_MDL_File",
        ]
                    ...
                    if item.name != "" and item.has_action():
                        ...
                        try:
                            ...
                            if test_fn.startswith("create_test_func_create_material_"):
                                mat_name = item.name.replace(" ", "_")
                                if mat_name in mdl_list:
        ...
        for material_name in material_list:
            ...
            try:
                await create_test_func_create_material(self, material_name)  # noqa: F405
            except Exception as exc:  # pylint: disable=broad-except
