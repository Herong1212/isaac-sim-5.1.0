...
# XXX these options coincidentally align with the corresponding enumeration in OgnPointsOnCurve
MODE_OPTIONS = ["Input Array", "Random Array"]
...
class CustomLayout:
    ...
    def apply(self, props):
        ...
        with frame:
            with CustomLayoutGroup("Parameters"):
                ...
                CustomLayoutProperty("inputs:uValues", "U-Values")
            with CustomLayoutGroup("Random Sampling"):
                CustomLayoutProperty("inputs:numberOfPoints", "Number of Points")
                CustomLayoutProperty("inputs:timeBasedSeed", "Vary Randomness over Time")
                CustomLayoutProperty("inputs:randomSeed", "Random Seed")
    ...
    def mode_build_fn(self, *args):
        with ui.HStack(spacing=HORIZONTAL_SPACING):
            ...
            with ui.ZStack():
                attr_path = self.compute_node_widget._payload[-1].AppendProperty("inputs:samplingMode")
