...
from .action_catalog_model import OmniGraphNodeQuickSearchModel
...
class ActionGraphExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id: str):
        ...
        self._hotkeys = OmniGraphHotkeys(ext_id, ActionGraphExtension.WINDOW_NAME)

        add_create_menu_type(
            "Action Graph", "execution", "ActionGraph", "action_graph.svg", ActionGraphExtension.show_graph
        )
        ui.Workspace.set_show_window_fn(ActionGraphExtension.WINDOW_NAME, partial(self._menu_show_window, None))

        editor_menu = omni.kit.ui.get_editor_menu()
        if editor_menu:
            self._menu = editor_menu.add_item(
                ActionGraphExtension.MENU_PATH, self._menu_show_window, toggle=True, value=False
            )
    ...
    def on_shutdown(self):
        ...
        remove_create_menu_type("Action Graph")
        ui.Workspace.set_show_window_fn(ActionGraphExtension.WINDOW_NAME, None)
        self.__extensions_subscription = None
    ...
    def _visibility_changed_fn(self, visible):
        if self._menu:
            omni.kit.ui.get_editor_menu().set_value(ActionGraphExtension.MENU_PATH, visible)
            self.show_window(None, visible)

    def show_window(self, menu, value):
        """Show/hide the window"""
        if value:
            self._window = ActionGraphWindow(
                ActionGraphExtension.WINDOW_NAME,
                width=600,
                height=800,
        ...
        # update menu state
        if self._menu:
            omni.kit.ui.get_editor_menu().set_value(ActionGraphExtension.MENU_PATH, value)
    ...
    def show_graph(prim_list: List[Usd.Prim]):
        ...
        async def import_prims():
            if not self._window:  # noqa: protected-access
                ...
                if self._window.app_window and hasattr(self._window.app_window, "focus"):  # noqa: protected-access
                    # This is necessary for external windows
                    self._window.app_window.focus()  # noqa: protected-access
    ...
    def _on_extensions_changed(self, loaded: bool, ext_id: str):
        ...
        if loaded:
            ...
            with suppress(TypeError):  # FIXME: can remove when quicksearch 2.1 is published
                # Register Omni Graph Action nodes in Quick Search
                self._quicksearch_sub = QuickSearchRegistry().register_quick_search_model(
                    "Omni Graph Action nodes",
            ...
            # Deregister Omni Graph Action nodes in Quick Search
            self._quicksearch_sub = None

    def _is_window_focused(self) -> bool:
        """Returns True if the OmniGraph Action graph window exists and has focus"""
    ...
    def _register_for_stage_open():
        """Register ourselves as an opener for execution graphs"""

        def can_open(prims: List[Usd.Prim]):
            if prims:
                first_prim = prims[0]
                graph = og.get_graph_by_path(first_prim.GetPrimPath().pathString)
                return graph and (og.get_graph_settings(graph).evaluator_type == "execution")
            return False

        return register_stage_graph_opener(can_open, ActionGraphExtension.show_graph)
