...
// Export usd materials surface. If it's unchecked and OMNI_CONVERTER_FLAGS_IGNORE_MATERIALS
// is not set, it will export MDL by default.
#define OMNI_CONVERTER_FLAGS_EXPORT_PREVIEW_SURFACE (0x1 << 5)
...
// Embed MDL inside USD. If it's not set, it will generate MDL on disks.
// This flag only works if OMNI_CONVERTER_FLAGS_EXPORT_PREVIEW_SURFACE is not set.
// WARNING: This flag is deprecated at this moment!!
#define OMNI_CONVERTER_FLAGS_EMBED_MDL (0x1 << 8)
...
// Import USD materials as glTF materials using the NV_materials_mdl extension
#define OMNI_CONVERTER_FLAGS_IMPORT_GLTF_NV_MATERIALS_MDL (0x1 << 26)
...
/**
 * Internal use only.
 * Callbacks for rtx.neuraylib.NeurayLib functions needed for importing/exporting the
 * NV_materials_mdl glTF vendor extension.
 */
typedef void* (*OmniConverterGetNeurayApi)();
typedef std::string (*OmniConverterCreateMdlModule)(const std::string& usdIdentifier, const std::string& dbScopeName);
typedef void (*OmniConverterDestroyMdlModule)(const std::string& dbName);
typedef std::string (*OmniConverterCreateTemporaryDbScope)();
typedef void (*OmniConverterDestroyTemporaryDbScope)(const std::string& dbScopeName);
typedef void* (*OmniConverterCreateReadingTransaction)(const std::string& dbScopeName);
typedef std::string (*OmniConverterUnmangleUri)(const std::string& mangledUri, const std::string& pathSeparator);
typedef std::vector<std::string> (*OmniConverterResolveTiledResourceUri)(const std::string& filenameMask);

/**
 * Sets the cache store for caching. For glTF/OBJ/FBX to in-memory USD conversion, the USD results will be cached. By
 * default, if the cache location is not provided, caching will be disabled. Btw, cacheFolder should only be system
 * path.
...
/**
 * Internal use only.
 * Sets the callbacks for rtx.neuraylib.NeurayLib functions needed for importing/exporting the
 * NV_materials_mdl glTF vendor extension.
 * @param getNeurayApiCallback Returns a pointer to an instance of mi::neuraylib::INeuray.
 *                             The caller is reresponsible for releasing the the neuray instance.
 * @param createMdlModuleCallback Returns
 */
OMNI_ASSET_CONVERTER_EXPORT void omniConverterSetNeurayLibCallbacks(
    OmniConverterGetNeurayApi getNeurayApiCallback,
    OmniConverterCreateMdlModule createMdlModuleCallback,
    OmniConverterDestroyMdlModule destroyMdlModuleCallback,
    OmniConverterCreateTemporaryDbScope createTemporaryDbScopeCallback,
    OmniConverterDestroyTemporaryDbScope destroyTemporaryDbScopeCallback,
    OmniConverterCreateReadingTransaction createReadingTransactionCallback,
    OmniConverterUnmangleUri unmangleUriCallback,
    OmniConverterResolveTiledResourceUri resolveTiledResourceUriCallback
);
