...
"""
This module contains bindings and helpers to omniverse_asset_converter interface. You can use it as follows:
...
if sys.platform == "win32":
    ctypes.WinDLL(os.path.join(os.path.dirname(__file__), "libs/assimp-vc142-mt.dll"))
...
class OmniAssetConverter:
    ...
    def __init__(
        ...
        import_gltf_mdl_extension=False,
        ...
        Args:
            ...
            import_gltf_mdl_extension (bool): Import USD materials as glTF materials using the NV_materials_mdl extension.
        ...
        self._import_gltf_mdl_extension = import_gltf_mdl_extension
    ...
    @classmethod
    def set_neuraylib_callback(
        cls,
        get_neuray_api_callback,
        create_mdl_module_callback,
        destroy_mdl_module_callback,
        create_temporary_db_scope_callback,
        destroy_temporary_db_scope_callback,
        create_reading_transaction_callback,
        unmangle_uri_callback,
        resolve_tiled_resource_uri_callback,
    ):
        """Sets callbacks for internal NeurayLib functions used when
        handling the NV_materials_mdl glTF vendor extension.

        Args:
            get_neuray_api_callback (Callable -> int): Function to query a pointer to an instance of INeuray.
            create_mdl_module_callback (Callable[str, str] -> str): Function to create an MDL module.
            destroy_mdl_module_callback (Callable[str]): Function to destroy an MDL module.
            create_temporary_db_scope_callback (Callback -> str): Function to create a temporary Neuray database scope.
            destroy_temporary_db_scope_callback (Callback[str]): Function to destroy a temporary Neuray database scope.
            create_reading_transaction_callback (Callable -> int): Function to create an MDL database reading transaction.
            unmangle_uri_callback (Callable[str, str] -> str): Function to unmangle MDL URIs to USD identifiers.
            resolve_tiled_resource_uri_callback (Callable[str] -> list[str]): Function to resolve a tiled MDL resource URI.
        """

        omniConverterSetNeurayLibCallbacks(
            get_neuray_api_callback,
            create_mdl_module_callback,
            destroy_mdl_module_callback,
            create_temporary_db_scope_callback,
    ...
    async def __aenter__(self):
        ...
        if self._import_gltf_mdl_extension:
            flags |= OMNI_CONVERTER_FLAGS_IMPORT_GLTF_NV_MATERIALS_MDL
