"""
Copied from isaacsim.replicator.agent.core.data_generation.object_info_manager
"""
...
class ObjectDetectionInfo:
    ...
    def get_characters_root_in_stage(self, count=-1, count_invisible=False):
        stage = omni.usd.get_context().get_stage()
        character_list = []
        character_root_path = ReplicatorCaptionSettings.characters_parent_path()
        ...
        for c in children:
            ...
            if count_invisible == True or UsdGeom.Imageable(c).ComputeVisibility() != UsdGeom.Tokens.invisible:
                prim_path = str(c.GetPrimPath())
                character_list.append(prim_path)
        return character_list

    def is_agent_prim_path(self, target_prim_path) -> bool:
        """check whether certain prim path point to a agent"""
        return self.is_character_prim_path(target_prim_path)

    def is_character_prim_path(self, target_prim_path) -> bool:
        """check whether certain prim path point to a character"""
        characters_by_primpath = self.get_characters_root_in_stage()
        return target_prim_path in characters_by_primpath
    ...
    #         ReplicatorCaptionSettings.AnnotatorPrefix.ObjectDetection.AGENT_SPECIFIC
    #     ]
    ...
    #             character_data = self.agent_manager.get_character_data_by_skelpath(str(skeleton_path))
    ...
    def refresh_object_info(self, annotator_data_dict: dict[str, Any]):
        ...
        annotator_data_key = annotator_data_dict.keys()
        object_annotator_list = self.annotator_classification["object_info"]
        ...
        for annotator_name, annotator in generic_annotators_dict.items():
            ...
            for idx, prim_path in enumerate(object_prim_paths):
                ...
                if self.is_agent_prim_path(prim_path):
