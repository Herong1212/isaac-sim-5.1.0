...
class StageModel(ui.AbstractItemModel):
    ...
    def __init__(self, stage: Usd.Stage, flat=False, load_payloads=False, check_missing_references=False, **kwargs):
        ...
        if self.__stage:
            ...
            if self.__usd_context and detect_outdate_state_changes:
                ...
                # TODO: Narrow down the list of events. Previously this had a generic handler that did work for each
                # event.
                self.__stage_event_subscription = []
                for i in range(int(omni.usd.StageEventType.COUNT)):
                    func = self.__on_generic_stage_event
                    if i == int(omni.usd.StageEventType.SELECTION_CHANGED):
                        func = self.__on_stage_selection_changed_event
                    self.__stage_event_subscription.append(
                        carb.eventdispatcher.get_eventdispatcher().observe_event(
                            observer_name="omni.kit.widget.stage:stage_model",
                            event_name=self.__usd_context.stage_event_name(omni.usd.StageEventType(i)),
                            on_event=func
                        )
                    )
        ...
        self.__show_inactive_prims = kwargs.get("show_inactive_prims", True)

        self.__show_undefined_prims = kwargs.get("show_undefined_prims", False)

        self.__show_abstract_prims = kwargs.get("show_abstract_prims", True)
    ...
    def __on_generic_stage_event(self, _):
        if self.__set_usd_selection:
            self.__set_usd_selection = False
    
    def __on_stage_selection_changed_event(self, _):
        if self.__set_usd_selection:
            self.__set_usd_selection = False
            return
    ...
    def _should_prim_be_excluded_from_tree_view(self, prim):
        if (
            ...
            (not self.__show_inactive_prims and not prim.IsActive()) or
            (not self.__show_abstract_prims and prim.IsAbstract())
        ):
            return True
    ...
    def _refresh_prim_subtree(self, prim_path):
        ...
        for stage_item in old_stage_items:
            ...
            if prim:
                if prim.IsActive() or self.__show_inactive_prims or (prim.IsAbstract() and self.__show_abstract_prims):
    ...
    def _filter_prim(self, prim: Usd.Prim):
        ...
        if self.__filter_name_text and self.__filter_name_text != "/" and self.__filter_name_text[-1] == '/':
            has_slash = True
        else:
            has_slash = False

        if has_slash:
            text_without_slash = self.__filter_name_text[:-1]
        else:
            text_without_slash = self.__filter_name_text

        # Don't search it as prim path if it shows displayName.
        if (
            text_without_slash
            and Sdf.Path.IsValidPathString(text_without_slash)
            and self._get_path_converter().IsAbsolutePath(self._get_path_converter()(text_without_slash))
        ):
            filter_path = self._get_path_converter()(text_without_slash)
            if not prim.GetPath().HasPrefix(filter_path):
                return False

            if has_slash and prim.GetPath() == text_without_slash:
                # Returns children only if it's in style "/World/cube/"
                return False
            elif not self.flat and not has_slash and prim.GetPath() != text_without_slash:
                # If it has no slash, it means to return the exact prim only in non-flat mode.
                return False
        else:
    ...
    def show_inactive_prims(self) -> bool:
        ...
        Returns:
            bool
        """
        return self.__show_inactive_prims
    ...
    def show_inactive_prims(self, value):
        ...
        Args:
            show (bool): Value to set.
        """
        if value != self.__show_inactive_prims:
            self.__show_inactive_prims = value
    ...
    def show_abstract_prims(self) -> bool:
        ...
        Returns:
            bool
        """
        return self.__show_abstract_prims
    ...
    def show_abstract_prims(self, value):
        ...
        Args:
            value (bool): Value to set.
        """
        if value != self.__show_abstract_prims:
            self.__show_abstract_prims = value
