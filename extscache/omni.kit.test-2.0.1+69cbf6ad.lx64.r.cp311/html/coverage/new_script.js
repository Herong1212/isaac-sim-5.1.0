...
coverage.wire_up_filter = function () {
    ...
    document.getElementById("filter").addEventListener("input", debounce(event => {
        ...
        var totalCoveragePercentage = null
        ...
        for (let column = 1; column < totals.length; column++) {
            ...
            if (column === totals.length - 1) {
                ...
                cell.textContent = denom
                    ...
                    : `${(100).toFixed(places)}%`;
                totalCoveragePercentage = cell.textContent;
            } else {
                cell.textContent = totals[column];
            }
        }
        if (totalCoveragePercentage) {
            // update the coverage report % at the top
            document.getElementsByClassName('pc_cov')[0].textContent = totalCoveragePercentage;
        }
    }));
