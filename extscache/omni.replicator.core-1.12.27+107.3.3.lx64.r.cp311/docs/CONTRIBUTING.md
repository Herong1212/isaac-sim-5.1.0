...
* Bug reports: Please report any bugs you find in the [issue tracker](https://github.com/omni.replicator.core/issues).
* Feature requests: Please suggest new features you would like to see in the [discussions](https://github.com/omni.replicator.core/discussions).
* Code contributions: Please submit a [pull request](https://github.com/omni.replicator.core/pulls).
  * Bug fixes
  * New features
  * Documentation improvements
  * Tutorials and tutorial improvements

We prefer GitHub [discussions](https://github.com/omni.replicator.core/discussions) for discussing ideas,
asking questions, conversations and requests for new features.

Please use the [issue tracker](https://github.com/omni.replicator.core/issues) only to track executable pieces of work
with a definite scope and a clear deliverable. These can be fixing bugs, new features, or general updates.

## Contributing Code

> **Note**: Please refer to the [Google Style Guide](https://google.github.io/styleguide/pyguide.html)
...
We use [GitHub](https://github.com/omni.replicator.core) for code hosting. Please
follow the following steps to contribute code:

1. Create an issue in the [issue tracker](https://github.com/omni.replicator.core/issues) to discuss
...
6. Submit a pull request to the [main branch](https://github.com/omni.replicator.core/compare).
...
> **Tip**: It is important to keep the pull request as small as possible. This makes it easier for the
...
for the documentation are located in the `source/extensions/omni.replicator.core/docs` directory. The documentation is
written in [reStructuredText](https://docutils.sourceforge.io/rst.html) format.
...
Please follow the steps mentioned in the Contributing Code section.

> **Caution**: To build the documentation, we recommend creating a [virtual environment](https://docs.python.org/3/library/venv.html)
> to install the dependencies. This can also be a [conda environment](https://docs.conda.io/projects/conda/en/latest/user-guide/tasks/manage-environments.html).
...
The documentation is generated in the `_build/docs` directory. To view the documentation, open
the `index.html` file in the `html` directory. This can be done by running the following command
in the terminal:

```bash
xdg-open _build/docs/current/index.html
```

> **Hint**: The `xdg-open` command is used to open the `index.html` file in the default browser. If you are
...
```bash
rm -rf _build/docs && ./repo.sh docs
```

## Contributing assets

Currently, we host the assets for the extensions on [NVIDIA Nucleus Server](https://docs.omniverse.nvidia.com/nucleus/latest/index.html).
Nucleus is a cloud-based storage service that allows users to store and share large files. It is
integrated with the [NVIDIA Omniverse Platform](https://developer.nvidia.com/omniverse).
...
> **Note**: We are currently working on a better way to contribute assets. We will update this section once we
...
Each extension maintains a changelog in the `CHANGELOG.md` file in the `docs` directory,
...
The `CHANGELOG.md` is a file that contains the curated, chronologically ordered list of notable changes
for each version of the extension.

> **Note**: The version number on the `extension.toml` file should be updated according to
> [Semantic Versioning](https://semver.org/) and should match the version number in the
> `CHANGELOG.md` file.
...
* The version number is updated according to [Semantic Versioning](https://semver.org/). The
...
> **Tip**: When in doubt, please check the style in the existing changelog files and follow the same style.
...
* Changed an existing feature. Earlier, we were using `torch.bmm` to perform the matrix multiplication.
  However, this was slow for large matrices. We have now switched to using `torch.einsum` which is
...
* Fixed crashing of the `my_function` when the input was too large.
  We now use `torch.einsum` that is able to handle larger inputs.
```

## Coding Style

We follow the [Google Style Guides](https://google.github.io/styleguide/pyguide.html) for the
codebase. For Python code, the PEP guidelines are followed. Most
important ones are [PEP-8](https://www.python.org/dev/peps/pep-0008/)
for code comments and layout,
[PEP-484](http://www.python.org/dev/peps/pep-0484) and
[PEP-585](https://www.python.org/dev/peps/pep-0585/) for
type-hinting.

For documentation, we adopt the [Google Style Guide](https://sphinxcontrib-napoleon.readthedocs.io/en/latest/example_google.html)
for docstrings. We use [Sphinx](https://www.sphinx-doc.org/en/master/) for generating the documentation.
...
[StackOverflow post](https://stackoverflow.com/questions/744373/circular-or-cyclic-imports-in-python).
...
To address circular imports, we leverage the [typing.TYPE_CHECKING](https://docs.python.org/3/library/typing.html#typing.TYPE_CHECKING) variable. This special variable is
...
To make the code more readable, we use [type hints](https://docs.python.org/3/library/typing.html) for
all the functions and classes. This helps in understanding the code and makes it easier to maintain. Following
this practice also helps in catching bugs early with static type checkers like [mypy](https://mypy.readthedocs.io/en/stable/).
...
We do not specify the return type of `None` in the docstrings. This is because
...
[Dart's effective documentation](https://dart.dev/effective-dart/documentation)
and [technical writing](https://en.wikiversity.org/wiki/Technical_writing/Style).
...
We use [unittest](https://docs.python.org/3/library/unittest.html) for unit testing.
...
* [flake8](https://flake8.pycqa.org/en/latest/): A wrapper around PyFlakes, pycodestyle and
  McCabe complexity checker.

Please check [here](https://pre-commit.com/#install) for instructions
