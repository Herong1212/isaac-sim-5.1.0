r"""Support for simplified access to data on nodes of type omni.replicator.core.OgnAugment
...
class OgnAugmentDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.replicator.core.OgnAugment
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:data', 'uchar[]', 0, None, 'Input data', {ogn.MetadataKeys.MEMORY_TYPE: 'cuda', ogn.MetadataKeys.DEFAULT: '[]', ogn.MetadataKeys.CUDA_POINTERS: 'cpu'}, True, [], False, ''),
        ('inputs:dataOutShape', 'int[]', 0, None, 'Desired dimensions of output array', {ogn.MetadataKeys.CUDA_POINTERS: 'cpu'}, True, [], False, ''),
        ...
        ('inputs:dataType', 'token', 0, None, 'Defines the data type', {ogn.MetadataKeys.DEFAULT: '"uint8"', ogn.MetadataKeys.CUDA_POINTERS: 'cpu'}, True, "uint8", False, ''),
        ...
        ('outputs:dataType', 'token', 0, None, 'Defines the data type', {ogn.MetadataKeys.CUDA_POINTERS: 'cpu'}, True, None, False, ''),
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            ...
            self._batchedReadValues = ["", "", 0, -1, 0, "uint8", None, 0, 0, [0, 0], 0]
        ...
        @property
        def dataOutShape(self):
            data_view = og.AttributeValueHelper(self._attributes.dataOutShape)
            return data_view.get()

        @dataOutShape.setter
        def dataOutShape(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.dataOutShape)
            data_view = og.AttributeValueHelper(self._attributes.dataOutShape)
            data_view.set(value)
            self.dataOutShape_size = data_view.get_array_size()
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnAugmentDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnAugmentDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnAugmentDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
    ...
    class abi:
        ...
        @staticmethod
        def get_node_type():
            get_node_type_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'get_node_type', None)
            if callable(get_node_type_function):  # pragma: no cover
                return get_node_type_function()
            return 'omni.replicator.core.OgnAugment'
        ...
        def compute(context, node):
            def database_valid():
                return True
            try:
                per_node_data = OgnAugmentDatabase.PER_NODE_DATA[node.node_id()]
                db = per_node_data.get('_db')
                if db is None:
                    db = OgnAugmentDatabase(node)
            ...
            try:
                compute_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'compute', None)
                ...
                with og.in_compute():
                    return OgnAugmentDatabase.NODE_TYPE_CLASS.compute(db)
        ...
        def initialize(context, node):
            OgnAugmentDatabase._initialize_per_node_data(node)
            initialize_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'initialize', None)
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnAugmentDatabase.abi.initialize(context, n)

        @staticmethod
        def release(node):
            release_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'release', None)
            if callable(release_function):  # pragma: no cover
                release_function(node)
            OgnAugmentDatabase._release_per_node_data(node)

        @staticmethod
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnAugmentDatabase._release_per_node_instance_data(node, graph_instance_id)

        @staticmethod
        def update_node_version(context, node, old_version, new_version):
            update_node_version_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'update_node_version', None)
        ...
        def initialize_type(node_type):
            initialize_type_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'initialize_type', None)
            ...
            if needs_initializing:
                ...
                OgnAugmentDatabase.INTERFACE.add_to_node_type(node_type)
                node_type.set_has_state(True)

        @staticmethod
        def on_connection_type_resolve(node):
            on_connection_type_resolve_function = getattr(OgnAugmentDatabase.NODE_TYPE_CLASS, 'on_connection_type_resolve', None)
    ...
    def register(node_type_class):
        OgnAugmentDatabase.NODE_TYPE_CLASS = node_type_class
        og.register_node_type(OgnAugmentDatabase.abi, 1)

    @staticmethod
    def deregister():
        og.deregister_node_type("omni.replicator.core.OgnAugment")
