r"""Support for simplified access to data on nodes of type omni.replicator.core.InstanceIdSegmentationLegacy
...
class OgnInstanceIdSegmentationLegacyDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.replicator.core.InstanceIdSegmentationLegacy
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:ids', 'uint[]', 0, None, 'Unoccluded instance ids (or color, if `colorize` is set to True).', {}, True, [], False, ''),
        ...
        ('outputs:data', 'uchar[]', 0, None, 'Instance id segmentation data', {ogn.MetadataKeys.MEMORY_TYPE: 'cpu', ogn.MetadataKeys.DEFAULT: '[]'}, True, [], False, ''),
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"bufferSize", "colorize", "cudaDeviceIndex", "dataPtr", "dataType", "exec", "height", "strides", "width", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.bufferSize, self._attributes.colorize, self._attributes.cudaDeviceIndex, self._attributes.dataPtr, self._attributes.dataType, self._attributes.exec, self._attributes.height, self._attributes.strides, self._attributes.width]
    ...
    class ValuesForOutputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"bufferSize", "dataType", "exec", "height", "idToLabels", "width", "_batchedWriteValues"}
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnInstanceIdSegmentationLegacyDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnInstanceIdSegmentationLegacyDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnInstanceIdSegmentationLegacyDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
    ...
    class abi:
        ...
        @staticmethod
        def get_node_type():
            get_node_type_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'get_node_type', None)
            if callable(get_node_type_function):  # pragma: no cover
                return get_node_type_function()
            return 'omni.replicator.core.InstanceIdSegmentationLegacy'
        ...
        def compute(context, node):
            def database_valid():
                return True
            try:
                per_node_data = OgnInstanceIdSegmentationLegacyDatabase.PER_NODE_DATA[node.node_id()]
                db = per_node_data.get('_db')
                if db is None:
                    db = OgnInstanceIdSegmentationLegacyDatabase(node)
            ...
            try:
                compute_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'compute', None)
                ...
                with og.in_compute():
                    return OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS.compute(db)
        ...
        def initialize(context, node):
            OgnInstanceIdSegmentationLegacyDatabase._initialize_per_node_data(node)
            initialize_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'initialize', None)
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnInstanceIdSegmentationLegacyDatabase.abi.initialize(context, n)

        @staticmethod
        def release(node):
            release_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'release', None)
            if callable(release_function):  # pragma: no cover
                release_function(node)
            OgnInstanceIdSegmentationLegacyDatabase._release_per_node_data(node)

        @staticmethod
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnInstanceIdSegmentationLegacyDatabase._release_per_node_instance_data(node, graph_instance_id)

        @staticmethod
        def update_node_version(context, node, old_version, new_version):
            update_node_version_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'update_node_version', None)
        ...
        def initialize_type(node_type):
            initialize_type_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'initialize_type', None)
            ...
            if needs_initializing:
                ...
                node_type.set_metadata(ogn.MetadataKeys.CATEGORY_DESCRIPTIONS, "Replicator,Instance id segmentation Legacy")
                node_type.set_metadata(ogn.MetadataKeys.DESCRIPTION, "This node outputs the legacy instance id segmentation data output")
                node_type.set_metadata(ogn.MetadataKeys.LANGUAGE, "Python")
                OgnInstanceIdSegmentationLegacyDatabase.INTERFACE.add_to_node_type(node_type)
                node_type.set_has_state(True)

        @staticmethod
        def on_connection_type_resolve(node):
            on_connection_type_resolve_function = getattr(OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS, 'on_connection_type_resolve', None)
    ...
    def register(node_type_class):
        OgnInstanceIdSegmentationLegacyDatabase.NODE_TYPE_CLASS = node_type_class
        og.register_node_type(OgnInstanceIdSegmentationLegacyDatabase.abi, 1)

    @staticmethod
    def deregister():
        og.deregister_node_type("omni.replicator.core.InstanceIdSegmentationLegacy")
