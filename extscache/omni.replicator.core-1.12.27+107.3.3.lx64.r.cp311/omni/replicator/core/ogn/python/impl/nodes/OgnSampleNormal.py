...
class OgnSampleNormalInternalState:
    def __init__(self):
        self.rng = rep.rng.ReplicatorRNG()


class OgnSampleNormal:
    @staticmethod
    def internal_state():
        return OgnSampleNormalInternalState()
    ...
    def compute(db) -> bool:
        ...
        try:
            if len(mean) != len(std):
                raise ValueError(
                    "For normal distribution, the lengths of inputs:mean and inputs:std must be equal, "
                    + f"but instead received lengths {len(mean)} and {len(std)}"
                )
            if np.any(std < 0):
                raise ValueError(
                    "For normal distribution, all values in inputs:std are expected to be non-negative "
                    + f"but instead received {std}"
                )
            ...
            sample_size = len(mean)

        except Exception as error:
            db.log_error(f"SampleNormal Error: {error}")
            return False
        ...
        samples = state.rng.generator.normal(mean, std, size=(num_samples, sample_size))
    ...
    def initialize(graph_context, node):
        connected_function_callback = OgnSampleNormal.on_connected_callback
        node.register_on_connected_callback(connected_function_callback)

        function_callback = OgnSampleNormal.on_value_changed_callback
    ...
    def on_connected_callback(upstream_attr, downstream_attr):
        ...
        if upstream_attr.get_resolved_type().base_type == og.BaseDataType.UNKNOWN:
            ...
            if downstream_resolved_type.base_type != og.BaseDataType.UNKNOWN:
                ...
                specified_type = node.get_attribute("inputs:mean").get_resolved_type()
