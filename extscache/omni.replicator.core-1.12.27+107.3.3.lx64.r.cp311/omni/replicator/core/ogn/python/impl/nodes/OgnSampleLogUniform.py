...
class OgnSampleLogUniformInternalState:
    def __init__(self):
        self.rng = rep.rng.ReplicatorRNG()


class OgnSampleLogUniform:
    @staticmethod
    def internal_state():
        return OgnSampleLogUniformInternalState()
    ...
    def compute(db) -> bool:
        ...
        try:
            if len(lower) != len(upper):
                raise ValueError(
                    "For log uniform distribution, the lengths of inputs:lower and inputs:upper must be equal, "
                    + f" but instead received lengths {len(lower)} and {len(upper)}, {lower, upper}"
                )
            if np.any(lower > upper):
                raise ValueError(
                    "For log uniform distribution, all values in inputs:lower are expected to be "
            ...
            if np.any(lower <= 0):
                raise ValueError(
                    "For log uniform distribution, all values in inputs:lower are expected to be "
                    + f"greater than 0, but instead received {lower}"
                )
            if num_samples < 1:
            ...
            db.log_error(f"SampleLogUniform Error: {error}")
            return False
        ...
        samples = state.rng.generator.uniform(np.log(lower), np.log(upper), size=(num_samples, sample_size))
    ...
    def initialize(graph_context, node):
        connected_function_callback = OgnSampleLogUniform.on_connected_callback
        node.register_on_connected_callback(connected_function_callback)

        function_callback = OgnSampleLogUniform.on_value_changed_callback
