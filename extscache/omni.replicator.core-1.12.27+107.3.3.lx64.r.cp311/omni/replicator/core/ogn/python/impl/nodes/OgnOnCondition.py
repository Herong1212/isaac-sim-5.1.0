...
from omni.replicator.core.ogn.OgnOnConditionDatabase import OgnOnConditionDatabase
...
class OgnOnConditionInternalState:
...
class OgnOnCondition:
    @staticmethod
    def internal_state():
        """Returns an object that will contain per-node state information"""
        return OgnOnConditionInternalState()

    @staticmethod
    def release(node):
        state = OgnOnConditionDatabase.shared_internal_state(node)
    ...
    def compute(db) -> bool:
        ...
        if state.impulse and db.inputs.condition:
        ...
        is_started = rep.orchestrator.get_status() in [
            ...
            orchestrator.Status.STEPPING,
        ]
        if not is_started or state._capture_time is None:
            return True
        ...
        if is_within_num_frames and db.inputs.condition:
            ...
            rep.orchestrator.set_minimum_next_rt_subframes(db.inputs.rt_subframes)
            if carb.settings.get_settings().get("/omni/replicator/debug"):
                carb.log_info(
                    f"    on-condition {state._capture_time} ({state._num_executions} out of {db.inputs.maxExecs})"
                )
