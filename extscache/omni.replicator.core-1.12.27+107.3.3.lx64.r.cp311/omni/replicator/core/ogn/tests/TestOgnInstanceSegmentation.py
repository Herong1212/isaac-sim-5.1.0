...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        test_file_name = "OgnInstanceSegmentationTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):  # pragma: no cover
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_omni_replicator_core_InstanceSegmentation")
        ...
        self.assertTrue(test_node.get_attribute_exists("inputs:filteredSemanticLabelTokenPtr"))
        attribute = test_node.get_attribute("inputs:filteredSemanticLabelTokenPtr")
        self.assertTrue(attribute.is_valid())
        expected_value = 0
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))
        ...
        self.assertTrue(test_node.get_attribute_exists("inputs:uniqueInstanceSegmentationIdsSDhostPtr"))
        attribute = test_node.get_attribute("inputs:uniqueInstanceSegmentationIdsSDhostPtr")
        self.assertTrue(attribute.is_valid())
        expected_value = 0
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))
        ...
        self.assertTrue(test_node.get_attribute_exists("outputs:semantics"))
        attribute = test_node.get_attribute("outputs:semantics")
        self.assertTrue(attribute.is_valid())
