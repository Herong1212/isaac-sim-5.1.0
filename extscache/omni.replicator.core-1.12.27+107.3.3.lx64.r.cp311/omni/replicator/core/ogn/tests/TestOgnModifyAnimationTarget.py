...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        from omni.replicator.core.ogn.OgnModifyAnimationTargetDatabase import OgnModifyAnimationTargetDatabase
        test_file_name = "OgnModifyAnimationTargetTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):  # pragma: no cover
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_omni_replicator_core_OgnModifyAnimationTarget")
        database = OgnModifyAnimationTargetDatabase(test_node)
        ...
        self.assertTrue(test_node.get_attribute_exists("inputs:reset_timeline"))
        attribute = test_node.get_attribute("inputs:reset_timeline")
        self.assertTrue(attribute.is_valid())
        db_value = database.inputs.reset_timeline
        database.inputs.reset_timeline = db_value
