...
try:
    ...
    # This will happen during the ui_nodes test suite
    pass
import omni.kit.app
...
class TestOmniGraphWidget(OmniUiTest):
    ...
    async def test_target_attribute(self):
        ...
        await wait_for_window("Select Targets")
    ...
    async def test_target_attribute_browse(self):
        ...
        await self.docked_test_window(
            ...
            restore_window=ui.Workspace.get_window("Layer") or ui.Workspace.get_window("Stage"),
            restore_position=ui.DockPosition.BOTTOM,
            block_devices=False,  # Required for ui_test to work properly
        )
        await ui_test.human_delay(5)
        try:
            ...
            path_ui = ui_test.find(
                f"Property//Frame/**/StringField[*].identifier=='sdf_relationship_{attr_name}[{model_index}]'"
            )
            # It seems that WidgetRef.input is flaky, so you end up with garbled text in the input
            # so we manually clear it first
            path_ui.model.begin_edit()
            path_ui.model.set_value(test_path)
            await ui_test.human_delay(1)
            path_ui.model.end_edit()
            await ui_test.human_delay(5)
    ...
    async def test_prim_node_template(self):
        ...
        rel = stage.GetPropertyAtPath(f"{graph_path}/ConstTarget.inputs:value")
    ...
    async def test_extended_attributes(self):
        ...
        # The UI should refresh every frame
        ogab.AUTO_REFRESH_PERIOD = 0

        target_path = Sdf.Path(f"{write_node.get_prim_path()}.inputs:value")
        ...
        async def test_numeric(name, value1, value2):
            ...
            for base in ogab.OmniGraphBase._instances:  # noqa: PLW0212
                if base._object_paths[0] == target_path and (  # noqa: PLW0212
        ...
        async def test_item(name, value1, value2):
            ...
            await ui_test.human_delay(2)
            for base in ogab.OmniGraphBase._instances:  # noqa: PLW0212
                if base._object_paths[0] == target_path:  # noqa: PLW0212
                    base.begin_edit()
                    base.set_value(value1)
                    await ui_test.human_delay(1)
                    base.set_value(value2)
        ...
        for attrib_name, test_values in zip(attribute_names, expected_values):
            ...
            elif attrib_name in ("a_string", "a_token"):
    ...
    async def test_nodes_templates(self):
        """
        Exercise the OG properties widget with templates from all loaded types
        """
        usd_context = omni.usd.get_context()
        ...
        # Instantate all the loaded types that have custom templates
        all_types = []
        all_names = []
        compute_node_widget = ComputeNodeWidget.get_instance()
        for type_name in og.get_registered_nodes():
            if type_name.startswith("omni.graph"):
                template_name = f"template_{type_name}.py"
                path = compute_node_widget.get_template_path(template_name)
                if path:
                    all_types.append(type_name)
                    all_names.append(type_name.split(".")[-1])

        # set up some attributes to set up for consistency
        fix_ups = [
            ("RandomBoolean.inputs:seed", 0),
            ("RandomGaussian.inputs:seed", 0),
            ("RandomNumeric.inputs:seed", 0),
            ("RandomUnitQuaternion.inputs:seed", 0),
            ("RandomUnitVector.inputs:seed", 0),
        ]

        (graph, nodes, _, _) = controller.edit(
            self.TEST_GRAPH_PATH,
            {keys.CREATE_NODES: list(zip(all_names, all_types)), keys.SET_VALUES: fix_ups},
        )
        await controller.evaluate(graph)
        ...
        try:
            # select each in turn and verify golden image
            for n, name in zip(nodes, all_names):
                with self.subTest(name):
                    usd_context.get_selection().set_selected_prim_paths([n.get_prim_path()], True)
                    await ui_test.human_delay(5)
                    # if there is a scroll bar because the template is too large, set the scroll to a position
                    # passed the header to expose the majority of the template. This has no effect on small
                    # templates
                    self._w.properties_frame.scroll_y = 120  # noqa: PLW0212
                    await ui_test.human_delay(5)
                    await self.capture_and_compare(
                        golden_img_dir=self._golden_img_dir, golden_img_name=f"test_nodes_templates_{name}.png"
                    )

                    # Make sure that each property widget is being cleaned up correctly. There should be at most
                    # only one visible 'Select Targets' window. If this assertion fails it won't necessarily be
                    # due to this node, it might have been the previous one.
                    nw = len([w for w in ui.Workspace.get_windows() if w.visible and w.title == "Select Targets"])
                    self.assertLess(nw, 2, f"Too many visible Select Targets windows for node {n}.")
        finally:
            await self.finalize_test_no_image()

    async def test_omnigraph_attribute_models(self):
        """
        The related code is in omnigraph_attribute_models.py
        """
        usd_context = omni.usd.get_context()
        stage = omni.usd.get_context().get_stage()
        ...
        async def input_multi(pos: ui_test.Vec2, value: str):
            # ui_test.input() doesn't support MultiFloatDragField, so WAR ensues
            await field.double_click(pos=pos)
            await ui_test.emulate_char_press(value)
            await ui_test.human_delay(2)
            await ui_test.emulate_keyboard_press(ui_test.input.KeyboardInput.ENTER)
            await ui_test.human_delay(5)
        ...
        (graph, (compare, color, int2, quatf, matrix4d, token), _, _) = controller.edit(
            self.TEST_GRAPH_PATH,
            {
                keys.CREATE_NODES: [
                    ("Compare", "omni.graph.nodes.Compare"),  # For the allowedTokens code path
                    # For the vector models
                    ("Color", "omni.graph.nodes.ConstantColor3f"),
                    ("Int2", "omni.graph.nodes.ConstantInt2"),
                    ("Quatf", "omni.graph.nodes.ConstantQuatf"),
                    # OmniGraphGfMatrixAttributeModel
                    ("Matrix4d", "omni.graph.nodes.ConstantMatrix4d"),
                    # no_allowed_tokens_model_cls
        ...
        try:
            # Compare
            usd_context.get_selection().set_selected_prim_paths([compare.get_prim_path()], True)
            await ui_test.human_delay(5)
            stage.GetAttributeAtPath(f"{compare.get_prim_path()}.inputs:operation").Set("==")
            await ui_test.human_delay(5)
            await self.capture_and_compare(golden_img_dir=self._golden_img_dir, golden_img_name="test_compare.png")

            # Color
            usd_context.get_selection().set_selected_prim_paths([color.get_prim_path()], True)
            await ui_test.human_delay(5)
            frame = ui_test.find("Property//Frame/**/CollapsableFrame[*].title=='Inputs'")
            components = frame.find_all("**/FloatSlider[*]")
            await components[1].input("0.33")
            await ui_test.human_delay(5)
            await self.capture_and_compare(
                golden_img_dir=self._golden_img_dir, golden_img_name="test_constantcolor3f.png"
            )

            # Int2 - since we don't supply a range we get an IntDrag, not IntSlider
            usd_context.get_selection().set_selected_prim_paths([int2.get_prim_path()], True)
            await ui_test.human_delay(5)
            frame = ui_test.find("Property//Frame/**/CollapsableFrame[*].title=='Inputs'")
            components = frame.find_all("**/IntDrag[*]")
            await components[1].input("42")
            await ui_test.human_delay(5)
            await self.capture_and_compare(golden_img_dir=self._golden_img_dir, golden_img_name="test_constantint2.png")

            # Quatf
            usd_context.get_selection().set_selected_prim_paths([quatf.get_prim_path()], True)
            await ui_test.human_delay(5)
            frame = ui_test.find("Property//Frame/**/CollapsableFrame[*].title=='Inputs'")
            field = frame.find_all("**/MultiFloatDragField[*]")[0]
            await input_multi(field.center - ui_test.Vec2(100, 0), "0.1")
            await self.capture_and_compare(
                golden_img_dir=self._golden_img_dir, golden_img_name="test_constantquatf.png"
            )

            # Matrix4d
            usd_context.get_selection().set_selected_prim_paths([matrix4d.get_prim_path()], True)
            await ui_test.human_delay(5)
            frame = ui_test.find("Property//Frame/**/CollapsableFrame[*].title=='Inputs'")
            field = frame.find_all("**/MultiFloatDragField[*]")[0]
            await input_multi(field.center - ui_test.Vec2(100, 0), "42")
            await self.capture_and_compare(
                golden_img_dir=self._golden_img_dir, golden_img_name="test_constantmatrix4d.png"
            )

            # Token
            usd_context.get_selection().set_selected_prim_paths([token.get_prim_path()], True)
            await ui_test.human_delay(5)
            frame = ui_test.find("Property//Frame/**/CollapsableFrame[*].title=='Inputs'")
            components = frame.find_all("**/StringField[*]")
            await components[0].input("foo")
            await ui_test.human_delay(5)
            await self.capture_and_compare(
                golden_img_dir=self._golden_img_dir, golden_img_name="test_constanttoken.png"
            )
