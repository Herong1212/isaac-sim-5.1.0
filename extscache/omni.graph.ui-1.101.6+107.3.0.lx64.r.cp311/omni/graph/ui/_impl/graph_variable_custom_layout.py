...
class GraphVariableCustomLayout:
    ...
    def __init__(self, compute_node_widget):
        ...
        self.graph_rel_model = None
        self.variables = []
        self.variable_name_model = None

        # 'graph_rel_model' was originally misnamed 'graph_rel_widget'. Since it is part of the public API we
        # have to keep it around for backward compatibility.
        self.graph_rel_widget = None  # Deprecated

    def _variable_name_build_fn(self, *args) -> ui.AbstractItemModel:
    ...
    def _graph_rel_build_fn(self, ui_prop: UsdPropertyUiEntry, *args) -> ui.AbstractItemModel:
        # build the graph relationship widget
        stage = self.compute_node_widget.stage
        node_prim_path = self.compute_node_widget.payload[-1]
        ui_prop.override_display_name("Graph")
        self.graph_rel_model = UsdPropertiesWidgetBuilder._relationship_builder(  # noqa: PLW0212
            stage,
            ui_prop.prop_name,
            ui_prop.metadata,
            [node_prim_path],
        ...
        self.graph_rel_widget = self.graph_rel_model

        return self.graph_rel_model
    ...
    def _on_graph_rel_changed(self, *args):
        ...
        self.graph_rel_model._set_dirty()  # noqa: PLW0212
    ...
    def apply(self, props):
        ...
        def value_build_fn(prop: UsdPropertyUiEntry):
            def build_fn(*args):
                ...
                # Normally we would return the item's model here so that build_property_item() can add it to the
                # widget's list of models. However we've already explicitly called build_property_item() above
                # so we return nothing here to ensure that the model doesn't get added twice.
