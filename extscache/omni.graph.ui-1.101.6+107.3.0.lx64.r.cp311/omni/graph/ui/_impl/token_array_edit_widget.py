...
from omni.kit.property.usd import AllowedTokenItem
...
class TokenArrayModel(ui.AbstractItemModel):
    ...
    def __init__(
        ...
        if isinstance(allowed_tokens[0], str):
            self._allowed_tokens = [AllowedTokenItem(token) for token in allowed_tokens]
        ...
        # if it's a list of tuples, the first item is the displayed value, the second is the actual value
            ...
            self._allowed_tokens = [AllowedTokenItem(token[0]) for token in allowed_tokens]
        ...
        if self._is_actual_token(current):
            ...
            self._allowed_tokens.insert(0, AllowedTokenItem(current))
...
class TokenArrayEditWidget:
    ...
    def __init__(self, stage, attr_name, prim_paths, metadata, additional_widget_kwargs):
        self._stage = stage
        self._model = None
        self._attribute_paths = [path.AppendProperty(attr_name) for path in prim_paths]
    ...
    def _build_remove_button(self, index: int):
        ...
        def on_remove_element(weak_self, index):
            weak_self = weak_self()
            if weak_self:
                token_array = list(weak_self._model.get_value())  # noqa: PLW0212
    ...
    def _build(self):
        # We get the model in the build so it will properly refresh
        model_cls = get_model_cls(UsdAttributeModel, self._additional_widget_kwargs)
        self._model = model_cls(self._stage, self._attribute_paths, False, self._metadata)

        # Show a text field if all attributes do not share the same data
        if self._model.is_ambiguous():
            ui.StringField(name="models", read_only=True, style={"color": 0xFFCC9E61}).model.set_value("Mixed")
        else:
            tokens = list(self._model.get_value())
            ...
            with ui.VStack(spacing=2):
                if allowed_tokens is not None and len(allowed_tokens) > 0:
                    self._build_combo_style_view(tokens, allowed_tokens)
                else:
                    self._build_list_style_view(tokens)
