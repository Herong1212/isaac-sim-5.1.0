...
class OmniGraphTab:
    def build(self, ext_info):
        ...
        with ui.VStack(height=0):
            ...
            if ext_path is None:
                ui.Label(
                    "You must install the extension to see if OmniGraph nodes are present",
                    style_type_name_override="ExtensionDescription.Title",
                )
            else:
                node_data = _get_omnigraph_info(ext_path)
                extension_name = node_data.get("extension", ext_name)
                if node_data:
                    node_name_map = {}
                    for node_name in node_data["nodes"]:
                        if node_name.find(".") < 0:
                            node_name_map[node_name] = f"{extension_name}.{node_name}"
                        else:
                            node_name_map[node_name] = node_name
