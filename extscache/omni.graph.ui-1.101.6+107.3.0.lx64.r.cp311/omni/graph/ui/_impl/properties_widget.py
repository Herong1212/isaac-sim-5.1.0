...
INPUT_DEF_PREFIX = "omniGraphInputDef:"
OUTPUT_DEF_PREFIX = "omniGraphOutputDef:"
...
class OmniGraphAPIPropertiesWidget(UsdPropertiesWidget):
        ...
        for prim_path in payload:  # noqa: PLR1702
            ...
            for attr in attrs:
                ...
                if attr_name_str == OMNIGRAPHS_REL_ATTR:
                    ...
                    processed = True
                elif attr_name_str.startswith(INPUT_DEF_PREFIX) or attr_name_str.startswith(OUTPUT_DEF_PREFIX):
                    processed = True
        return processed
    ...
    def _filter_props_to_build(self, props):
        def allow_prop(prop):
            ...
            name = prop.GetName()
            return (
                name.startswith(OMNIGRAPHS_REL_ATTR)
                or (name.startswith(GRAPH_VARIABLE_ATTR_PREFIX) and prop.GetName() in self._omnigraph_vars)
                or (name.startswith(INPUT_DEF_PREFIX))
                or (name.startswith(OUTPUT_DEF_PREFIX))
            )

        return [prop for prop in props if allow_prop(prop)]
    ...
    def _get_default_value(self, attr_name):
        """Retrieves the default value from the backing attribute"""
    ...
    def _customize_props_layout(self, attrs):
        ...
        instance_props = set()
        for attr in attrs:
            # multi apply properties are 'prefix:var:prop'
            name_split = attr.attr_name.split(":")
            if attr.attr_name == OMNIGRAPHS_REL_ATTR:
                attr.override_display_group("Graphs")
                attr.override_display_name("Graphs")
            elif attr.attr_name in self._omnigraph_vars:
            ...
            elif attr.attr_name.startswith(INPUT_DEF_PREFIX) and len(name_split) >= 2:
                attr.override_display_group(f"Input Definitions:{name_split[1]}")
                if not attr.metadata.get(Sdf.PropertySpec.DisplayNameKey):
                    attr.override_display_name(name_split[2])
            elif attr.attr_name.startswith(OUTPUT_DEF_PREFIX) and len(name_split) >= 2:
                attr.override_display_group(f"Output Definitions:{name_split[1]}")
                if not attr.metadata.get(Sdf.PropertySpec.DisplayNameKey):
                    attr.override_display_name(name_split[2])
        ...
        # modified and become actual properties
        def sort_key(elem):
    ...
    def __build_fn_intercept(
        ...
        # Replace the is_different_from_default method to change the behavior of the reset to default widget
        if models:
