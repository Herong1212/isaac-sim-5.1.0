...
class OmniGraphPropertiesWidgetBuilder(UsdPropertiesWidgetBuilder):
    ...
    # tf_gf_matrix2d = Sdf.ValueTypeNames.Matrix2d.type
    # tf_gf_matrix3d = Sdf.ValueTypeNames.Matrix3d.type
    tf_gf_matrix4d = Sdf.ValueTypeNames.Matrix4d.type
    tf_gf_frame4d = Sdf.ValueTypeNames.Frame4d.type
    tf_gf_quatd = Sdf.ValueTypeNames.Quatd.type
    tf_gf_quatf = Sdf.ValueTypeNames.Quatf.type
    tf_gf_quath = Sdf.ValueTypeNames.Quath.type
    ...
    def init_builder_table(cls):
        ...
        cls.widget_builder_table = {
            ...
            cls.tf_gf_quath: cls._quat_builder,
            cls.tf_gf_quatf: cls._quat_builder,
            cls.tf_gf_quatd: cls._quat_builder,
            # cls.tf_gf_matrix2d: cls._matrix_builder,
            # cls.tf_gf_matrix3d: cls._matrix_builder,
            cls.tf_gf_matrix4d: cls._matrix_builder,
            cls.tf_gf_frame4d: cls._matrix_builder,
        ...
        cls.default_model_table = {
            ...
            # _quat_builder
            cls.tf_gf_quath: OmniGraphGfQuatAttributeModel,
            cls.tf_gf_quatf: OmniGraphGfQuatAttributeModel,
            cls.tf_gf_quatd: OmniGraphGfQuatAttributeModel,
            # _matrix_builder
            # FIXME: matrix2d & matrix3d no not work due to issue with column_count being ignored
            # cls.tf_gf_matrix2d: OmniGraphGfMatrixAttributeModel,
            # cls.tf_gf_matrix3d: OmniGraphGfMatrixAttributeModel,
            cls.tf_gf_matrix4d: OmniGraphGfMatrixAttributeModel,
            cls.tf_gf_frame4d: OmniGraphGfMatrixAttributeModel,
            cls.tf_tf_token: {
    ...
    def _update_target_widget_kwargs(
        ...
        additional_widget_kwargs["relationship_array_single_value_model_cls"] = (
    ...
    def build(
        ...
        if property_type == Usd.Attribute:
            ...
            return cls._target_map_wrapper_builder(
                build_func,
                stage,
                attr_name,
                type_name,
        ...
        if property_type == Usd.Relationship:
            ...
            # _relationship_builder takes a different number of arguments than _target_map_wrapper_builder
            def build_wrapper(
                _stage,
                _attr_name,
                _type_name,
                _metadata,
                _prim_paths,
                _additional_label_kwargs,
                _additional_widget_kwargs,
            ):
                return cls._relationship_builder(
                    _stage, _attr_name, _metadata, _prim_paths, _additional_label_kwargs, _additional_widget_kwargs
                )
    ...
    def _quat_builder(
        ...
        model = model_cls(
            stage, [path.AppendProperty(attr_name) for path in prim_paths], type_name.type, False, metadata
        )
        label = cls._create_label(attr_name, metadata, additional_label_kwargs)
        with ui.HStack():
            kwargs = {"min": -1, "max": 1, "step": 0.001}
            ...
            value_widget.identifier = f"quat_{attr_name}"
        cls._create_control_state(
            model=model, value_widget=value_widget, mixed_overlay=mixed_overlay, **kwargs, label=label
        )
        return model
    ...
    def _matrix_builder(
        ...
        comp_count = int(type_name.type.typeName[-2])
        model_cls = get_model_cls(OmniGraphAttributeModel, additional_widget_kwargs)
        model = model_cls(
            stage, [path.AppendProperty(attr_name) for path in prim_paths], comp_count, type_name.type, False, metadata
        )
        label = cls._create_label(attr_name, metadata, additional_label_kwargs)
        labels = [("X", 0xFF5555AA), ("Y", 0xFF76A371), ("Z", 0xFFA07D4F), ("W", 0xFFAA5555)]
        with ui.HStack():
            kwargs = {"step": 0.001, "column_count": comp_count}
            if additional_widget_kwargs:
                kwargs.update(additional_widget_kwargs)
            value_widget, mixed_overlay = cls._create_multi_float_drag_with_labels(
                model, comp_count=comp_count, labels=labels[:comp_count], **kwargs
            )
    ...
    def _unresolved_builder(
        ...
        if additional_widget_kwargs and additional_widget_kwargs.get("no_control_state"):
            kwargs["no_control_state"] = True
    ...
    @classmethod
    def _target_map_wrapper_builder(
        cls,
        build_func,
        stage,
        attr_name,
        ...
        # check if it has the appropriate customdata
        custom_data = None
        for path in prim_paths:
            prop = stage.GetPropertyAtPath(path.AppendProperty(attr_name))
            if not prop:
                custom_data = None
                break

            value_data = prop.GetCustomDataByKey("omni:graph:targetMap")
            if value_data is None:
                custom_data = None
                break

            # The display is only overridden if all the custom data values are the same.
            # Otherwise it falls back to the regular builder
            token = str(value_data)
        ...
        if custom_data is not None:
            with ui.HStack(spacing=HORIZONTAL_SPACING):
                ...
                ui.Label(f"Mapped to attribute {custom_data} on the Graph Target", style_type_name_override="Code")
            model_cls = get_model_cls(OmniGraphAttributeModel, additional_widget_kwargs)
            return model_cls(stage, [path.AppendProperty(attr_name) for path in prim_paths], False, metadata)
