...
WindowClassType = TypeVar("WindowClassType", OmniGraphSettingsEditor, Toolkit)

VISUAL_SCRIPTING_MENU_INDEX_OFFSET = 20
MENU_GRAPH = "menu_graph.svg"

original_svg_color = carb.settings.get_settings().get_as_bool("/exts/omni.kit.menu.create/original_svg_color")
...
class Menu(metaclass=Singleton):
    ...
    def add_create_menu_type(cls, menu_title: str, evaluator_type: str, prefix: str, glyph_file: str, open_editor_fn):
        ...
        def build_context_menu(objects):
            ...
            objects["vs_menu"] = ContextMenuWidgetExtension.uiMenu(
                "Visual Scripting", glyph=MENU_GRAPH, submenu=True, tearable=True
            )
            objects["vs_menus"] = []
            with objects["vs_menu"]:
                for (
                    title,
                    evaluator_type,
                    prefix,
                    ...
                    objects["vs_menus"].append(
                        ContextMenuWidgetExtension.uiMenuItem(
                            title,
        ...
        if not instance.__context_menus:  # noqa: PLW0212
            ...
            instance.__context_menus.append(  # noqa: PLW0212
                omni.kit.widget.context_menu.add_menu(menu_dict, "CREATE", "omni.kit.widget.stage")
            )
            instance.__context_menus.append(  # noqa: PLW0212
                omni.kit.widget.context_menu.add_menu(menu_dict, "CREATE", "omni.kit.viewport.window")
            )
