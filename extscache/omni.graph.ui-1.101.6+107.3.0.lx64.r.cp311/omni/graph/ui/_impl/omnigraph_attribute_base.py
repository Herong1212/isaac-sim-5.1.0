...
HELD_ATTRIB = og.Attribute | PlaceholderOmniGraphAttribute
...
class OmniGraphBase:
    ...
    def _change_property(self, attribute: og.Attribute, new_value, old_value):
        ...
        # For extended attributes the USD property only contains a token, not the actual value: that is stored
        # in Fabric. We need to update the Fabric value directly using the OG API.
        #
        # For regular attributes the USD property contains the initial value, before any changes that might have
        # happened as a result of evaluation. The current "runtime" value is stored in Fabric and that is the value
        # displayed by the OG property widget. If the user edits the value in the widget we want it to go into both
        # USD and Fabric. We do this by changing the USD value and letting OG's USD listener update Fabric.
        #
        # However, it's possible that the user is changing the value to what's still stored in USD, and USD won't
        # send a notification if the new value is the same as the old. In that case we again need to update Fabric
        # directly using the OG API.
        is_extended_attr = attribute.get_extended_type() != og.ExtendedAttributeType.EXTENDED_ATTR_TYPE_REGULAR
        usd_value = self._stage.GetAttributeAtPath(path).Get()
        if is_extended_attr or new_value == usd_value:
            if new_value.__class__.__module__ == "pxr.Gf" and new_value.__class__.__name__.startswith("Quat"):
                og_value = [*new_value.GetImaginary(), new_value.GetReal()]
            else:
                og_value = new_value
            og.Controller.set(attribute, og_value)

        # Now change the value in USD.
        if is_extended_attr:
            # Even though the value is not stored in USD we still want a USD notice to be generated.
            #
            # The USD attrib is actually a token, so the hack here is to change the token value.
            # FIXME: Obviously not ideal to be tokenizing the value.
            omni.kit.commands.execute("ChangeProperty", prop_path=path, value=str(new_value), prev=str(usd_value))
        else:
            ...
            omni.kit.commands.execute("ChangeProperty", prop_path=path, value=new_value, prev=usd_value)
    ...
    @carb.profiler.profile
    def _read_value_and_type(self, attr_object: HELD_ATTRIB) -> tuple[Any, og.Type | None]:
