...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        ...
        },
        {
            '': {
                ...
                'outputs': [
                    ['outputs:faceVertexCounts', [4, 4, 4, 4, 4, 4, 4, 4, 4], False],
                    ['outputs:faceVertexIndices', [0, 1, 3, 2, 1, 0, 2, 3, 4, 5, 8, 7, 5, 6, 9, 8, 6, 4, 7, 9, 10, 11, 15, 14, 11, 12, 16, 15, 12, 13, 17, 16, 13, 10, 14, 17], False],
                ],
            },
        },
        {
            '': {
                ...
                'outputs': [
                    ['outputs:faceVertexCounts', [], False],
                    ['outputs:faceVertexIndices', [], False],
                ],
            },
        },
        {
            '': {
                'inputs': [
                    ['inputs:rows', [1, 2, 3], False],
                    ['inputs:cols', [-1, 0, 2], False],
                ],
                'outputs': [
                    ['outputs:faceVertexCounts', [4, 4, 4, 4, 4, 4], False],
                    ['outputs:faceVertexIndices', [0, 1, 3, 2, 1, 0, 2, 3, 2, 3, 5, 4, 3, 2, 4, 5, 4, 5, 7, 6, 5, 4, 6, 7], False],
                ],
            },
        },
    ]
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_CreateTubeTopology", "omni.graph.nodes.CreateTubeTopology", test_run, _TestGraphAndNode())
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_CreateTubeTopology","omni.graph.nodes.CreateTubeTopology", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
    ...
    async def test_thread_safety(self):
        ...
        for (key, test_info) in instance_setup.copy().items():
            instance_setup[key] = await _test_setup_scene(self, og.Controller(allow_exists_prim=True), key, "TestNode_omni_graph_nodes_CreateTubeTopology", "omni.graph.nodes.CreateTubeTopology", test_data_setup[key], test_info)
        ...
        for (key, test_info) in instance_setup.items():
            _test_verify_scene(self, og.Controller(), test_data_setup[key], test_info, f"omni.graph.nodes.CreateTubeTopology User test case instance{key}")
