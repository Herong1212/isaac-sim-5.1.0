...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        {
            '': {
                'inputs': [
                    ['inputs:tuple', {'type': 'double[2]', 'value': [42.0, 1.0]}, False],
                ],
                'outputs': [
        ...
        },
        {
            '': {
                'inputs': [
                    ['inputs:tuple', {'type': 'half[2]', 'value': [42.0, 1.0]}, False],
                ],
                'outputs': [
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_BreakVector2", "omni.graph.nodes.BreakVector2", test_run, _TestGraphAndNode())
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_BreakVector2","omni.graph.nodes.BreakVector2", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
    ...
    async def test_thread_safety(self):
        ...
        for (key, test_info) in instance_setup.copy().items():
            instance_setup[key] = await _test_setup_scene(self, og.Controller(allow_exists_prim=True), key, "TestNode_omni_graph_nodes_BreakVector2", "omni.graph.nodes.BreakVector2", test_data_setup[key], test_info)
        ...
        for (key, test_info) in instance_setup.items():
            _test_verify_scene(self, og.Controller(), test_data_setup[key], test_info, f"omni.graph.nodes.BreakVector2 User test case instance{key}")
