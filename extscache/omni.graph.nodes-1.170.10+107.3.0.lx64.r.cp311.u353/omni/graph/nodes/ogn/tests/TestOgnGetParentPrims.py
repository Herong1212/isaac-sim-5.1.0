...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        {
            '': {
                'inputs': [
                    ['inputs:prims', "/A", False],
                ],
                'outputs': [
                    ['outputs:parentPrims', "/", False],
                ],
            },
        },
        {
            '': {
                'inputs': [
                    ['inputs:prims', ["/A", "/A/B", "/A/B/C"], False],
                ],
                'outputs': [
                    ['outputs:parentPrims', ["/", "/A", "/A/B"], False],
                ],
            },
        },
    ]
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_GetParentPrims", "omni.graph.nodes.GetParentPrims", test_run, _TestGraphAndNode())
            await controller.evaluate(test_info.graph)
            _test_verify_scene(self, controller, test_run, test_info, f"omni.graph.nodes.GetParentPrims User test case #{i+1}")
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_omni_graph_nodes_GetParentPrims","omni.graph.nodes.GetParentPrims", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
            _test_verify_scene(self, controller, test_run, test_info, f"omni.graph.nodes.GetParentPrims User test case #{i+1}", 16)
    ...
    async def test_thread_safety(self):
        ...
        for (key, test_info) in instance_setup.copy().items():
            instance_setup[key] = await _test_setup_scene(self, og.Controller(allow_exists_prim=True), key, "TestNode_omni_graph_nodes_GetParentPrims", "omni.graph.nodes.GetParentPrims", test_data_setup[key], test_info)
        ...
        for (key, test_info) in instance_setup.items():
            _test_verify_scene(self, og.Controller(), test_data_setup[key], test_info, f"omni.graph.nodes.GetParentPrims User test case instance{key}")
