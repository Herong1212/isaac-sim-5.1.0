...
template <typename T>
size_t tryComputeAssumingType(OgnToRadDatabase& db, size_t count)
{
    auto functor = [](auto const& degrees, auto& radians) { radians = static_cast<T>(pxr::GfDegreesToRadians(degrees)); };
    return ogn::compute::tryComputeWithArrayBroadcasting<T>(db.inputs.degrees(), db.outputs.radians(), functor, count) ?
               count :
               0;
}
...
template <>
size_t tryComputeAssumingType<pxr::GfHalf>(OgnToRadDatabase& db, size_t count)
{
    auto functor = [](auto const& degrees, auto& radians)
    { radians = static_cast<pxr::GfHalf>(static_cast<float>(pxr::GfDegreesToRadians(degrees))); };
    return ogn::compute::tryComputeWithArrayBroadcasting<pxr::GfHalf>(
               db.inputs.degrees(), db.outputs.radians(), functor, count) ?
               count :
               0;
}
...
{
    auto functor = [](auto const& degrees, auto& radians) { radians = static_cast<T>(pxr::GfDegreesToRadians(degrees)); };
    return ogn::compute::tryComputeWithTupleBroadcasting<N, T>(db.inputs.degrees(), db.outputs.radians(), functor, count) ?
               count :
               0;
}
...
{
public:
    static size_t computeVectorized(OgnToRadDatabase& db, size_t count)
    {
        try
        {
            const auto& inputType = db.inputs.degrees().type();
