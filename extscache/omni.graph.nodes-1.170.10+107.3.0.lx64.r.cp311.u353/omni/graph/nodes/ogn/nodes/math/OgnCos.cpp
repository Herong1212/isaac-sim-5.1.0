...
template <typename T>
size_t tryComputeAssumingType(OgnCosDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<T>(std::cos(pxr::GfDegreesToRadians(value))); };
...
template <>
size_t tryComputeAssumingType<pxr::GfHalf>(OgnCosDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<pxr::GfHalf>(static_cast<float>(std::cos(pxr::GfDegreesToRadians(value)))); };
...
{
public:
    static size_t computeVectorized(OgnCosDatabase& db, size_t count)
    {
        try
        {
