...
bool tryComputeAssumingType(OgnSubtractDatabase& db, size_t count)
{
    ...
    {
        auto functor = [](auto const& a, auto const& b, auto& result) { result = a - b; };
        return ogn::compute::tryComputeWithArrayBroadcasting<T>(
            db.inputs.a(), db.inputs.b(), db.outputs.difference(), functor, count);
    }
    else
    {
        ...
        return ogn::compute::tryComputeInputsWithArrayBroadcasting<T>(
            inputArray, db.outputs.difference(), functor, count);
    }
}
...
public:
    ...
    static void onConnectionTypeResolve(const NodeObj& node)
    {
        ...
        auto difference = node.iNode->getAttributeByToken(node, outputs::difference.token());
        for (auto const& attr : allAttributes)
        {
            if (attr.iAttribute->getPortType(attr) == AttributePortType::kAttributePortType_Input)
            {
                ...
                {
                    difference.iAttribute->setResolvedType(difference, Type(BaseDataType::eUnknown));
                    return;
                }
        ...
        attributes.push_back(difference);
