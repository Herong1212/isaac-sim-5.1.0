...
template <typename T>
size_t tryComputeAssumingType(OgnATan2Database& db, size_t count)
{
    auto functor = [](auto const& a, auto const& b, auto& result)
    { result = static_cast<T>(pxr::GfRadiansToDegrees(std::atan2(a, b))); };
...
template <>
size_t tryComputeAssumingType<pxr::GfHalf>(OgnATan2Database& db, size_t count)
{
    auto functor = [](auto const& a, auto const& b, auto& result)
    { result = static_cast<pxr::GfHalf>(static_cast<float>(pxr::GfRadiansToDegrees(std::atan2(a, b)))); };
...
{
public:
    static size_t computeVectorized(OgnATan2Database& db, size_t count)
    {
        try
        {
