...
{
template <typename T>
size_t tryComputeAssumingType(OgnMaximumDatabase& db, size_t count)
{
    auto const& dynamicInputs = db.getDynamicInputs();
    if (dynamicInputs.empty())
    {
        auto functor = [](T const& a, T const& b, T& result) { result = std::max(a, b); };
        return ogn::compute::tryComputeWithArrayBroadcasting<T>(
                   db.inputs.a(), db.inputs.b(), db.outputs.maximum(), functor, count) ?
                   count :
                   0;
    }
    else
    {
        ...
        auto functor = [](const auto& input, auto& result) { result = std::max(input, result); };
        return ogn::compute::tryComputeInputsWithArrayBroadcasting<T>(inputArray, db.outputs.maximum(), functor, count) ?
                   count :
                   0;
    }
}
...
size_t tryComputeAssumingType(OgnMaximumDatabase& db, size_t count)
{
    ...
    else
    {
        ...
        return ogn::compute::tryComputeInputsWithTupleBroadcasting<N, T>(
                   inputArray, db.outputs.maximum(), functor, count) ?
                   count :
                   0;
    }
}
...
{
public:
    static size_t computeVectorized(OgnMaximumDatabase& db, size_t count)
    {
        try
        {
            auto& outputType = db.outputs.maximum().type();
    ...
    static void onConnectionTypeResolve(const NodeObj& node)
    {
        ...
        auto output = node.iNode->getAttributeByToken(node, outputs::maximum.token());
