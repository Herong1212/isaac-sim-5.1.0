...
template <typename T>
size_t tryComputeAssumingType(OgnAsinDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<T>(pxr::GfRadiansToDegrees(std::asin(value))); };
...
template <>
size_t tryComputeAssumingType<pxr::GfHalf>(OgnAsinDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<pxr::GfHalf>(static_cast<float>(pxr::GfRadiansToDegrees(std::asin(value)))); };
...
{
public:
    static size_t computeVectorized(OgnAsinDatabase& db, size_t count)
    {
        try
        {
