...
template <typename T>
size_t tryComputeAssumingType(OgnSinDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<T>(std::sin(pxr::GfDegreesToRadians(value))); };
...
template <>
size_t tryComputeAssumingType<pxr::GfHalf>(OgnSinDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<pxr::GfHalf>(static_cast<float>(std::sin(pxr::GfDegreesToRadians(value)))); };
...
{
public:
    static size_t computeVectorized(OgnSinDatabase& db, size_t count)
    {
        try
        {
