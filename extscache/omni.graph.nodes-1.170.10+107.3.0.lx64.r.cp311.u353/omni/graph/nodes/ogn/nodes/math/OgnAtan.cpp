...
template <typename T>
size_t tryComputeAssumingType(OgnAtanDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<T>(pxr::GfRadiansToDegrees(std::atan(value))); };
...
template <>
size_t tryComputeAssumingType<pxr::GfHalf>(OgnAtanDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& result)
    { result = static_cast<pxr::GfHalf>(static_cast<float>(pxr::GfRadiansToDegrees(std::atan(value)))); };
...
{
public:
    static size_t computeVectorized(OgnAtanDatabase& db, size_t count)
    {
        try
        {
