...
/**
 * Boolean NOR on two inputs.
 * If a and b are arrays, NOR operations will be performed pair-wise. Sizes of a and b must match.
...
size_t tryCompute(OgnNorDatabase& db, size_t count)
{
    ...
    // The NOR operator is not associative.
    ...
    auto orFunctor = [](bool const& a, bool& result) { result = static_cast<bool>(a || result); };
    bool result =
        ogn::compute::tryComputeInputsWithArrayBroadcasting<bool>(inputs, db.outputs.result(), orFunctor, count);
...
{
public:
    static size_t computeVectorized(OgnNorDatabase& db, size_t count)
    {
        return tryComputeOperator<OgnNorDatabase>(tryCompute, db, count);
    }
