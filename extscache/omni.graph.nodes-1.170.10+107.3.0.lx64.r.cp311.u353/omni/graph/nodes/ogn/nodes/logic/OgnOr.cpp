...
/**
 * Boolean OR on two inputs.
 * If a and b are arrays, OR operations will be performed pair-wise. Sizes of a and b must match.
...
size_t tryCompute(OgnOrDatabase& db, size_t count)
{
...
{
public:
    static size_t computeVectorized(OgnOrDatabase& db, size_t count)
    {
        return tryComputeOperator<OgnOrDatabase>(tryCompute, db, count);
    }
