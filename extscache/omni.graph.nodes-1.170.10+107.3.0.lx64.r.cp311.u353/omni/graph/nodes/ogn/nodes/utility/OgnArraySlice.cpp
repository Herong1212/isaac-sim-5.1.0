...
#include <OgnArraySliceDatabase.h>
...
// Helper to wrap a given index to [0:arrayLength].
size_t tryWrapIndex(int index, size_t arraySize)
{
    ...
    if (wrappedIndex < 0)
        wrappedIndex = 0;
    else if (wrappedIndex > static_cast<int>(arraySize))
        wrappedIndex = static_cast<int>(arraySize);
    return static_cast<size_t>(wrappedIndex);
}

template <typename BaseType>
size_t tryComputeAssumingType(OgnArraySliceDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
        const auto inputArray = db.inputs.array(idx).template get<BaseType[]>();
        auto outputArray = db.outputs.array(idx).template get<BaseType[]>();
        const size_t inputArraySize = db.inputs.array(idx).size();
        const size_t start = tryWrapIndex(db.inputs.start(idx), inputArraySize);
        const size_t end = db.inputs.useLength(idx) ? inputArraySize : tryWrapIndex(db.inputs.end(idx), inputArraySize);

        if (inputArray && outputArray)
        {
            if (!inputArray->empty() && (start < end))
            {
                const size_t outputArraySize = end - start;
                outputArray->resize(outputArraySize);
                memcpy(outputArray->data(), inputArray->data() + start, sizeof(BaseType) * outputArraySize);
            }
            else
...
{
public:
    static size_t computeVectorized(OgnArraySliceDatabase& db, size_t count)
    {
        try
        {
