...
template <typename T>
size_t tryComputeAssumingType(OgnToUintDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& converted) { converted = static_cast<uint32_t>(value); };
    return ogn::compute::tryComputeWithArrayBroadcasting<T, uint32_t>(
...
public:
    ...
    static size_t computeVectorized(OgnToUintDatabase& db, size_t count)
    {
    ...
    static void onConnectionTypeResolve(const NodeObj& nodeObj)
    {
        ...
        {
            Type resultType(BaseDataType::eUInt, 1, valueType.arrayDepth);
