...
#include <OgnArrayAppendValueDatabase.h>
...
size_t tryComputeAssumingType(OgnArrayAppendValueDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
        ...
        if (value && inputArray && outputArray)
        {
            ...
            memcpy(&((*outputArray)[inputArraySize]), &*value, sizeof(BaseType));
        }
    }
    return count;
}
...
class OgnArrayAppendValue
{
public:
    static size_t computeVectorized(OgnArrayAppendValueDatabase& db, size_t count)
    {
        try
        {
