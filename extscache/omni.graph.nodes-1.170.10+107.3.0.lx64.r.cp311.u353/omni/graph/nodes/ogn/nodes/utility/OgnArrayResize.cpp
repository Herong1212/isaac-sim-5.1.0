...
size_t tryComputeAssumingType(OgnArrayResizeDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
        ...
        size_t newSize = std::max(db.inputs.newSize(idx), 0);
...
{
public:
    static size_t computeVectorized(OgnArrayResizeDatabase& db, size_t count)
    {
        try
        {
