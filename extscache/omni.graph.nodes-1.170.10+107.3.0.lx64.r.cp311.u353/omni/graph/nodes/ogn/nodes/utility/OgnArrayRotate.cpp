...
constexpr ForwardIt rotateArray(ForwardIt first, ForwardIt n_first, ForwardIt last)
{
    if (first == n_first)
        return last;
    if (n_first == last)
...
template <typename T>
size_t tryComputeAssumingType(OgnArrayRotateDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
        auto array = db.inputs.array(idx).template get<T[]>();
        auto result = db.outputs.array(idx).template get<T[]>();
        ...
        if (array && result)
        {
            *result = *array;
            if (!result->empty())
            {
                // Values will wrap back to zero when they are -arrayLength or arrayLength
                auto steps = db.inputs.steps(idx) % (int)array->size();
                if (steps < 0)
                {
...
{
public:
    static size_t computeVectorized(OgnArrayRotateDatabase& db, size_t count)
    {
        try
        {
