...
{
template <typename T>
size_t tryComputeAssumingType(OgnToFloatDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& converted) { converted = static_cast<float>(value); };
    return ogn::compute::tryComputeWithArrayBroadcasting<T, float>(
...
class OgnToFloat
{
    ...
    {
        auto& state = OgnToFloatDatabase::sSharedState<OgnToFloat>(nodeObj);
        ...
        Type resultType = getOutputType(valueType, BaseDataType::eFloat, state.m_role);
...
public:
    ...
    static void initialize(GraphContextObj const& context, NodeObj const& nodeObj)
    {
        ...
        state.m_roles = { { OgnToFloatDatabase::tokens.eNone, AttributeRole::eNone },
                          { OgnToFloatDatabase::tokens.eColor, AttributeRole::eColor },
                          { OgnToFloatDatabase::tokens.eNormal, AttributeRole::eNormal },
                          { OgnToFloatDatabase::tokens.ePosition, AttributeRole::ePosition },
                          { OgnToFloatDatabase::tokens.eQuaternion, AttributeRole::eQuaternion },
                          { OgnToFloatDatabase::tokens.eTexCoord, AttributeRole::eTexCoord },
                          { OgnToFloatDatabase::tokens.eVector, AttributeRole::eVector } };
    ...
    static size_t computeVectorized(OgnToFloatDatabase& db, size_t count)
    {
