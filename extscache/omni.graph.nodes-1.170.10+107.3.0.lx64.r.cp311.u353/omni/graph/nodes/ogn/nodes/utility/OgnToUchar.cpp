...
{
template <typename T>
size_t tryComputeAssumingType(OgnToUcharDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& converted) { converted = static_cast<uchar>(value); };
    return ogn::compute::tryComputeWithArrayBroadcasting<T, uint8_t>(
...
public:
    ...
    static size_t computeVectorized(OgnToUcharDatabase& db, size_t count)
    {
    ...
    static void onConnectionTypeResolve(const NodeObj& nodeObj)
    {
        ...
        {
            Type resultType(BaseDataType::eUChar, 1, valueType.arrayDepth);
