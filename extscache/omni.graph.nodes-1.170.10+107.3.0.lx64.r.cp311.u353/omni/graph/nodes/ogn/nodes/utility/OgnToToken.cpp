...
{
template <typename T>
size_t tryComputeAssumingType(OgnToTokenDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
        ...
        if (!converted.empty())
            db.outputs.converted(idx) = db.stringToToken(converted.c_str());
    }
    return count;
}
...
template <>
size_t tryComputeAssumingType<ogn::Token>(OgnToTokenDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
...
template <>
size_t tryComputeAssumingType<string>(OgnToTokenDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
...
public:
    ...
    static size_t computeVectorized(OgnToTokenDatabase& db, size_t count)
    {
