...
size_t tryComputeAssumingType(OgnArrayInsertValueDatabase& db, size_t count)
{
    for (size_t idx = 0; idx < count; ++idx)
    {
        ...
        if (value && inputArray && outputArray)
        {
            ...
            memcpy(outputArray->data() + index + 1, inputArray->data() + index,
                   sizeof(BaseType) * (inputArraySize - index));
        }
    }
    return count;
}
...
{
public:
    static size_t computeVectorized(OgnArrayInsertValueDatabase& db, size_t count)
    {
        try
        {
