...
{
template <typename T>
size_t tryComputeAssumingType(OgnToDoubleDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& converted) { converted = static_cast<double>(value); };
    return ogn::compute::tryComputeWithArrayBroadcasting<T, double>(
...
class OgnToDouble
{
    ...
    {
        auto& state = OgnToDoubleDatabase::sSharedState<OgnToDouble>(nodeObj);
        ...
        Type resultType = getOutputType(valueType, BaseDataType::eDouble, state.m_role);
...
public:
    ...
    static void initialize(GraphContextObj const& context, NodeObj const& nodeObj)
    {
        ...
        state.m_roles = { { OgnToDoubleDatabase::tokens.eNone, AttributeRole::eNone },
                          { OgnToDoubleDatabase::tokens.eColor, AttributeRole::eColor },
                          { OgnToDoubleDatabase::tokens.eMatrix, AttributeRole::eMatrix },
                          { OgnToDoubleDatabase::tokens.eFrame, AttributeRole::eFrame },
                          { OgnToDoubleDatabase::tokens.eNormal, AttributeRole::eNormal },
                          { OgnToDoubleDatabase::tokens.ePosition, AttributeRole::ePosition },
                          { OgnToDoubleDatabase::tokens.eQuaternion, AttributeRole::eQuaternion },
                          { OgnToDoubleDatabase::tokens.eTexCoord, AttributeRole::eTexCoord },
                          { OgnToDoubleDatabase::tokens.eTimeCode, AttributeRole::eTimeCode },
                          { OgnToDoubleDatabase::tokens.eVector, AttributeRole::eVector } };
    ...
    static size_t computeVectorized(OgnToDoubleDatabase& db, size_t count)
    {
