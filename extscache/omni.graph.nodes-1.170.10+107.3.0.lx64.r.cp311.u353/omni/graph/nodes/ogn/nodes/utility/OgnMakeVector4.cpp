...
{
template <typename T>
size_t tryComputeAssumingType(OgnMakeVector4Database& db, size_t count)
{
    auto functor = [](auto const& x, auto const& y, auto const& z, auto const& w, auto& output)
    {
        auto& vector = *reinterpret_cast<vec4<T>*>(output);
        vector = vector.Set(x, y, z, w);
    };
    return ogn::compute::tryComputeWithArrayBroadcasting<T, T, T, T, T[4]>(
               db.inputs.x(), db.inputs.y(), db.inputs.z(), db.inputs.w(), db.outputs.tuple(), functor, count) ?
               count :
               0;
}
} // namespace
...
{
public:
    static size_t computeVectorized(OgnMakeVector4Database& db, size_t count)
    {
        try
        {
    ...
    static void onConnectionTypeResolve(const NodeObj& nodeObj)
    {
        ...
        auto zType = y.iAttribute->getResolvedType(z);
        auto wType = z.iAttribute->getResolvedType(w);
