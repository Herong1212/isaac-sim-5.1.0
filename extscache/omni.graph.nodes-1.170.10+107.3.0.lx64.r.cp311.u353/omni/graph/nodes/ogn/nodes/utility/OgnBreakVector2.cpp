...
{
template <typename T>
bool tryComputeAssumingType(OgnBreakVector2Database& db, size_t count)
{
    switch (db.inputs.tuple().type().arrayDepth)
    {
    case 0:
    {
        const auto vector = db.inputs.tuple().template get<T[2]>();
        ...
        if (vector && x && y)
        {
            ...
            if (!pVector.empty() && !px.empty() && !py.empty())
            {
    ...
    case 1:
        for (size_t idx = 0; idx < count; idx++)
        {
            const auto vector = db.inputs.tuple(idx).template get<T[][2]>();
...
{
public:
    static size_t computeVectorized(OgnBreakVector2Database& db, size_t count)
    {
        try
        {
    ...
    static void onConnectionTypeResolve(const NodeObj& nodeObj)
    {
        ...
        if (vectorType.baseType != BaseDataType::eUnknown)
        {
            ...
            std::array<uint8_t, 3> arrays{ vectorType.arrayDepth, vectorType.arrayDepth, vectorType.arrayDepth };
