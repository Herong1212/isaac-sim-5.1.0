...
{
template <typename T>
size_t tryComputeAssumingType(OgnToBoolDatabase& db, size_t count)
{
    auto functor = [](auto const& value, auto& converted) { converted = static_cast<bool>(value); };
    return ogn::compute::tryComputeWithArrayBroadcasting<T, bool>(
...
public:
    ...
    static size_t computeVectorized(OgnToBoolDatabase& db, size_t count)
    {
    ...
    static void onConnectionTypeResolve(const NodeObj& nodeObj)
    {
        ...
        if (valueType.baseType != BaseDataType::eUnknown)
            outAttr.iAttribute->setResolvedType(outAttr, Type(BaseDataType::eBool, 1, valueType.arrayDepth));
        else
