...
{
template <typename T>
size_t tryComputeAssumingType(OgnToInt64Database& db, size_t count)
{
    auto functor = [](auto const& value, auto& converted) { converted = static_cast<int64_t>(value); };
    return ogn::compute::tryComputeWithArrayBroadcasting<T, int64_t>(
...
public:
    ...
    static size_t computeVectorized(OgnToInt64Database& db, size_t count)
    {
    ...
    static void onConnectionTypeResolve(const NodeObj& nodeObj)
    {
        ...
        {
            Type resultType(BaseDataType::eInt64, 1, valueType.arrayDepth);
