...
{
template <typename T>
bool tryComputeAssumingType(OgnBreakVector3Database& db, size_t count)
{
    switch (db.inputs.tuple().type().arrayDepth)
    {
    case 0:
    {
        const auto vector = db.inputs.tuple().template get<T[3]>();
    ...
    case 1:
        for (size_t idx = 0; idx < count; idx++)
        {
            const auto vector = db.inputs.tuple(idx).template get<T[][3]>();
...
public:
    ...
    static void onConnectionTypeResolve(const NodeObj& nodeObj)
    {
        ...
        if (vectorType.baseType != BaseDataType::eUnknown)
        {
            ...
            std::array<uint8_t, 4> arrays{ vectorType.arrayDepth, vectorType.arrayDepth, vectorType.arrayDepth,
                                           vectorType.arrayDepth };
            std::array<AttributeRole, 4> roles{ vectorType.role, AttributeRole::eNone, AttributeRole::eNone,
                                                AttributeRole::eNone };
