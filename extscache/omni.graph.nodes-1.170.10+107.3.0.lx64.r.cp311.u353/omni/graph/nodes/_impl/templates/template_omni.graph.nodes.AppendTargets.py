...
from omni.graph.nodes.scripts.ui_utils import _build_add_remove_buttons, _retrieve_existing_numeric_dynamic_inputs
        ...
        if new_count > current_attrib_count:
            ...
            while create_count > 0:
                ...
                self.controller.create_attribute(
                    ...
                    None,
                    og.ExtendedAttributeType.REGULAR,
                )
        else:
    ...
    def apply(self, props):
        ...
        with frame:
            with CustomLayoutGroup("Inputs"):
                prop = find_prop("inputs:allowDuplicates")
                if prop is not None:
                    CustomLayoutProperty(prop.prop_name)

                for attrib in input_attribs:
                    attrib.set_metadata(ogn.MetadataKeys.ALLOW_MULTI_INPUTS, "1")
            ...
            prop = find_prop("outputs:targets")
            if prop is not None:
