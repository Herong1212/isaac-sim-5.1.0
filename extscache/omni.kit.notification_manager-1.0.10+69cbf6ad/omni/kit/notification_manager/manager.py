...
class Notification:
    ...
    async def _docking_to(self, window_x, window_y, window_width, window_height, offset_y=0) -> bool:
        if self._prompt:
            return await self._prompt.docking_to(window_x, window_y, window_width, window_height, offset_y)
        return False
    ...
    async def _pre_warming(self):
        ...
        async def _warming():
            while not self._notification_mananger._shutdown and self._prompt and (self._prompt.width == 0.0 or self._prompt.height == 0.0):
                await omni.kit.app.get_app().next_update_async()
            self._hide()

            if not self._notification_mananger._shutdown and self._prompt:
    ...
    def on_startup(self):
        ...
        async def timer_fn():
            while not self._shutdown:
                ...
                while len(self._pre_warming_notifications) > 0:
                    notification = self._pre_warming_notifications.pop(0)
                ...
                self._notifications = [n for n in self._notifications if n not in expired_notifications]
                ...
                if changed:
                    offset_y = 0
                    for notification in self._notifications:
                        docked = await notification._docking_to(self._docking_window_pos_x, self._docking_window_pos_y, self._docking_window_width, self._docking_window_height, offset_y)
                        if docked:
