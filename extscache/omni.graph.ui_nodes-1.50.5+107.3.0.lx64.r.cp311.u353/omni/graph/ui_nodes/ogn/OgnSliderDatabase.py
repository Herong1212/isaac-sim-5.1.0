r"""Support for simplified access to data on nodes of type omni.graph.ui_nodes.Slider
...
class OgnSliderDatabase(og.Database):  # pragma: no cover
    """Helper class providing simplified access to data on nodes of type omni.graph.ui_nodes.Slider
    ...
    INTERFACE = og.Database._get_interface([
        ('inputs:create', 'execution', 0, 'Create', 'Signal to the graph that this node is ready to create and show the widget.', {}, True, None, False, ''),
        ('inputs:disable', 'execution', 0, 'Disable', 'Signal to the graph that this node is ready to disable this button so that it cannot be pressed.', {}, True, None, False, ''),
        ('inputs:enable', 'execution', 0, 'Enable', 'Signal to the graph that this node is ready enable this button after it has been disabled.', {}, True, None, False, ''),
        ('inputs:hide', 'execution', 0, 'Hide', 'Signal to the graph that this node is ready to hide the widget and all its child widgets.', {}, True, None, False, ''),
        ...
        ('inputs:show', 'execution', 0, 'Show', 'Signal to the graph that this node is ready to show the widget and all its child widgets after they become hidden.', {}, True, None, False, ''),
        ('inputs:step', 'float', 0, 'Step', 'The step size of the slider', {ogn.MetadataKeys.DEFAULT: '0.01'}, True, 0.01, False, ''),
        ('inputs:tearDown', 'execution', 0, 'Tear Down', 'Signal to the graph that this node is ready to tear down the widget and all its child widgets.', {}, True, None, False, ''),
        ...
        ('outputs:created', 'execution', 0, 'Created', 'When the widget is created, signal to the graph that execution can continue downstream.', {}, True, None, False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.disable = og.AttributeRole.EXECUTION
        role_data.inputs.enable = og.AttributeRole.EXECUTION
        role_data.inputs.hide = og.AttributeRole.EXECUTION
        role_data.inputs.show = og.AttributeRole.EXECUTION
        role_data.inputs.tearDown = og.AttributeRole.EXECUTION
    ...
    class abi:
        ...
        def get_node_type():
            ...
            return 'omni.graph.ui_nodes.Slider'
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnSliderDatabase.abi.initialize(context, n)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnSliderDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnSliderDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnSliderDatabase._release_per_node_instance_data(node, graph_instance_id)
    ...
    @staticmethod
    def deregister():
        og.deregister_node_type("omni.graph.ui_nodes.Slider")
