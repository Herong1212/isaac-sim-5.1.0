r"""Support for simplified access to data on nodes of type omni.graph.ui_nodes.OnWidgetClicked
...
class OgnOnWidgetClickedDatabase(og.Database):  # pragma: no cover
    """Helper class providing simplified access to data on nodes of type omni.graph.ui_nodes.OnWidgetClicked
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('outputs:clicked', 'execution', 0, 'Clicked', 'When the widget is clicked, signal to the graph that execution can continue downstream.', {}, True, None, False, ''),
    ])
    ...
    def _populate_role_data(cls):
        ...
        role_data.outputs.clicked = og.AttributeRole.EXECUTION
        return role_data
    ...
    class abi:
        ...
        def get_node_type():
            ...
            return 'omni.graph.ui_nodes.OnWidgetClicked'
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnOnWidgetClickedDatabase.abi.initialize(context, n)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnOnWidgetClickedDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnOnWidgetClickedDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnOnWidgetClickedDatabase._release_per_node_instance_data(node, graph_instance_id)
    ...
    @staticmethod
    def deregister():
        og.deregister_node_type("omni.graph.ui_nodes.OnWidgetClicked")
