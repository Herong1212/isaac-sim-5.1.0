r"""Support for simplified access to data on nodes of type omni.graph.ui_nodes.OnWidgetValueChanged
...
class OgnOnWidgetValueChangedDatabase(og.Database):  # pragma: no cover
    """Helper class providing simplified access to data on nodes of type omni.graph.ui_nodes.OnWidgetValueChanged
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('outputs:valueChanged', 'execution', 0, 'Value Changed', 'When the value of the widget is changed, signal to the graph that execution can continue downstream.', {}, True, None, False, ''),
    ])
    ...
    class abi:
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnOnWidgetValueChangedDatabase.abi.initialize(context, n)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnOnWidgetValueChangedDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnOnWidgetValueChangedDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnOnWidgetValueChangedDatabase._release_per_node_instance_data(node, graph_instance_id)
    ...
    @staticmethod
    def deregister():
        og.deregister_node_type("omni.graph.ui_nodes.OnWidgetValueChanged")
