...
GRAPH_VARIABLE_PREFIX = "anim:graph:variable:"
...
def get_filtered_variables(prim: Usd.Prim) -> List[str]:
    ...
    return [attr.GetName()[len(GRAPH_VARIABLE_PREFIX):] for attr in prim.GetAttributes() if not attr.IsHidden() and is_usable(attr.GetTypeName()) and attr.GetName().startswith(GRAPH_VARIABLE_PREFIX)]
...
class GraphVariableCustomLayout:
    ...
    def _variable_name_build_fn(self, *args):
        ...
        node_prim_path = self.compute_node_widget._payload[-1]
    ...
    def _graph_rel_build_fn(self, ui_prop: UsdPropertyUiEntry, *args):
        ...
        self.graph_rel_widget = UsdPropertiesWidgetBuilder._relationship_builder(
            stage,
            ui_prop.prop_name,
            ui_prop.metadata,
            [node_prim_path],
    ...
    def _on_graph_rel_changed(self, *args):
        ...
        self.variable_name_model._set_dirty()
        self.graph_rel_widget._set_dirty()
    ...
    def apply(self, props):
        ...
        def value_build_fn(prop: UsdPropertyUiEntry):
            def build_fn(*args):
                ...
                self.compute_node_widget.build_property_item(
                    self.compute_node_widget.stage, prop, self.compute_node_widget._payload
                )
            return build_fn

        def skel_root_path_build_fn(prop: UsdPropertyUiEntry):
            def build_fn(*args):
                # the resolved attribute is procedural and does not inherit the meta-data of the extended attribute
                # so we need to manually set the display name
                prop.override_display_name("Skel Root Path")
                self.compute_node_widget.build_property_item(
                    self.compute_node_widget.stage, prop, self.compute_node_widget._payload
                )
            return build_fn
        ...
        with frame:
            with CustomLayoutGroup("Inputs"):
                prop = find_prop("inputs:skelRootPath")
                if prop is not None:
                    CustomLayoutProperty(None, None, build_fn=skel_root_path_build_fn(prop))
