...
EXT_PATH = Path(carb.tokens.get_tokens_interface().resolve("${omni.graph.window.generic}"))


class OgGenericTestCatalogModel(GenericNodeTypeCatalogModel):
...
class OgGenericTestGraphWidget(GenericGraphWidget):
    def on_build_startup(self):
        # We don't want the initial Edit/Create selection widgets, just a blank graph.
        pass


class OgGenericTestWindow(GenericGraphWindow):
    def on_build_window(self):
        self._main_widget = OgGenericTestGraphWidget(  # noqa: protected-access
            catalog_model=OgGenericTestCatalogModel()
        )


class TestOgWindowGenericUI(OmniUiTest):
    ...
    async def __initialize_test_window(self):
        ...
        # rather than use OgGenericTestWindow (previous behavior), we now create an OmniGraphWindow object and
        # pass in a function to call during on_build_window. This way Pybind11 does not delete the parent object from its
        ...
        graph_window = OmniGraphWindow(
            "OgWindowGenericTest",
            createWidgetFunc=partial(OgGenericTestGraphWidget, catalog_model=OgGenericTestCatalogModel()),
    ...
    async def test_simple_graph(self):
        ...
        await self.finalize_test(
            ...
            golden_img_name="omni.graph.window.generic.test_simple_graph.png",
        )
    ...
    async def test_compound_nodes(self):
        ...
        await self.__wait_for_window_to_draw()
        await self.finalize_test(
            golden_img_dir=self._golden_img_dir, golden_img_name="omni.graph.window.generic.test_compound_nodes.png"
        )
    ...
    async def test_enter_compound(self):
        ...
        await self.__wait_for_window_to_draw()
        await self.finalize_test(
            golden_img_dir=self._golden_img_dir, golden_img_name="omni.graph.window.generic.test_enter_compounds.png"
        )
    ...
    async def _validate_enter_compound(self, graph_create_fn, compounds: List[str], test_name: str):
        ...
        await self.finalize_test(
            golden_img_dir=self._golden_img_dir, golden_img_name=f"omni.graph.window.generic.{test_name}.png"
        )

    @unittest.skipIf(os.name == "posix" and os.getenv("ETM_ACTIVE"), "OM-79384: Skip test in Linux ETM")
    @unittest.skipIf(not _is_kit_version_or_greater((105, 1)), "105.1 required for compound subgraphs")
    async def test_compound_no_connections(self):
        """Test entering a compound with no connections produces unconnectedinput and output virtual nodes in the image"""
