...
"""This module provides utility functions for retrieving, parsing, and extracting advanced rendering feature configurations from a Farm Queue server's settings."""
...
def get_farm_utilities_server(settings: Dict[str, Any]) -> Optional[str]:
    return (
        ADVANCED_RENDERING_FEATURES_SETTINGS_KEY in settings
...
def _has_advanced_feature(settings: Dict[str, Any], feature_key: str) -> bool:
    return (
        _supports_advanced_feature(settings=settings, feature_key=feature_key)
