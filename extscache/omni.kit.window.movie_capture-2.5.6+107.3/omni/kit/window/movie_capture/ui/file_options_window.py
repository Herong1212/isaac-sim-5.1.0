...
"""This module defines a FileOptionsWindow class that builds and manages an Omni UI window for configuring movie capture file options in various formats such as PNG, TGA, EXR, and MP4."""


import carb
import omni.ui as ui
from ..base_widget import WINDOW_DARK_STYLE, FRAME_SPACING, RIGHT_SPACING
...
class FileOptionsWindow:
    ...
    def _collect_exr_options(self):
        ...
        self.__options.set_option(
            FileOptions.OPTION_EXR_COMP_METHOD,
            FileOptions.EXR_COMP_METHODS[self._ui_exr_cm_radio_collection.model.as_int],
        )
    ...
    def _collect_mp4_options(self):
        ...
        self.__options.set_option(
            FileOptions.OPTION_MP4_ENCODING_PRESET, self._get_combobox_string_value(self._ui_mp4_preset)
        )
        self.__options.set_option(
            FileOptions.OPTION_MP4_ENCODING_PROFILE, self._get_combobox_string_value(self._ui_mp4_profile)
        )
        self.__options.set_option(
            FileOptions.OPTION_MP4_ENCODING_RC_MODE, self._get_combobox_string_value(self._ui_mp4_rc_mode)
        )
        self.__options.set_option(
            FileOptions.OPTION_MP4_ENCODING_RC_TARGET_QUALITY, self._ui_mp4_rc_target_quality.value
        )
        self.__options.set_option(
            FileOptions.OPTION_MP4_ENCODING_VIDEO_FULL_RANGE_FLAG,
            self._ui_mp4_video_full_range_flag_check.model.as_bool,
        )
    ...
    def _apply_mp4_options(self):
        self._ui_mp4_bitrate.value = self.__options.get_option(
            FileOptions.OPTION_MP4_ENCODING_BITRATE, FileOptions.DEFAULT_MP4_ENCODING_BITRATE
        )
        self._ui_mp4_iframe_interval.value = self.__options.get_option(
        ...
        self._set_combobox_string_value(
            self._ui_mp4_preset,
            self.__options.get_option(FileOptions.OPTION_MP4_ENCODING_PRESET, FileOptions.DEFAULT_MP4_ENCODING_PRESET),
        )
        self._set_combobox_string_value(
            self._ui_mp4_profile,
            self.__options.get_option(
                FileOptions.OPTION_MP4_ENCODING_PROFILE, FileOptions.DEFAULT_MP4_ENCODING_PROFILE
            ),
        )
        self._set_combobox_string_value(
            self._ui_mp4_rc_mode,
            self.__options.get_option(FileOptions.OPTION_MP4_ENCODING_RC_MODE, FileOptions.DEFAULT_MP4_ENCODING_RCMODE),
        )
        self._ui_mp4_rc_target_quality.value = self.__options.get_option(
            FileOptions.OPTION_MP4_ENCODING_RC_TARGET_QUALITY, FileOptions.DEFAULT_MP4_ENCODING_RC_TARGET_QUALITY
        )
        self._ui_mp4_video_full_range_flag_check.model.as_bool = self.__options.get_option(
            FileOptions.OPTION_MP4_ENCODING_VIDEO_FULL_RANGE_FLAG,
    ...
    def _build_ui(self):
        window_flags = (
            ui.WINDOW_FLAGS_NO_SCROLLBAR | ui.WINDOW_FLAGS_MODAL | ui.WINDOW_FLAGS_NO_RESIZE | ui.WINDOW_FLAGS_NO_CLOSE
        )
            ...
            with ui.HStack():
                ...
                with ui.VStack():
                    ...
                    with ui.HStack(style=WINDOW_DARK_STYLE):
                        self._ui_save_alpha_check = ui.CheckBox(
                            width=0, name="green_check", identifier="file_option_id_check_save_alpha"
                        )
                        ui.Label(" Save Alpha")
                    ui.Spacer()
        ...
        with self._ui_exr_options:
            ...
            with ui.HStack():
                ...
                with ui.VStack():
                    ui.Spacer()
                    with ui.HStack(style=WINDOW_DARK_STYLE):
                        self._ui_hdr_output_check = ui.CheckBox(
                            width=0, name="green_check", identifier="file_option_id_check_hdr_output"
                        )
                        ui.Label(" HDR Output")
                    ui.Spacer()
            with ui.HStack():
