"""Module provides base widget components for movie capture, including camera and animation FPS management and utilities for render settings and extension integration using Omni UI."""
...
FRAME_SPACING = 5
RIGHT_SPACING = 12
GLYPH_CODE_UP = ui.get_custom_glyph_code("${glyphs}/arrow_up.svg")
GLYPH_CODE_DOWN = ui.get_custom_glyph_code("${glyphs}/arrow_down.svg")
GLYPH_CODE_SETTINGS = ui.get_custom_glyph_code("${glyphs}/cog.svg")
GLYPH_CODE_FOLDER = ui.get_custom_glyph_code("${glyphs}/folder.svg")
GLYPH_CODE_FOLDER_OPEN = ui.get_custom_glyph_code("${glyphs}/folder_open.svg")
GLYPH_CODE_JUMP_TO = ui.get_custom_glyph_code("${glyphs}/share.svg")
GLYPH_CODE_LINK = ui.get_custom_glyph_code("${glyphs}/link.svg")
GLYPH_CODE_UNLINK = ui.get_custom_glyph_code("${glyphs}/unlink.svg")
TUNE_ARROW_SIZE = 10
...
FARM_EARLY_QUIT_TIME_LIMIT_OPTION_NAME = "early_quit_time_limit_per_frame_in_minutes"
RT_WAIT_FOR_RENDER_RESOLVE_OPTION_NAME = "rt_wait_for_render_resolve_in_seconds"
...
class CamerasModel(ui.AbstractItemModel):
    ...
    def _refresh_cameras(self, index=0):
        ...
        if carb.settings.get_settings().get_as_bool(USE_SEQUENCER_CAMERA):
            ...
            new_cameras[self._active_camera] = CamerasItem(
                ui.SimpleStringModel("Current (" + self._active_camera + ")")
            )
        else:
            new_cameras[self._active_camera] = CamerasItem(
                ui.SimpleStringModel("Current (" + self._active_camera + ")")
    ...
    def _on_sequencer_camera_synced(self):
        ...
        for index, camera_name in enumerate(cameras):
            if (sync_sequencer_camera and camera_name == SEQUENCER_CAMERA) or (
                not sync_sequencer_camera and camera_name.startswith("Current (")
            ):
...
class AnimationFpsModel(ui.AbstractItemModel):
    ...
    def _refresh_fps(self, index=0):
        ...
        if not self._track_current_fps:
            ...
            all_fps_new.append(
                AnimationFpsItem(ui.SimpleStringModel("Current (" + self._format_fps_num(self._active_fps) + ")"))
            )
            all_fps = [fps.model.as_string for fps in self._all_fps]
            index = 0
...
class BaseMovieCaptureWidget:
    ...
    def _launch_render_setting_window(self, renderer):
        rtx_settings_ext_name = "omni.rtx.window.settings"
        if not self._is_ext_enabled(rtx_settings_ext_name):
            failure = (
                f"Failed to show renderer settings window due to extension {rtx_settings_ext_name} is not enabled. "
            )
            action = "Please enable it in Extension Manager and try again."
            carb.log_warn(f"Movie Capture: {failure}{action}")
            return

        from omni.rtx.window.settings import RendererSettingsFactory

        RendererSettingsFactory.set_current_renderer(renderer)
        RendererSettingsFactory._get_render_settings_extension().show_window(True)
    ...
    def _show_iray_settings(self):
        ...
        if self._is_ext_enabled(iray_ext_name):
            self._launch_render_setting_window("Accurate (Iray)")
        else:
            failure = f"Failed to show Iray settings window due to Iray extension {iray_ext_name} is not enabled."
            action = "Please enable it in Extension Manager and try again."
            carb.log_warn(f"Movie Capture: {failure}{action}")

    def show_render_settings(self, renderer):
        if renderer.find("Iray") >= 0:
