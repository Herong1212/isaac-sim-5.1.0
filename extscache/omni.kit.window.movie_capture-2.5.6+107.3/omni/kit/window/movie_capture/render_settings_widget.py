"""This module implements UI components and supporting classes to configure render settings for movie capture using Omni UI elements integrated with Carb settings."""
...
RENDER_PRODUCT_REQUIRED_EXTS = ["omni.graph", "omni.graph.nodes", "omni.graph.examples.cpp"]
...
RENDERER_ACTIVE_PATH = "/renderer/active"

if not hasattr(omni.kit.capture.viewport.CaptureRenderPreset, "REAL_TIME_PATHTRACING"):
    omni.kit.capture.viewport.CaptureRenderPreset.REAL_TIME_PATHTRACING = 3


@dataclass
class RenderInfo:
    """
    Informaiton of a specific render
    """

    engine: str  # Name of render engine
    mode: str  # Render mode
    display_name: str  # Display name
    capture_render_preset: int  # Render preset defined in omni.kit.capture.viewport
    render_setting_page_title: str  # Page title shown in the "Render Settings" window


class RenderItem(ui.AbstractItem):
    def __init__(self, render_info: Optional[RenderInfo] = None, actived: bool = False):
        super().__init__()
    ...
    def display_name(self):
        return self.render_info.display_name if not self.actived else f"{self.render_info.display_name} (Current)"


class ActiveRenderItem(RenderItem):
    def __init__(self, render_info: Optional[RenderInfo] = None):
        super().__init__(render_info, True)

    @property
    def display_name(self):
        return f"Use Current ({self.render_info.display_name})" if self.render_info else ""


class RenderModel(ui.AbstractItemModel):
    def __init__(self, capture_instance, on_render_selection_changed_fn: Callable[[RenderInfo], None]):
        super().__init__()
        self._capture_instance = capture_instance
        self._on_render_selection_changed_fn = on_render_selection_changed_fn
        self._settings = carb.settings.get_settings()

        self._renders: List[RenderInfo] = []
        for hd_engine in self.enabled_engines:
            # Only support rtx and iray for now
            if hd_engine == "rtx":
                rtEnabled = self._settings.get("/rtx-transient/rtEnabled")
                if rtEnabled is None:
                    rtEnabled = True
                ptEnabled = self._settings.get("/rtx-transient/ptEnabled")
                if ptEnabled is None:
                    ptEnabled = True
                rt2Enabled = self._settings.get("/rtx-transient/rt2Enabled")
                if rt2Enabled is None:
                    rt2Enabled = self._settings.get("/rtx-transient/pathtracing/rtptEnabled")
                if ptEnabled:
                ...
                if rt2Enabled:
                    self._renders.append(
                        RenderInfo(
                            ...
                            "RealTimePathTracing",
                            "RTX - Real-Time 2.0",
                            omni.kit.capture.viewport.CaptureRenderPreset.REAL_TIME_PATHTRACING,
                            "Real-Time 2.0 (Preview)",
                        )
                    )
            elif hd_engine == "iray":
                self._renders.append(
                    RenderInfo(
                        ...
                        "iray",
                        "RTX - Accurate (Iray)",
                        omni.kit.capture.viewport.CaptureRenderPreset.IRAY,
                        "Accurate (Iray)",
                    )
                )

        # Active render which is current renderer in viewport
        self._active_render = self._get_active_render()

        # Render items to be shown, including a "Using current" item (may hide if active render is not in the available renders) and a list of available renders
        self._render_items: List[RenderItem] = []
        self._render_items.append(ActiveRenderItem(self._active_render))
        for render in self._renders:
            self._render_items.append(RenderItem(render, actived=render == self._active_render))

        # selected render which is the selected item in the combo box
        default_render_mode = self._settings.get(DEFAULT_RENDER_PRESET_SETTING_PATH)
        if default_render_mode:
            self._selected_render = self._get_render_from_mode(default_render_mode) or self._active_render
        else:
            self._selected_render = self._active_render

        # The current index of the combo box to match the selected render
        selected_index = 0
        selected_item = None
        for index, item in enumerate(self.get_item_children()):
            if item.render_info == self._selected_render:
                selected_index = index
                selected_item = item
                break
        self._use_current = isinstance(selected_item, ActiveRenderItem)
        self._selection_model = ui.SimpleIntModel(selected_index)
        self._selection_model.add_value_changed_fn(self._on_selection_changed)

        # Subscribe to settings changes
        self._rtx_mode_change_sub = self._settings.subscribe_to_node_change_events(
            RTX_RENDER_MODE_PATH, self._on_rtx_mode_changed
        )
        self._render_engine_change_sub = self._settings.subscribe_to_node_change_events(
            RENDERER_ACTIVE_PATH, self._on_render_engine_changed
        )
    ...
    def enabled_engines(self):
        engines = self._settings.get("/renderer/enabled")
        # Only support rtx and iray for now
        supported_engines = ("rtx", "iray")
        if engines:
            engines = [engine for engine in engines.split(",") if engine and engine in supported_engines]
            return engines
        else:
            return ("rtx", "iray")
    ...
    def selected_index(self) -> int:
        return self._selection_model.as_int

    @selected_index.setter
    def selected_index(self, value: int) -> None:
        if value != self.selected_index:
            self._selection_model.set_value(value)

    @property
    def selected_render(self) -> Optional[RenderInfo]:
        return self._selected_render

    @selected_render.setter
    def selected_render(self, render_info: RenderInfo):
        if render_info == self._selected_render:
            return
        self._selected_render = render_info
        if self._on_render_selection_changed_fn:
            self._on_render_selection_changed_fn(render_info)

    def destroy(self):
        self._settings.unsubscribe_to_change_events(self._rtx_mode_change_sub)
        self._settings.unsubscribe_to_change_events(self._render_engine_change_sub)
        self._settings.unsubscribe_to_change_events(self._render_preset_setting_changed_sub)

    def get_item_children(self, item: Optional[RenderItem] = None) -> List[RenderItem]:
        if item is None:
            if self._render_items[0].render_info:
                return self._render_items
            else:
                return self._render_items[1:]
        return []

    def get_item_value_model(
        self, item: Optional[RenderItem] = None, index: int = 0
        ...
        return ui.SimpleStringModel(item.display_name)
    ...
    def set_render_mode(self, mode: str) -> bool:
        render_info = self._get_render_from_mode(mode)
        if render_info:
            self.selected_render = render_info
            self.selected_index = self._get_selection_index(use_current=False)
            self._item_changed(None)
            return True

        carb.log_warn(f"Movie Capture can't find render mode {mode} in the render preset list.")
        return False

    def _get_render_from_mode(self, mode: str) -> Optional[RenderInfo]:
        for render in self._renders:
            if render.mode == mode:
                return render
        return None

    def _set_active_render(self, render: Optional[RenderInfo]):
        if render == self._active_render:
            return
        self._active_render = render
        self._render_items[0].render_info = render
        for item in self._render_items[1:]:
            item.actived = item.render_info == render

        self.selected_index = self._get_selection_index(use_current=self._use_current)
        self._item_changed(None)

    def _get_selection_index(self, use_current: bool) -> int:
        if self._active_render is None:
            # Active render is None, keep selected render no change but update selection index
            for index, item in enumerate(self.get_item_children(None)):
                if item.render_info == self._selected_render:
                    return index
        else:
            # Active render is not None,
            # if using current, set selection to use current but change selected render
            # otherwise, keep selected render no change but update selection index
            if use_current:
                self.selected_render = self._active_render
                return 0
            else:
                for index, item in enumerate(self.get_item_children()[1:]):
                    if item.render_info == self._selected_render:
                        return index + 1

    def _on_selection_changed(self, _):
        selected_index = self.selected_index
        selected_item = self.get_item_children()[selected_index]
        self._use_current = isinstance(selected_item, ActiveRenderItem)
        self.selected_render = selected_item.render_info

        self._item_changed(None)

    def _on_rtx_mode_changed(self, item, event_type):
        active_render = self._get_active_render()
        if active_render and active_render.engine != "rtx":
            return
        if self._is_capturing():
            return

        self._set_active_render(active_render)

    def _on_render_engine_changed(self, item, event_type):
        if self._is_capturing():
            return

        self._set_active_render(self._get_active_render())

    def _on_render_preset_setting_changed(self, item, event_type):
        render_plug_in = self._settings.get(DEFAULT_RENDER_PRESET_SETTING_PATH)
        self.set_render_mode(render_plug_in)

    def _is_capturing(self):
        return self._capture_instance.progress.capture_status != omni.kit.capture.viewport.CaptureStatus.NONE

    def _get_active_render(self) -> Optional[RenderInfo]:
        # Get the "active" engine and mode from the Viewport first
        engine, mode, vp = None, None, get_active_viewport()
        if vp:
            engine, mode = vp.hydra_engine, vp.render_mode
        # Otherwise fallback to global settings
        if not engine:
            engine = self._settings.get(RENDERER_ACTIVE_PATH)
        if not mode:
            mode = "iray" if engine == "iray" else self._settings.get(RTX_RENDER_MODE_PATH)
        for render in self._renders:
            if render.engine == engine and render.mode == mode:
                return render
        return None
...
class RenderSettingsWidget(base_widget.BaseMovieCaptureWidget):
    def __init__(self, capture_instance):
        ...
        self._iray_last_subframes_per_frame_value = self._settings.get_as_int(
            DEFAULT_IRAY_SUBFRAMES_PER_FRAME_SETTING_PATH
        )
        self._pt_last_subframes_per_frame_value = self._settings.get_as_int(DEFAULT_SUBFRAME_PER_FRAME_SETTING_PATH)
        self._render_model: Optional[RenderModel] = None

    def build_ui(self):
        self._build_ui_rendering_settings()

        self._set_default_settings()
        self._on_render_changed(self._render_model.selected_render)

    def destroy(self):
        self._capture_instance = None
        if self._render_model:
            self._render_model.destroy()
        ...
        with self._ui_render_product_area:
            ...
            with ui.HStack(style=base_widget.WINDOW_DARK_STYLE):
                ...
                with ui.VStack(width=0):
                    ui.Spacer()
                    self._ui_use_render_product_check = ui.CheckBox(
                        height=0, name="green_check", identifier="render_setting_id_check_use_render_product"
                    )
            ...
            with self._ui_render_product_input_area:
                ...
                with ui.HStack(spacing=base_widget.FRAME_SPACING, style=base_widget.WINDOW_DARK_STYLE):
                    ...
                    self._ui_render_product = ui.ComboBox(
                        0, "", "/renderview", identifier="render_setting_id_combo_render_product"
                    )
    ...
    def _build_ui_rendering_rtpt_settings(self):
        with ui.VStack(spacing=0):
            ...
            with ui.HStack(spacing=base_widget.FRAME_SPACING, style=base_widget.WINDOW_DARK_STYLE):
                ...
                self._render_model = RenderModel(self._capture_instance, self._on_render_changed)
    ...
    def _build_ui_rendering_realtime_settings(self):
        ...
        with self._ui_kit_realtime_settings_area:
            with ui.HStack():
                self._build_ui_left_column(
                    "Time to wait for render resolve (in seconds)", base_widget.LEFT_COLUMN_WIDTH_IN_PERCENT_WIDE
                )
                tooltip_msg = (
                    "Time in seconds to wait for the render effects, like temporal, to resolve for each frame."
                )
                self._ui_wait_for_render_resolve_time_input = QuickNumberInput(
                    input_type=QuickNumberInputType.INT,
                    init_value=0,
                    step=1,
                    min_value=0,
                    tooltip=tooltip_msg,
                    identifier="render_setting_id_wait_for_render_resolve_input",
                )
                ui.Spacer(width=base_widget.RIGHT_SPACING)

    def _build_ui_rendering_common_settings(self):
        self._ui_kit_common_render_settings_area = ui.VStack(visible=True)
        with self._ui_kit_common_render_settings_area:
            with ui.HStack():
                self._build_ui_left_column("Settle latency", base_widget.LEFT_COLUMN_WIDTH_IN_PERCENT_WIDE)
    ...
    def _build_ui_rendering_iray_settings(self):
        self._ui_kit_iray_settings_area = ui.VStack(visible=False)
    ...
    def _build_ui_rendering_ptmb_settings(self):
        ...
        with self._ui_kit_pathtrace_motionblur_settings_area:
            with ui.HStack(style=base_widget.WINDOW_DARK_STYLE):
                ...
                with ui.VStack(width=0):
                    ui.Spacer()
                    self._ui_kit_motion_blur_check = ui.CheckBox(
                        height=0, name="green_check", identifier="render_setting_id_check_motion_blur"
                    )
            ...
            with self._ui_kit_pathtrace_motionblur_values_area:
                ...
                with ui.HStack():
                    self._build_ui_left_column(
                        "        Subframes per frame", base_widget.LEFT_COLUMN_WIDTH_IN_PERCENT_WIDE
                    )
    ...
    def _get_selected_render_preset(self):
        return self._render_model.selected_render.capture_render_preset if self._render_model.selected_render else None
    ...
    def _on_render_product_ext_yes_clicked(self, dialog):
        ...
        for ext in self._render_product_required_exts:
            ...
            if enabled:
                carb.log_info(f"Movie Capture: {ext} enabled for render product capture.")
            else:
                exts_enable_failed += f"\r\n {ext}"
                carb.log_warn(
                    f"Movie Capture: Failed to enable {ext} for render product capture. Capture will not be done with render product."
                )
        dialog.hide()
        ...
        # Here is callback function of previous dialog, so we need to center the window manually.
        enable_result_dialog.window.position_x = (
            ui.Workspace.get_main_window_width() - enable_result_dialog.window.width
        ) / 2
        enable_result_dialog.window.position_y = (
            ui.Workspace.get_main_window_height() - enable_result_dialog.window.height
        ) / 2
    ...
    def _update_subframe_default_value(self, render_preset):
        if render_preset == omni.kit.capture.viewport.CaptureRenderPreset.PATH_TRACE:
            ...
            self._apply_int_setting(
                "subframe per frame", self._pt_last_subframes_per_frame_value, 1, self._ui_ptmb_spf_input
            )
            ...
            self._ui_ptmb_spf_input.value_changed_fn = None
            self._apply_int_setting(
                "subframe per frame", self._iray_last_subframes_per_frame_value, 1, self._ui_ptmb_spf_input
            )
            self._ui_ptmb_spf_input.value_changed_fn = self._on_ptmb_spf_input_changed
        else:
            return

    def _on_render_preset_selection_changed(self, render_info: RenderInfo):
        render_preset = render_info.capture_render_preset if render_info else None
        self._set_pathtrace_settings_visibility(
        ...
        self._set_realtime_settings_visibility(
            render_preset
            in [
                omni.kit.capture.viewport.CaptureRenderPreset.RAY_TRACE,
                omni.kit.capture.viewport.CaptureRenderPreset.REAL_TIME_PATHTRACING,
        ...
        self._set_pathtrace_motionblur_visibility(
            render_preset
            in [
                omni.kit.capture.viewport.CaptureRenderPreset.IRAY,
                omni.kit.capture.viewport.CaptureRenderPreset.PATH_TRACE,
    ...
    def _set_pathtrace_settings_visibility(self, visible):
        ...
        self._ui_kit_pathtrace_motionblur_settings_area.visible = (
            visible and self._ui_kit_motion_blur_check.model.as_bool
        )
    ...
    def _apply_frame_shutter_open_setting(self, value):
        if value < -1.0:
            ...
            carb.log_warn(
                f"Movie capture's frame shutter open value should be no greater than the close value. Set to {self._ui_ptmb_fsc_input.value}."
            )
    ...
    def _apply_frame_shutter_close_setting(self, value):
        if value < self._ui_ptmb_fso_input.value:
            carb.log_warn(
                f"Movie capture's frame shutter close value should be no less than the open value. Set to {self._ui_ptmb_fso_input.value}."
            )
    ...
    def _on_render_changed(self, render_info: RenderInfo):
        self._on_render_preset_selection_changed(render_info)

    def _on_render_preset_settings_clicked(self):
        page_title = (
            self._render_model.selected_render.render_setting_page_title if self._render_model.selected_render else ""
        )
        if page_title:
            self.show_render_settings(page_title)

    def _on_render_preset_refresh_clicked(self):
        self._refresh_render_products()

    def collect_settings(self, options: omni.kit.capture.viewport.capture_options.CaptureOptions) -> bool:
        render_preset = self._get_selected_render_preset()
        if render_preset is None:
            carb.log_error("Movie capture: no render preset selected!")
            return False
        ...
        if self._ui_render_product_area.visible is True and self._ui_use_render_product_check.model.as_bool is True:
            ...
            if len(rp_string) > 0:
                ...
                carb.log_warn(
                    "Movie Capture: render product capture is enabled but no render product is selected so will not do render product capture."
                )
        ...
        if options.render_preset == omni.kit.capture.viewport.CaptureRenderPreset.PATH_TRACE:
            ...
            options.ptmb_fso = 0
            options.ptmb_fsc = 1
            # only pass the wait for render resolve value to capture option if it's supported
            if hasattr(options, base_widget.RT_WAIT_FOR_RENDER_RESOLVE_OPTION_NAME):
                options.rt_wait_for_render_resolve_in_seconds = self._ui_wait_for_render_resolve_time_input.value
            else:
                carb.log_warn(
                    "The Time to wait for render resolve option is not supported in current omni.kit.capture.viewport extention."
                    "\nYou may want to upgrade it to v1.5.0 to support this option."
                )
    ...
    def get_ui_values(self, ui_values: UIValuesStorage):
        ...
        ui_values.set(UIValuesStorage.SETTING_NAME_RENDER_PRESET, self._render_model.selected_render.mode)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_REALTIME_WAIT_FOR_RENDER_RESOLVE,
            self._ui_wait_for_render_resolve_time_input.value,
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_REALTIME_SETTLE_LATENCY, self._ui_common_settle_latency_input.value)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_PATHTRACE_SPP_PER_ITERATION_MGPU, self._ui_spp_per_iteration_input.value
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_PATHTRACE_SPP_PER_SUBFRAME, self._ui_spp_input.value)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_PATHTRACE_ENABLE_MB_CHECKED, self._ui_kit_motion_blur_check.model.as_bool
        )
    ...
    def apply_ui_values(self, ui_values: UIValuesStorage):
        ...
        self._ui_wait_for_render_resolve_time_input.value = ui_values.get(
            UIValuesStorage.SETTING_NAME_REALTIME_WAIT_FOR_RENDER_RESOLVE, 0
        )
        self._ui_common_settle_latency_input.value = ui_values.get(UIValuesStorage.SETTING_NAME_REALTIME_SETTLE_LATENCY)
        self._ui_spp_per_iteration_input.value = ui_values.get(
            UIValuesStorage.SETTING_NAME_PATHTRACE_SPP_PER_ITERATION_MGPU
        )
        self._ui_spp_input.value = ui_values.get(UIValuesStorage.SETTING_NAME_PATHTRACE_SPP_PER_SUBFRAME)
        self._ui_kit_motion_blur_check.model.as_bool = ui_values.get(
            UIValuesStorage.SETTING_NAME_PATHTRACE_ENABLE_MB_CHECKED
        )
        ...
        self._render_model.set_render_mode(ui_values.get(UIValuesStorage.SETTING_NAME_RENDER_PRESET))

        # Force update the subframe default value for Iray and PT
        if self._render_model.selected_render:
            self._update_subframe_default_value(self._render_model.selected_render.capture_render_preset)
