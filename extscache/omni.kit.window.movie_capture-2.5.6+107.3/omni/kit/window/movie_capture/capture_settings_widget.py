"""This module defines a CaptureSettingsWidget class that builds and manages an Omni UI interface for configuring movie capture settings, including movie type, capture range, resolution options, and sunstudy capture parameters."""
...
CAPTURE_RANGE_TYPES = (
    omni.kit.capture.viewport.CaptureRangeType.FRAMES,
    omni.kit.capture.viewport.CaptureRangeType.SECONDS,
)
...
class CaptureSettingsWidget(base_widget.BaseMovieCaptureWidget):
    def __init__(self):
        super(CaptureSettingsWidget, self).__init__()
    ...
    def _build_ui_capture_settings(self):
        with ui.CollapsableFrame("Capture Settings", height=0):
            with ui.VStack(spacing=base_widget.FRAME_SPACING, height=0):
                self._build_ui_movie_type_settings()
                with ui.HStack():
                    self._build_ui_left_column("Camera")
                    self._camera_combo_model = base_widget.CamerasModel()
                    self._ui_kit_combobox_camera_type = ui.ComboBox(
                        self._camera_combo_model, identifier="cap_setting_id_combo_camera_type"
                    )
    ...
    def _build_ui_app_level_capture_settings(self):
        with ui.HStack(style=base_widget.WINDOW_DARK_STYLE):
            self._build_ui_left_column("Capture Application")
            with ui.HStack(
                width=0,
                tooltip="To capture scene elements, it will show the viewport only and hide other UI parts during capture, e.g. title and menu bars.",
            ):
                with ui.VStack(width=0):
                    ui.Spacer()
                    self._ui_kit_capture_app_check = ui.CheckBox(
                        height=0, name="green_check", identifier="cap_setting_id_check_capture_app"
                    )
                    ui.Spacer()
    ...
    def _build_ui_capture_range(self):
        with ui.HStack():
            self._build_ui_left_column("Frame Rate")
            self._animation_fps_model = base_widget.AnimationFpsModel()
            self._ui_kit_combobox_fps = ui.ComboBox(
                self._animation_fps_model, width=ui.Percent(35), identifier="cap_setting_id_combo_fps"
            )
            # self._set_default_fps()
    ...
    def _build_ui_movie_type_settings(self):
        with ui.VStack(spacing=base_widget.FRAME_SPACING, height=0):
            with ui.HStack():
                self._build_ui_left_column("Movie Type")
                self._ui_kit_combobox_movie_type = ui.ComboBox(
                    0, "Sequence", "Sunstudy", identifier="cap_setting_id_combo_movie_type"
                )
                self._movie_type_changed_fn = self._ui_kit_combobox_movie_type.model.add_item_changed_fn(
                    self._on_movie_type_changed
                )
                self._refresh_list()
                ui.Spacer(width=base_widget.RIGHT_SPACING)
            ui.Spacer()
    ...
    def _build_ui_capture_range_sequence(self):
        with self._ui_capture_range_sequence:
            with ui.HStack():
                self._build_ui_left_column("Capture Range")
                self._ui_kit_combobox_range = ui.ComboBox(
                    0, "Custom Range - Frames", "Custom Range - Seconds", identifier="cap_setting_id_combo_range"
                )
                self._ui_kit_combobox_range.model.add_item_changed_fn(self._on_kit_capture_range_changed)
                ui.Spacer(width=base_widget.RIGHT_SPACING)
            ui.Spacer(height=base_widget.FRAME_SPACING)
            self._ui_kit_capture_range_input_frames = ui.HStack(style=base_widget.WINDOW_DARK_STYLE)
            self._buid_ui_frames_input()
            self._ui_kit_capture_range_input_seconds = ui.HStack(style=base_widget.WINDOW_DARK_STYLE)
    ...
    def _build_ui_renumber_negative_frames(self):
        with ui.HStack(style=base_widget.WINDOW_DARK_STYLE):
            ...
            with ui.HStack(width=ui.Percent(50)):
                with ui.VStack(width=0):
                    ui.Spacer()
                    self._ui_kit_renumber_negative_frames_check = ui.CheckBox(
                        height=0, name="green_check", identifier="cap_setting_id_checkbox_renumber_negative_frames"
                    )
                    ui.Spacer()
    ...
    def _build_ui_preroll_frames(self):
        with ui.HStack(style=base_widget.WINDOW_DARK_STYLE):
            ...
            with ui.HStack(width=ui.Percent(50)):
                with ui.VStack(width=0):
                    ui.Spacer()
                    self._ui_kit_preroll_frames_check = ui.CheckBox(
                        height=0, name="green_check", identifier="cap_setting_id_check_preroll_frames"
                    )
    ...
    def _build_ui_capture_nth_frames(self):
        with ui.HStack(style=base_widget.WINDOW_DARK_STYLE):
            ...
            with ui.HStack(width=ui.Percent(50)):
                with ui.VStack(width=0):
                    ui.Spacer()
                    self._ui_kit_nth_frames_check = ui.CheckBox(
                        height=0, name="green_check", identifier="cap_setting_id_check_nth_frame"
                    )
    ...
    def _build_ui_capture_resolution_settings(self):
        with ui.HStack():
            self._build_ui_left_column("Resolution")
            self._ui_kit_combobox_res_type = ui.ComboBox(1, "HD", "Custom", identifier="cap_setting_id_combo_res_type")
            self._is_res_hd = False
        ...
        with ui.VStack(spacing=0):
            ...
            with ui.HStack():
                self._build_ui_left_column("")
                with ui.HStack():
                    ui.Label("Width", alignment=ui.Alignment.CENTER_BOTTOM, width=ui.Percent(33))
                    ui.Spacer(width=ui.Percent(5))
                    ui.Label("Height", alignment=ui.Alignment.CENTER_BOTTOM, width=ui.Percent(33))
                    ui.Spacer()
            with ui.HStack():
                self._build_ui_left_column("")
                with ui.HStack(style=base_widget.WINDOW_DARK_STYLE):
                    ...
                    with ui.VStack():
                        ui.Spacer(height=5)
                        self._ui_kit_res_ratio = ui.ComboBox(
                            0, "16:9", "4:3", identifier="cap_setting_id_combo_res_ratio"
                        )
                        self._res_ratio_changed_fn = self._ui_kit_res_ratio.model.add_item_changed_fn(
                            self._on_res_ratio_changed
                        )
                    ui.Spacer(width=base_widget.RIGHT_SPACING)
    ...
    def _get_sunstudy(self):
        if self._is_ext_enabled(SUNSTUDY_EXT_NAME):
            try:
                from omni.kit.environment.core import SunstudyTimeSlider, get_sunstudy_player
    ...
    def _get_selected_movie_type(self):
        ...
        carb.log_info(f"movie type changed to {selected_value}")
    ...
    def _kit_update_time_range(self):
        if (
            self._ui_kit_combobox_range.model.get_item_value_model().as_int
            == omni.kit.capture.viewport.CaptureRangeType.SECONDS
        ):
    ...
    def _on_res_width_input_changed(self, model):
        ...
        if self._res_linked:
            ...
            if aspect_ration == "16:9":
                ...
                self._ui_res_height_input.value = int(width * self._custom_res_height / self._custom_res_width)
    ...
    def _on_res_height_input_changed(self, model):
        ...
        if self._res_linked:
            ...
            if aspect_ration == "16:9":
                ...
                self._ui_res_width_input.value = int(height * self._custom_res_width / self._custom_res_height)
    ...
    def _refresh_res_ratio_options(self):
        ...
        self._custom_res_width = self._ui_res_width_input.value
        self._custom_res_height = self._ui_res_height_input.value
        if self._custom_res_width >= self._custom_res_height:
            r = self._custom_res_width / self._custom_res_height
            s = "{:.3f}:1".format(r)
        else:
            r = self._custom_res_height / self._custom_res_width
            s = "1:{:.3f}".format(r)
        global CAPTURE_RES_RATIO
    ...
    def _read_sequence_capture_settings(self, options: omni.kit.capture.viewport.capture_options.CaptureOptions):
        ...
        if options.range_type == omni.kit.capture.viewport.CaptureRangeType.FRAMES:
            ...
            if options.start_frame < 0:
                options.renumber_negative_frame_number_from_0 = (
                ...
                if self._ui_kit_renumber_negative_frames_check.model.as_bool:
                    carb.log_warn(
                        "Movie capture will only renumber negative frame numbers. This renumber option will be ignored."
                    )
        else:
    ...
    def get_ui_values(self, ui_values: UIValuesStorage):
        ui_values.set(
            UIValuesStorage.SETTING_NAME_MOVIE_TYPE, self._get_combobox_string_value(self._ui_kit_combobox_movie_type)
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_CAMERA_NAME, self._camera_combo_model.get_current_camera())
        # set both movie encoding fps and animation fps to the same value for now, until we make decision on
        # whether to allow a differenct movie encoding fps or not.
        ui_values.set(UIValuesStorage.SETTING_NAME_FPS, self._animation_fps_model.get_current_fps())
        ui_values.set(UIValuesStorage.SETTING_NAME_ANIMATION_FPS, self._animation_fps_model.get_current_fps())
        ui_values.set(
            UIValuesStorage.SETTING_NAME_CAPTURE_RANGE, self._get_combobox_string_value(self._ui_kit_combobox_range)
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_CAPTURE_FRAME_START, self._ui_start_frame_input.value)
        ui_values.set(UIValuesStorage.SETTING_NAME_CAPTURE_FRAME_END, self._ui_end_frame_input.value)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_CAPTURE_RANGE, self._get_combobox_string_value(self._ui_kit_combobox_range)
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_CAPTURE_TIME_START, self._ui_start_time_input.value)
        ui_values.set(UIValuesStorage.SETTING_NAME_CAPTURE_TIME_END, self._ui_end_time_input.value)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_RENUMBER_NEGATIVE_FRAMES_CHECKED,
            self._ui_kit_renumber_negative_frames_check.model.as_bool,
        )
        ui_values.set(
            UIValuesStorage.SETTING_NAME_RUN_N_FRAMES_CHECKED, self._ui_kit_preroll_frames_check.model.as_bool
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_RUN_N_FRAMES, self._ui_nth_frame_input.value)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_CAPTURE_EVERY_NTH_FRAMES_CHECKED, self._ui_kit_nth_frames_check.model.as_bool
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_CAPTURE_EVERY_NTH_FRAMES, self._ui_nth_frame_input.value)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_RESOLUTION_TYPE,
            self._get_combobox_string_value(self._ui_kit_combobox_res_type),
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_RESOLUTION_WIDTH, self._ui_res_width_input.value)
        ui_values.set(UIValuesStorage.SETTING_NAME_RESOLUTION_HEIGHT, self._ui_res_height_input.value)
        ui_values.set(UIValuesStorage.SETTING_NAME_RESOLUTION_W_H_LINKED, self._res_linked)
        ui_values.set(
            UIValuesStorage.SETTING_NAME_ASPECT_RATIOS, self._get_combobox_all_string_values(self._ui_kit_res_ratio)
        )
        ui_values.set(
            UIValuesStorage.SETTING_NAME_ASPECT_RATIO_SELECTED,
            self._ui_kit_res_ratio.model.get_item_value_model().as_int,
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_CAPTURE_APPLICATION, self._ui_kit_capture_app_check.model.as_bool)
        ...
        if self._sunstudy_widgets_built:
            ...
            ui_values.set(
                UIValuesStorage.SETTING_NAME_SUNSTUDY_MOVIE_MINUTES, self._ui_sunstudy_movie_length_minutes_input.value
            )
            ui_values.set(
                UIValuesStorage.SETTING_NAME_SUNSTUDY_MOVIE_SECONDS, self._ui_sunstudy_movie_length_seconds_input.value
            )
        else:
    ...
    def apply_ui_values(self, ui_values: UIValuesStorage):
        self._set_combobox_string_value(
            self._ui_kit_combobox_movie_type, ui_values.get(UIValuesStorage.SETTING_NAME_MOVIE_TYPE)
        )
        self._camera_combo_model.set_current_camera(ui_values.get(UIValuesStorage.SETTING_NAME_CAMERA_NAME))
        ...
        self._set_combobox_string_value(
            self._ui_kit_combobox_range, ui_values.get(UIValuesStorage.SETTING_NAME_CAPTURE_RANGE)
        )
        ...
        self._ui_kit_renumber_negative_frames_check.model.as_bool = ui_values.get(
            UIValuesStorage.SETTING_NAME_RENUMBER_NEGATIVE_FRAMES_CHECKED
        )
        self._ui_kit_preroll_frames_check.model.as_bool = ui_values.get(
        ...
        self._ui_kit_nth_frames_check.model.as_bool = ui_values.get(
        ...
        self._res_ratio_changed_fn = model.add_item_changed_fn(self._on_res_ratio_changed)
        self._set_combobox_string_value(
            self._ui_kit_combobox_res_type, ui_values.get(UIValuesStorage.SETTING_NAME_RESOLUTION_TYPE)
        )

        self._ui_kit_capture_app_check.model.as_bool = ui_values.get(
            UIValuesStorage.SETTING_NAME_CAPTURE_APPLICATION, False
        )
        ...
        if self._sunstudy_widgets_built:
            try:
                ...
                self._ui_sunstudy_movie_length_minutes_input.value = ui_values.get(
                    UIValuesStorage.SETTING_NAME_SUNSTUDY_MOVIE_MINUTES
                )
                self._ui_sunstudy_movie_length_seconds_input.value = ui_values.get(
                    UIValuesStorage.SETTING_NAME_SUNSTUDY_MOVIE_SECONDS
                )
            except Exception as e:
                carb.log_warn(f"{e}")
