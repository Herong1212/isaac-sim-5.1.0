...
"""This module implements an Omni UI widget to manage and configure movie capture settings for render farms, including queue selection, job type configuration, advanced rendering feature toggles, and asynchronous updates."""
...
from .base_widget import (
    BaseMovieCaptureWidget,
    LEFT_COLUMN_WIDTH_IN_PERCENT_WIDE,
    FARM_EARLY_QUIT_TIME_LIMIT_OPTION_NAME,
)
...
from .utils.farm_queue_utils import (
    ...
    get_ovc_available_farm_queues,
    get_ui_generate_shader_cache,
    get_ui_should_upload,
    get_ui_skip_upload_to_s3,
    supports_ui_generate_shader_cache,
    supports_ui_should_upload,
    supports_ui_skip_upload_to_s3,
    supports_ui_task_extensions,
    supports_ui_task_registries,
)
...
class StringModel(ui.SimpleStringModel):
    ...
    def end_edit(self):
        ...
        all_options = [combo_model.get_item_value_model(child).as_string for child in combo_model.get_item_children()]
        ...
        if fieldString:
            if fieldString in all_options:
                ...
                combo_model.append_child_item(None, ui.SimpleStringModel(fieldString))
...
class FarmSettingsWidget(BaseMovieCaptureWidget):
    def __init__(self):
        super(FarmSettingsWidget, self).__init__()
        self._farms = self._settings.get("exts/omni.kit.window.movie_capture/available_farms")
        self._default_job_type = (
            self._settings.get_as_string("exts/omni.kit.window.movie_capture/farm_job_name") or "create-render"
        )
        self._autoload_prefix = (
            self._settings.get_as_string("exts/omni.kit.window.movie_capture/farm_autoload_job_type_prefix")
            or "create-render"
        )
    ...
    async def handle_advanced_rendering_features(self) -> None:
        ...
        self._ui_should_upload.model.set_value(
            get_ui_should_upload(settings=queue_settings) and self._stored_upload_to_s3_value
        )
        self._ui_skip_upload.model.set_value(
            get_ui_skip_upload_to_s3(settings=queue_settings) and self._stored_skip_uploat_to_s3_value
        )
        self._ui_generate_shader_cache.model.set_value(
            get_ui_generate_shader_cache(settings=queue_settings) and self._stored_generate_shader_cache_value
        )
        ...
        with ui.CollapsableFrame("Queue settings", height=0, collapsed=True):
            with ui.HStack(height=0):
                with ui.VStack(spacing=FRAME_SPACING):
                    ...
                    self._build_ui_early_quit_time_limit()
    ...
    def _build_ui_farm_location(self):
        ...
        with ui.HStack():
            ...
            with ui.HStack(spacing=0):
                self._ui_stringfield_farm = ui.StringField(
                    self._farm_field_model, height=22, identifier="farm_setting_id_stringfield_farm"
                )
                self._ui_kit_farm_location = ui.ComboBox(
                    self._default_farm_index,
                    *self._farms.keys(),
                    width=0,
                    arrow_only=True,
                    identifier="farm_setting_id_combo_farm_location",
                )
                self._ui_kit_farm_location.model.add_item_changed_fn(self.farm_location_combo_changed)
                self._farm_field_model.populate(
                    self._ui_kit_farm_location, list(self._farms.keys())[self._default_farm_index]
                )
    ...
    async def _populate_create_render_task_types(self) -> None:
        ...
        for job_name in await self._get_job_templates():
            ...
            if job_name != self._default_job_type:
                combo_model.append_child_item(None, ui.SimpleStringModel(job_name))
    ...
    def _build_ui_farm_job_type(self):
        ...
        with ui.HStack():
            ...
            with ui.HStack(spacing=0):
                self._ui_stringfield_task_type = ui.StringField(
                    self._task_type_field_model, height=22, identifier="farm_setting_id_stringfield_task_type"
                )
                self._ui_kit_farm_task_type = ui.ComboBox(
                    self._default_farm_index,
                    *[self._default_job_type],
                    width=0,
                    arrow_only=True,
                    identifier="farm_setting_id_combo_farm_task_type",
                )
    ...
    def _build_ui_texture_streaming_memory_budget(self):
        with ui.HStack():
            self._build_ui_left_column(
                "Texture streaming memory budget (*experimental, Farm only)", LEFT_COLUMN_WIDTH_IN_PERCENT_WIDE
            )
    ...
    def _build_ui_early_quit_time_limit(self):
        with ui.HStack():
            self._build_ui_left_column("Per-frame time limit (in minutes)", LEFT_COLUMN_WIDTH_IN_PERCENT_WIDE)
            self._ui_early_quit_time_limit = QuickNumberInput(
                input_type=QuickNumberInputType.INT,
                init_value=0,
                step=1,
                min_value=0,
                identifier="farm_setting_id_drag_early_quit_time_limit_input",
                tooltip="If set above 0, then Movie Capture will fail the render job when the time spent capturing a frame exceeds this limit.",
            )
        ...
        with self._ui_stack_task_extensions:
            ...
            with ui.HStack(spacing=FRAME_SPACING):
                ...
                to_enable = [
                    ...
                    if all(keyword not in extension for keyword in ["gtc", "farm"])
                ]
    ...
    def get_early_quit_time_limit(self) -> int:
        return self._ui_early_quit_time_limit.value
    ...
    def collect_settings(self, options: omni.kit.capture.viewport.capture_options.CaptureOptions):
        # only pass the early quit time limit value to capture option if it's supported
        if hasattr(options, FARM_EARLY_QUIT_TIME_LIMIT_OPTION_NAME):
            options.early_quit_time_limit_per_frame_in_minutes = self._ui_early_quit_time_limit.value
        else:
            carb.log_warn(
                "The Per-frame time limit (in minutes) option is not supported in current omni.kit.capture.viewport extention."
    ...
    def get_ui_values(self, ui_values: UIValuesStorage):
        ...
        ui_values.set(
            UIValuesStorage.SETTING_NAME_TEXTURE_STREAMING_MEMORY_BUDGET, self.get_texture_streaming_memory_budget()
        )
        ui_values.set(UIValuesStorage.SETTING_NAME_EARLY_QUIT_TIME_LIMIT, self._ui_early_quit_time_limit.value)
    ...
    def apply_ui_values(self, ui_values: UIValuesStorage):
        ...
        self._stored_generate_shader_cache_value = ui_values.get(
            UIValuesStorage.SETTING_NAME_GENERATE_SHADER_CACHE, False
        )
        ...
        self._ui_task_valid_frame_threshold.value = ui_values.get(
            UIValuesStorage.SETTING_NAME_MAX_BAD_FREAM_THRESHOLD, 3
        )
        ...
        self._ui_early_quit_time_limit.value = ui_values.get(UIValuesStorage.SETTING_NAME_EARLY_QUIT_TIME_LIMIT, 0)
