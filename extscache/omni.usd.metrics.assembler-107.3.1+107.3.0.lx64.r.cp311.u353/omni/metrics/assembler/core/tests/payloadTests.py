...
# Tests for divergent payload USD file
class MetricsAssemblerPayloadTests(omni.kit.test.AsyncTestCase):
    ...
    def setup_payload(self, stage, file_name):
        ...
        xform.GetPrim().GetPayloads().AddPayload(data_folder + file_name + ".usda")

    async def test_payload_divergency_mpu_check(self):
    ...
    async def test_payload_divergency_kgpu_check(self):
        stage = Usd.Stage.CreateInMemory()
        stage_id = self.setup(stage)
    ...
    async def test_payload_divergency_upaxis_check(self):
        stage = Usd.Stage.CreateInMemory()
        stage_id = self.setup(stage)
    ...
    async def test_payload_rule_resolve_attribute(self):
        stage = Usd.Stage.CreateInMemory()
        stage_id = self.setup(stage)
    ...
    async def test_payload_xform_scale_resolve(self):
        stage = Usd.Stage.CreateInMemory()
        stage_id = self.setup(stage)
        self._stageId = stage_id
        self.setup_payload(stage, "cube_meters")
