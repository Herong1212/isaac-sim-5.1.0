...
class CustomLayout:
    ...
    def _retrieve_existing_outputs(self):
        """Retrieve the output attributes that already exist on the node"""
        all_attributes = self.node.get_attributes()
        outputs = [
            attrib
            for attrib in all_attributes
            if (attrib.get_port_type() == og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT) and attrib.is_dynamic()
        ]
        return outputs
    ...
    def _add_attribute_button_build_fn(self, *args):
        ...
        def on_click_add():
            CreateAttributePopupDialog(create_dynamic_attribute, og.AttributePortType.OUTPUT)
    ...
    def _remove_attribute_button_build_fn(self, *args):
        ...
        def _remove_attribute_menu_build_fn():
            ...
            outputs = self._retrieve_existing_outputs()
