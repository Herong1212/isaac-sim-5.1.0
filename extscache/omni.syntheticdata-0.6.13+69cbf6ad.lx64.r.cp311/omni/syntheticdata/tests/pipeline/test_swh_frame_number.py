...
class TestSWHFrameNumber(omni.kit.test.AsyncTestCase):
    ...
    async def wait_for_num_sims(self, num_sims):
        await omni.syntheticdata.sensors.next_render_simulation_async(self.render_product_path(self._hydra_texture_0), num_sims)
    ...
    async def setUp(self):
        ...
        i_settings = carb.settings.acquire_settings_interface()
        ...
        self._hydra_texture_0 = create_hydra_texture(
            ...
            is_async=i_settings.get("/app/asyncRendering")
        )
        render_product_path_0 = self.render_product_path(self._hydra_texture_0)

        self._hydra_texture_1 = create_hydra_texture(
            "TEX1",
            512,
            512,
            omni.usd.get_context().get_name(),
        ...
        if not sdg_iface.is_node_template_registered("TestSyncCross"):
            ...
            sdg_iface.register_node_template(
                SyntheticData.NodeTemplate(
                    SyntheticDataStage.ON_DEMAND, # node template stage
                    "omni.syntheticdata.SdTestRationalTimeSyncGate", # node template type
                    # node template connections
                    [
