...
class TestInstanceSeg(omni.kit.test.AsyncTestCase):
    ...
    async def test_instance_seg_new_schema(self):
        """ Test sensor on new semantic schema. Please refer to this link for more details:
        https://openusd.org/release/api/usd_semantics_overview.html
        """
        settings = carb.settings.get_settings()

        path = os.path.join(FILE_DIR, "../data/scenes/new_semantic_schema.usda")
