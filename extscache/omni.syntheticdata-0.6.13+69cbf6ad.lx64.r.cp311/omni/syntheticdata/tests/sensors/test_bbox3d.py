...
class TestBBox3D(omni.kit.test.AsyncTestCase):
    ...
    async def test_parsed_nested_Y_pathtracing(self):
        ...
        for bbox, bounds in zip([parent_bbox, child1_bbox, child2_bbox], [parent_bounds, child1_bounds, child2_bounds]):
            ...
            assert np.allclose(bbox["corners"], gf_corners, atol=ATOL)
    ...
    async def test_camera_frame_simple_ftheta(self):
        ...
        camera.CreateAttribute("fthetaWidth", Sdf.ValueTypeNames.Float).Set(960)
        camera.CreateAttribute("fthetaHeight", Sdf.ValueTypeNames.Float).Set(604)
        camera.CreateAttribute("fthetaCx", Sdf.ValueTypeNames.Float).Set(460)
        camera.CreateAttribute("fthetaCy", Sdf.ValueTypeNames.Float).Set(340)
        camera.CreateAttribute("fthetaMaxFov", Sdf.ValueTypeNames.Float).Set(200.0)
        camera.CreateAttribute("fthetaPolyA", Sdf.ValueTypeNames.Float).Set(0.0)
        camera.CreateAttribute("fthetaPolyB", Sdf.ValueTypeNames.Float).Set(0.0059)
        camera.CreateAttribute("fthetaPolyC", Sdf.ValueTypeNames.Float).Set(0.0)
        camera.CreateAttribute("fthetaPolyD", Sdf.ValueTypeNames.Float).Set(0.0)
        camera.CreateAttribute("fthetaPolyE", Sdf.ValueTypeNames.Float).Set(0.0)
        ...
        corners = bbox3d_data[0]["corners"]
        projected_corners = syn.helpers.world_to_image(corners.reshape(-1, 3), self.viewport).reshape(-1, 8, 3)

        ground_truth = [
            [
                [ 0.18583049, 0.57037037, 0.0 ],
                [ 0.18229988, 0.57037037, 0.0 ],
                [ 0.19081469, 0.47716569, 0.0 ],
                [ 0.18981974, 0.45606047, 0.0 ],
                [ 0.15482905, 0.57037037, 0.0 ],
                [ 0.14503058, 0.57037037, 0.0 ],
                [ 0.16077268, 0.46745514, 0.0 ],
                [ 0.15422151, 0.44199697, 0.0 ]
            ]
        ]
        assert np.allclose(ground_truth, projected_corners)
    ...
    async def test_camera_frame_reference(self):
        ...
        assert np.allclose(bbox3d_data_world[0]["corners"], corners_tf, atol=ATOL)
        # tf = np.eye(4)
        # tf[3, 2] = 10.0
        assert np.allclose(bbox3d_data_world[0]["transform"], combined_tf, atol=ATOL)
        ...
        assert np.allclose(np.matmul(pt_camera_min, bbox3d_data_camera[0]["transform"])[:3], corners[0, 0], atol=ATOL)
        assert np.allclose(np.matmul(pt_camera_max, bbox3d_data_camera[0]["transform"])[:3], corners[0, 7], atol=ATOL)
    ...
    async def test_camera_frame_Z(self):
        ...
        # Move camera with random transform
        camera_tf = np.eye(4)
        camera_tf[:3, :3] = Gf.Matrix3d(Gf.Rotation(self.rng.random(3).tolist(), self.rng.random(3).tolist()))
        camera_tf[3, :3] = self.rng.random((1, 3))
        UsdGeom.Xformable(camera).AddTransformOp().Set(Gf.Matrix4d(camera_tf))
        camera_tf_inv = np.linalg.inv(camera_tf)
        self.viewport.camera_path = camera.GetPath()
    ...
    async def test_bbox_3d_scene_instance(self):
        ...
        await syn.sensors.create_or_retrieve_sensor_async(
            self.viewport, syn._syntheticdata.SensorType.BoundingBox3D
        )

        # Render one frame
        await syn.sensors.next_sensor_data_async(self.viewport,True)

        data = syn.sensors.get_bounding_box_3d(self.viewport, parsed=True, return_corners=True, camera_frame=True)
    ...
    async def test_bbox_3d_time_sampled(self):
        """ Test sensor on time sampled transforms. Also tests nested time samples
        """
        path = os.path.join(FILE_DIR, "../data/scenes/bbox3d_timeseries.usda")

        await omni.usd.get_context().open_stage_async(path)

        box_stage = omni.usd.get_context().get_stage()
        box_corners =  [[-2.1701815, -0.8723255,  -6.921256],
                        [ 1.7708915, -1.5539756,  -6.978676],
                        [-2.1070461, -0.34487537, -8.849419],
                        [ 1.8340269, -1.0265254,  -8.906838],
                        [-1.8340269,  1.0265254,  -6.390817],
                        [ 2.1070461,  0.34487537, -6.448237],
                        [-1.7708915,  1.5539756,  -8.318979],
                        [ 2.1701815,  0.8723255,  -8.376399]]
        ...
        self.assertAlmostEqual(bbox3d_data[0]["corners"][0][0], box_corners[0][0], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][0][1], box_corners[0][1], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][0][2], box_corners[0][2], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][1][0], box_corners[1][0], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][1][1], box_corners[1][1], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][1][2], box_corners[1][2], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][2][0], box_corners[2][0], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][2][1], box_corners[2][1], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][2][2], box_corners[2][2], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][3][0], box_corners[3][0], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][3][1], box_corners[3][1], places=2)
        self.assertAlmostEqual(bbox3d_data[0]["corners"][3][2], box_corners[3][2], places=2)
