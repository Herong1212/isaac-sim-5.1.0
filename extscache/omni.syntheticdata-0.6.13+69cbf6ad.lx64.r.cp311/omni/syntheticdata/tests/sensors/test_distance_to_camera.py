...
class TestDistanceToCamera(omni.kit.test.AsyncTestCase):
    ...
    async def test_pinhole_camera(self):
        """ Test the functionality of the sensor using a pinhole camera.
        """
        settings = carb.settings.get_settings()
        settings.set_string("/rtx/rendermode", "PathTracing")
        settings.set_int("/rtx/pathtracing/spp", 32)
        ...
        path = os.path.join(FILE_DIR, "../data/scenes/depth_pinhole.usda")
        await omni.usd.get_context().open_stage_async(path)

        # Re-create sensor when loading a new stage
        await syn.sensors.create_or_retrieve_sensor_async(self.viewport, syn._syntheticdata.SensorType.DistanceToCamera)
        ...
        golden_image = np.load(self.golden_image_path / "distance_to_camera_pinhole.npz")["array"]
