r"""Support for simplified access to data on nodes of type omni.syntheticdata.SdTestPrintRawArray
...
class OgnSdTestPrintRawArrayDatabase(og.Database):  # pragma: no cover
    ...
    class abi:
        ...
        def initialize_nodes(context, nodes):
            for n in nodes:
                OgnSdTestPrintRawArrayDatabase.abi.initialize(context, n)
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnSdTestPrintRawArrayDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnSdTestPrintRawArrayDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnSdTestPrintRawArrayDatabase._release_per_node_instance_data(node, graph_instance_id)
