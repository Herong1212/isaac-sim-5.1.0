...
class SyntheticData:
    ...
    _ogn_templates_registry = {
        ...
        _defaultSemanticFilterName: NodeTemplate(
            ...
            [NodeConnectionTemplate(_instanceMappingCtrl, attributes_mapping={"outputs:exec": "inputs:exec"})],
            attributes={"inputs:name": "default", "inputs:predicate": "*:*"}
        ),
        _defaultSemanticFilterName+"Post": NodeTemplate(
            SyntheticDataStage.POST_RENDER,
            "omni.syntheticdata.SdSemanticLabelsMap",
            [
                NodeConnectionTemplate(
                    _rendererTemplateName,
                    attributes_mapping={
                ...
                NodeConnectionTemplate(
                    _defaultSemanticFilterName,
                    attributes_mapping={"outputs:exec": "inputs:exec"}
                )
            ],
            attributes={"inputs:semanticFilterName": "default"}
        ),
        _defaultSemanticFilterName+"Ptr": NodeTemplate(
            SyntheticDataStage.ON_DEMAND,
            "omni.syntheticdata.SdSemanticLabelsMap",
            [
                NodeConnectionTemplate("PostProcessDispatch"),
                NodeConnectionTemplate(
                    _defaultSemanticFilterName+"Post",
                    attributes_mapping={"outputs:exec": "inputs:exec"}
                )
            ],
            attributes={"inputs:semanticFilterName": "default"}
    ...
    def register_display_rendervar_templates() -> None:
        ...
        for tplName in ogn_registry_keys:
            ...
            if (tplParams.node_type_id in SyntheticData._ogn_post_display_types) and (tplNameDisplay not in SyntheticData._ogn_templates_registry):
                ...
                if renderVarDisplay not in SyntheticData._ogn_rendervars:
                    SyntheticData._ogn_rendervars[renderVarDisplay] = tplName
    ...
    def register_device_rendervar_tex_to_buff_templates(rendervars: list) -> None:
        ...
        """
        cuda_copy_setting = carb.settings.get_settings().get("/exts/omni.syntheticdata/renderVarTextureToBuffer/cudaCopyNoStride")
        # copy the rendervars list since the registration may modify the list
        rendervars_copy = rendervars.copy()
        for rv in rendervars_copy:
                ...
                SyntheticData.register_node_template(
                    SyntheticData.NodeTemplate(
                        ...
                        ],
                        {
                            ...
                            "inputs:cudaCopyNoStride" : cuda_copy_setting
                        }
                    ),
                    rendervars=[rv_buff],
                    template_name=template_name,
            ...
            if template_name not in SyntheticData._ogn_templates_registry:
                SyntheticData.register_node_template(
                    SyntheticData.NodeTemplate(
                        ...
                        [
                            SyntheticData.NodeConnectionTemplate("PostProcessDispatch"),
                            SyntheticData.NodeConnectionTemplate(rv, attributes_mapping={"outputs:exec": "inputs:exec"}),
                        ],
                        {"inputs:renderVar": rv}
                    ),
                    template_name=template_name,
    ...
    @staticmethod
    def register_host_rendervar_to_disk_templates(rendervars: list) -> None:
        ...
        """
        detach_resource_default = carb.settings.get_settings().get("/exts/omni.syntheticdata/renderVarCopyToDisk/detachResourceBeforeDispatch")
        # copy the rendervars list since the registration may modify the list
        rendervars_copy = rendervars.copy()
        for rv in rendervars_copy:
            template_name = rv + "PostCopyToDisk"
            if template_name not in SyntheticData._ogn_templates_registry:
                node_template_connections = [
                    SyntheticData.NodeConnectionTemplate(rv, attributes_mapping={}),
                    SyntheticData.NodeConnectionTemplate(
                        SyntheticData._rendererTemplateName,
                        attributes_mapping={
                            "outputs:rp": "inputs:rp",
                ...
                # instance mapping base nodes are using the instance mapping trigger
                if rv in SyntheticData._ogn_rendervars and SyntheticData._ogn_rendervars[rv]=="InstanceMappingPost":
                    node_template_connections.insert(0,SyntheticData.NodeConnectionTemplate( SyntheticData._instanceMappingChangeTriggerTemplateName))
                # use the default render var to host
                else:
                    node_template_connections.insert(0,SyntheticData.NodeConnectionTemplate( SyntheticData._renderVarHostToDiskTriggerTemplateName))

                SyntheticData.register_node_template(
                    SyntheticData.NodeTemplate(
                        SyntheticDataStage.POST_RENDER,
                        "omni.syntheticdata.SdPostRenderVarHostToDisk",
                        node_template_connections,
                        {
                            "inputs:renderVar": rv,
                            "inputs:detachResourceBeforeDispatch" : detach_resource_default
                        }
                    ),
                    template_name=template_name,
                )
    ...
    def disable_async_rendering():
        ...
        if settings.get("/app/asyncRendering") or settings.get("/app/asyncRenderingLowLatency"):
            carb.log_warn("SyntheticData is not supporting asyncRendering : switching it off.")
    ...
    @staticmethod
    def _graphBackedByUsd():
        return carb.settings.get_settings().get_as_bool("/exts/omni.syntheticdata/graphBackedByUsd")
    ...
    @staticmethod
    def _get_node_path(templateOrRenderVarName: str, renderProductPath: str = None) -> str:
        templateName = templateOrRenderVarName
        if (templateName not in SyntheticData._ogn_templates_registry) and (templateName in SyntheticData._ogn_rendervars):
    ...
    def _add_node_downstream_intergraph_dependency(node, downstream_node_handle) -> int:
        ...
        if not node.get_attribute_exists(dependency_attribute_name):
            ...
            if (dep_attrib is None) or not node.get_attribute_exists(dependency_attribute_name):
                raise SyntheticDataException(f"failed to create node dependency for {node.get_prim_path()} @ {downstream_node_handle}.")
        ...
        dep_data = numpy.append(dep_attrib_data.get(),[downstream_node_handle])
        dep_attrib_data.set(dep_data)
        return len(dep_data)

    @staticmethod
    def _remove_inactive_node_downstream_intergraph_dependencies(node, active_node_handles : list) -> int:
        ...
        dep_attrib = node.get_attribute(dependency_attribute_name)
        dep_attrib_data = dep_attrib.get_attribute_data()
        dep_attrib_data.set(numpy.intersect1d(dep_attrib_data.get(), active_node_handles))
        return len(dep_attrib_data.get())

    @staticmethod
    def _connect_nodes(srcNode, dstNode, connectionMap, enable) -> bool:
        if srcNode.get_graph() == dstNode.get_graph():
            success = True
    ...
    def Initialize():
        ...
        if _sdg_iface is None:
            ...
            SyntheticData.register_host_rendervar_to_disk_templates(SyntheticData._ogn_rendervars)
    ...
    def register_node_template(node_template: NodeTemplate, rendervars: list = None, template_name: str = None) -> str:
        ...
        for conn in i_connections:
            ...
            if not type(conn_map) is dict:
                raise SyntheticDataException(
                    "failed to register node template. connection attributes map is not a dictionnary")
        ...
        if rendervars:
            if node_template.pipeline_stage != SyntheticDataStage.POST_RENDER:
                raise SyntheticDataException(
                    "failed to register node template. Only postRender nodes may produce renderVars")
            for rv in rendervars:
    ...
    def _get_active_node_handles(self):
        active_node_handles = []
        for node_graph in self._nodeGraphs.values():
            for node in node_graph.get_wrapped_graph().get_nodes():
                if node.is_valid():
                    active_node_handles.append(node.get_handle())
        return active_node_handles
    ...
    def _get_or_create_graph(self, path: str, stage: int, renderProductPath: object) -> object:
        ...
        # Note: We use this variable to disable USD backing if a prim already exists at the graph's
        # given path, allowing the graph to still be constructed (DriveSim had some issues related
        # to this, hence described approach).
        prim_exist_workaround = not omni.usd.get_context().get_stage().GetPrimAtPath(path)
        ...
        nodeGraph = orchestration_graphs[0].create_graph_as_node(
            name=path.replace("/", "_"),
            path=path,
            evaluator=executionModel,
            is_global_graph=True,
            is_backed_by_usd=SyntheticData._graphBackedByUsd() and prim_exist_workaround,
    ...
    def _activate_node_rec(self, templateName: str, renderProductIndex: int = -1, renderProductPaths: list = None, render_var_activations: dict = None) -> None:
        ...
        if templateName in SyntheticData._ogn_rendervars:
            ...
            templateName = SyntheticData._ogn_rendervars[templateName]
            if (render_var_activations is not None) and renderProductPath and (templateName == SyntheticData._rendererTemplateName):
        ...
        for attrName, attrVal in template.attributes.items():
            if node.get_attribute_exists(attrName):
                og.AttributeValueHelper(node.get_attribute(attrName)).set(attrVal, update_usd=True)
            else:
        ...
        if renderProductPath and node.get_attribute_exists(SyntheticData._renderProductAttributeName):
            og.AttributeValueHelper(
                node.get_attribute(SyntheticData._renderProductAttributeName)
            ).set(renderProductPath, update_usd=True)
    ...
    def _deactivate_node_rec(
        ...
        if templateName in SyntheticData._ogn_rendervars:
            renderVarName = templateName
            templateName = SyntheticData._ogn_rendervars[templateName]
            if (render_var_deactivations is not None) and renderProductPath and (templateName == SyntheticData._rendererTemplateName):
        ...
        for attr in node.get_attributes():
            if attr.get_downstream_connection_count():
                return templateName
        # abort if the node has a downstream dependency
        if active_node_handles is None:
           active_node_handles = self._get_active_node_handles()
        if SyntheticData._remove_inactive_node_downstream_intergraph_dependencies(node, active_node_handles) > 0:
            return templateName

        active_node_handles.remove(node.get_handle())
    ...
    def _post_process_graph_tick(self, e: carb.eventdispatcher.Event):
    ...
    def __init__(self) -> None:
        ...
        self._post_process_graph_tick_sub = carb.eventdispatcher.get_eventdispatcher().observe_event(
            event_name=omni.kit.app.GLOBAL_EVENT_UPDATE,
            on_event=self._post_process_graph_tick,
            order=SyntheticData.post_process_graph_tick_order(),
            observer_name="[omni.syntheticdata] PostProcessGraph Tick"
        )
    ...
    def get_semantic_filter_label_template_name(self, filter_name: str, post_render_stage:bool=True):
        ...
        Returns:
            the template name of the node exposing the semantic labels of the given semantic filter
        """
        template_name = SyntheticData._defaultSemanticFilterName if filter_name == "default" else filter_name
        return template_name + ("SemanticFilterPost" if post_render_stage else "SemanticFilterPtr")
    ...
    def set_semantic_filter(self, filter_name: str, predicate:str, hierarchical_labels=False, matching_labels=True):
        ...
        Args:
            filter_name
            predicate : a semantic filter predicate. (if empty the filter will be removed)
            hierarchical_labels : option to propagate semantic labels within the hiearchy, from parent to childrens
            matching_labels : option to remove from the set of labels the one that do not match the predicate
        ...
        if filter_name == "default":
            self.set_default_semantic_filter(predicate, hierarchical_labels, matching_labels)
        else:
            template_name = filter_name + "SemanticFilter"
            if template_name not in SyntheticData._ogn_templates_registry:
                SyntheticData.register_node_template(
                    SyntheticData.NodeTemplate(
                        SyntheticDataStage.SIMULATION,
                        ...
                        [
                            SyntheticData.NodeConnectionTemplate(
                                SyntheticData._instanceMappingCtrl,
                                attributes_mapping={"outputs:exec": "inputs:exec"}
                            )
                        ],
                        attributes={"inputs:name": filter_name, "inputs:predicate": predicate}
                    ),
                    template_name=template_name
                )
                SyntheticData.register_node_template(
                    SyntheticData.NodeTemplate(
                        ...
                        attributes={"inputs:semanticFilterName": filter_name}
                    ),
                    template_name=template_name+"Post"
                )
                SyntheticData.register_node_template(
                    SyntheticData.NodeTemplate(
                        SyntheticDataStage.ON_DEMAND,
                    ...
                    template_name=template_name+"Ptr"
                )
            SyntheticData._ogn_templates_registry[template_name].attributes["inputs:predicate"] = predicate
            SyntheticData._ogn_templates_registry[template_name].attributes["inputs:hierarchicalLabels"] = hierarchical_labels
            SyntheticData._ogn_templates_registry[template_name].attributes["inputs:matchingLabels"] = matching_labels
            node_path = SyntheticData._get_node_path(template_name)
            if node_path in self._graphNodes:
                self.set_node_attributes(
                    template_name,
                    attributes={
                        "inputs:predicate":predicate,
                        "inputs:hierarchicalLabels":hierarchical_labels,
                        "inputs:matchingLabels": matching_labels
                    }
                )
    ...
    def get_semantic_filter(self, filter_name:str):
        ...
        Returns:
            the predicate of the semantic filter
        """
        if filter_name == "default":
            return self.get_default_semantic_filter()
        else:
            template_name = filter_name + "SemanticFilter"
            attr = self.get_node_attributes(template_name, ["inputs:predicate"])
            return attr["inputs:predicate"] if "inputs:predicate" in attr else "*"
