...
class VisualizerWindow:
    ...
    def __init__(self, name, viewport_api):
        ...
        self.__frame_changed_sub = carb.eventdispatcher.get_eventdispatcher().observe_event(
            event_name=omni.kit.app.GLOBAL_EVENT_UPDATE,
            order=SyntheticData.post_process_graph_tick_order() + 1,
            on_event=self.__frame_changed,
            observer_name="[omni.syntheticdata] VisualizerWindow"
        )

    def __frame_changed(self, e: carb.eventdispatcher.Event):
        render_product = self._render_var_combo_model.get_render_product_path()
        self.update(render_product, None)
    ...
    def update(self, render_product_path: str, stage=None):
