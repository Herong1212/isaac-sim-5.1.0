...
from omni.importer.onshape.scripts.preferences import OnshapeImporterPreferences
from omni.importer.onshape.widgets.assembly_widget import Mate
from omni.importer.onshape.widgets.visual_materials_widget import VisualMaterial
...
def make_index(array_with_duplicates):
    new_array = set(array_with_duplicates)
    index_map = {}
    index = []
    for item in array_with_duplicates:
        if item not in index_map:
            index_map[item] = len(index_map)
        index.append(index_map[item])
    return list(new_array), index
...
DEFAULT_TEMP_FOLDER_SETTING = "/ext/omni.importer.onshape_importer/default_temp"
...
class UsdGenerator:
    ...
    def set_payload_mesh(self, part):
        ...
        try:
            ...
            if self.rig_physics:
                # Apply collision APIs directly to the mesh
                UsdPhysics.CollisionAPI.Apply(mesh_prim)
                collisionAPI = UsdPhysics.MeshCollisionAPI.Apply(mesh_prim)
                    ...
                    if (a_id, f_id) in self.groupMates:
                        ...
                        if not g_prim:
                            ...
                            for i in range(len(pending_parents) - 1, -1, -1):
                                prim = UsdGeom.Xform.Define(stage, pending_parents[i]).GetPrim()
                                model_api = Usd.ModelAPI(prim)
                                model_api.SetKind(Kind.Tokens.assembly)
        ...
        if 1:
            if (
                ...
                for f_id in [
                    ...
                    Usd.ModelAPI(body_0).SetKind(Kind.Tokens.group)
                    Usd.ModelAPI(body_1).SetKind(Kind.Tokens.group)
    ...
    def _delayed_make_assembly(self):
        with self.assembly_lock:
            try:
                ...
                Usd.ModelAPI(UsdGeom.Xform.Define(stage, self.root).GetPrim()).SetKind(Kind.Tokens.assembly)
                model_api = Usd.ModelAPI(_root)
                model_api.SetKind(Kind.Tokens.assembly)
