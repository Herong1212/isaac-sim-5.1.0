...
class RandomizerUtil:
    ...
    AIM_CAMERAS_AT_CHARACTERS = "/persistent/exts/isaacsim.replicator.agent/aim_cameras_at_characters"
    ...
    # This function decomposes a distance into two random vectors using a local RNG
    @staticmethod
    def decompose_distance(distance, seed=None):
        # Use a per-call RNG to avoid global RNG state pollution
        rng = np.random.default_rng(seed)
        # Sample x in [-distance, distance]
        x = float(rng.uniform(-distance, distance))
        # Guard against tiny negative due to float error inside the sqrt
        rem = max(0.0, distance * distance - x * x)
        y = math.sqrt(rem)
        # Randomly flip sign of y
    ...
    @staticmethod
    def aim_at_characters():  # noqa
        aim_cameras_at_characters = carb.settings.get_settings().get(RandomizerUtil.AIM_CAMERAS_AT_CHARACTERS)
        return bool(aim_cameras_at_characters)

    @staticmethod
    def set_aim_cameras_at_characters(usr_input):
        carb.settings.get_settings().set(RandomizerUtil.AIM_CAMERAS_AT_CHARACTERS, usr_input)
    ...
    # Collect all valid camera raycast hits around each character.
    @staticmethod
    def collect_character_raycast_hits(
        character_list: Iterable[str],
        raycast_angle_step: int = 5,
    ) -> List[RayCast_Container]:
        """
        Build a flat list of valid raycast hit descriptors for a set of characters.

        - character_list: iterable of character prim paths.
        - raycast_angle_step: angular step in degrees used to sample the sphere
          around each character.

        Returns a list of RayCast_Container instances, one per valid direction-
        distance range.
        """
        hits: List[RayCast_Container] = []
        for character in character_list:
            radius, center = RandomizerUtil.get_character_radius_and_center(character)
            character_hits = RandomizerUtil.get_character_surronding_raycast(
    ...
    # compute a camera orientation that looks at the character from the given position
    @staticmethod
    def compute_camera_look_at_character_rotation(character_prim_path, camera_position):
        in_view, focus_point = RandomizerUtil.check_character_visible_in_pos(character_prim_path, camera_position)
        if not in_view or focus_point is None:
            return None
        return Gf.Quatd(
            lookat_to_quatf(
